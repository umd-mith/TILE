...
void TesseractLinesOfImage(const char* input_file, IMAGE* image, STRING* text_out){
	int bytes_per_line = check_legal_image_size(image->get_xsize(),
                                              image->get_ysize(),
                                              image->get_bpp());
     //output a txt file that has the Math for all of the lines
     //want to be able to eventually create RECT objects out of this data
     //A Grant Dickie Joint
     char* text;
...
int main(int argc, char **argv) {
  ...
  if (len > 3 && strcmp("tif", argv[1] + len - 3) == 0) {
    ...
    do {
      ...
      //TesseractLinesOfImage(argv[1],&image,&text_out);
