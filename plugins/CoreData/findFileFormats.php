...
class FileFormatSearch{
	...
	public function search(){
		...
		/*
		if($handle=opendir('.')){
			# find correct file ending in import.php and
			# include it
			while(($file=readdir($handle))){
				if(preg_match('/import\.php$/i',$file)>0){
					#include_once($file);
			...
			# check coredata for all registered namespaces
			// $parser=new CoreData('',array());
			// $html=$parser->parse_namespaces();
			$html=$this->processList();
			return $html;
