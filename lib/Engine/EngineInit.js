...
var EngineInit=Monomyth.Class.extend({
	...
	//NEW: called by sideBarDone custom event
	loadCanvas:function(){
	...
	loadJSONImages:function(e,r,p){
		...
		// 			});
		// 		});
		//this.preLoadData=imgs; //set PreLoadData variable for when TabBar loads tag data
		var obj=e.data.obj;
		var imgs=p.Images;
		//go through and add _base url to each uri
		for(i in imgs){
			imgs[i].uri=obj._base+imgs[i].uri;
			for(s in imgs[i].shapes){
				imgs[i].shapes[s].uri=obj._base+imgs[i].shapes[s].uri;
			}
		}
	
		obj.raphael.addUrl(imgs);
		//this.Scroller.loadImages(this.raphael.getUrlList());
		//inner function to handle when Raphael has set current URL
		var _d=function(e){
			var obj=e.data.obj;
			$("body").unbind("RaphaelImageReady",_d); //release inner function from repeating
			
			obj.SideBar.loadJSONTags(imgs,$("#srcImageForCanvas").attr("src"));
		};
		//bind inner function 
		$("body").bind("RaphaelImageReady",{obj:obj},_d);
		obj.DOM.trigger("imageAdded");
	},
	saveSettings:function(e){
		...
		//ADD SCHEMA
		data.schema=obj.jsonReader.schemaFile.replace(obj._base,"");
		...
		//get rid of base in all Image tag uris and schema
		for(img in data.Images){
			data.Images[img].uri=data.Images[img].uri.replace(obj._base,"");
		}
