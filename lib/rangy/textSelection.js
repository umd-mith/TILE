...
var TS={
	start:function(e,json,layout){
		...
		self.getHLite=$("#getHLite").bind('click',function(e){
			...
			$("body:first").trigger("addLink",[{id:sel.id,type:"highlights"}]);
	...
	addLinkHandle:function(e,ref){
		...
		for(m in self.manifest[url]){
			if(self.manifest[url][m].id==self.activeSel){
				if(self.manifest[url][ref.type]){
					if($.inArray(ref.id,self.manifest[url][ref.type])<0){
						self.curLink=ref;
						// add to array
						self.manifest[url][ref.type].push(ref.id);
					...
					self.curLink=ref;
					self.manifest[url][ref.type]=[ref.id];
					self._attachLinkDiv(ref);
	...
	_loadItemsHandle:function(e,data){
		...
		for(x in self.manifest[url]){
			if($.inArray(self.manifest[url][x].id,data)){
