...
var TS={
	...
	_attachLinkDiv:function(ref,sel){
		...
		$("<span class=\"button\">"+ref.id+"<span id=\"del"+self.activeSel+ref.id+"\" class=\"button\">X</span></span>").appendTo($("span."+self.activeSel));
		$("#del"+self.activeSel+ref.id).bind("click",function(e){
			...
			if(__v) console.log("found the ref.id at: "+n);
			if((n==null)||(n==-1)) return;
			
			
			if(self.curLink.id==ref.id) self.curLink=null;
			$(this).parent().remove();
			$("body:first").trigger(self.deleteCall,{id:ref.id,type:t,tool:tool,parentTool:self.id,parentObj:sel.id,parentType:"selections"});
			// $(".line_selected").removeClass("line_selected");
			return;
		});
	},
