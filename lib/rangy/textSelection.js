	...
	start:function(){
		...
		self.getHLite=$("#getHLite").bind('click',function(e){
			...
			if($("span."+sel.id).length>1){
				$("<span id=\"deleteHLite"+sel.id+"\" class=\"button\">Delete</span>").appendTo($("span."+sel.id+":eq("+($("span."+sel.id).length-1)+")"));
			} else {
				$("<span id=\"deleteHLite"+sel.id+"\" class=\"button\">Delete</span>").appendTo($("span."+sel.id));
			}
			
			$("span."+sel.id+" > #colorSelector").addClass("btnIconLarge");
			
			// $("#hideHLite").click(function(e){
			// 			$(this).parent().hide();
			// 		});
