...
var Dashboard={
	start:function(engine){
		...
		// create html
		self.html='<div class="header"><a id="backToViews">Home</a></div><div id="dashboard_main" class="body"></div>';
		self.startHTML='<div class="dashboard_list"><a id="seeJSON">JSON View</a><br/></div>';
		// create mode with a callback
		var onActive=function(e){
			$("#dashboard_main").empty().append(self.startHTML);
		};
	...
	displayOnDashboardHandle:function(e,o){
		...
		// o must have a mode 
		if(!self.modeData[o.mode]){
			self.modeData[o.mode]=o;
			self.startHTML='<div class="dashboard_list"><a id="seeJSON">JSON View</a><br/>';
			for(var t in self.modeData){
				
				self.startHTML+='<a id="'+t+'" class="dashModeItem">'+t+'</a><br/>';
			}
			self.startHTML+='</div>';
		} else {
			self.modeData[o.mode]=o;
