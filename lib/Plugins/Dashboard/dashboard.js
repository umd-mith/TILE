...
var Dashboard={
	start:function(engine){
		...
		// create html
		self.html='<div class="header"><a id="backToViews">Home</a></div><div id="dashboard_body" class="body"><div id="dashboard_main"></div></div>';
		// main page that users begin with
		self.startHTML='<div id="startPageDashboard" class="dashboard_list"><h3><a id="seeJSON">JSON View</a></h3><br/><p>View the Core Data as a list.</p><h3>Modes:</h3><div id="modeChoose"></div><div id="modules"></div></div>';
		...
		$(self.startHTML).insertBefore($("#dashboard_main"));
	...
	displayOnDashboardHandle:function(e,o){
		...
		if($("#modules > #"+o.mode).length){
			$("#modules > #"+o.mode).empty().append(o.html);
		} else {
	...
	displayMode:function(mode){
		...
		$("#dashboard_main").empty().append(shell);
	},
