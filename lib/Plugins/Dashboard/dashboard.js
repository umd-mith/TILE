...
var Dashboard={
	...
	displayLabels:function(){
		var self=this;
		var html='<div id="labelmodule" class="dashboard_table"><a id="close">X</a><h3>Current Labels:</h3></div>';
	...
	createPageHTML:function(n){
		...
		for(var x in self.data.pages[n]){
			if(($.isArray(self.data.pages[n]))&&(!(/info/.test(x)))){
				html+='<div id="'+x+'" class="itemlist">';
				for(var prop in self.data.pages[n][x]){
					html+='<div class="item"><a id="'+self.data.pages[n][x].id+'">'+self.data.pages[n][x].id+'</a></div>';
				}
				html+='</div>';
			}
		}
