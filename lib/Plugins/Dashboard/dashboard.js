...
var Dashboard={
	...
	createPagesHTML:function(){
		var self=this;
		if(__v) console.log("create pages");
		// collect data
		var columns=[];
		var headers=['pages'];
		...
		// create Table
		var html='<table id="pagedata" class="pagelist"><thead><tr>';
		var rowhtml='';
		// headers
		...
		for(var row in columns){
			...
			$(".dashboard_main > #pagedata > tbody").append('<tr id="t'+row+'">'+rowhtml+'</tr>');
			for(var prop in columns[row]){
				// attach cell data
				if(prop=='pages'){
					$('.pagelist > tbody > #t'+row+' > td > #'+prop).append('<a id="'+row+'">'+columns[row][prop]+'</a>');
				} else {
					$('.pagelist > tbody > #t'+row+' > td > #'+prop).append('<p>'+columns[row][prop]+'</p>');
				}
			}
		}
