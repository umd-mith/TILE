...
var LB={
	...
	start:function(engine){
		...
		for(var x in json.labels){
			...
			self.metaData.push(lbl);
		}
		if(__v) console.log('engine getting '+JSON.stringify(self.metaData)+' in labels');
		if(self.metaData.length) engine.insertTags(self.metaData);
		
		
		// TODO: create persistent state?
		$("body").bind("newPageLoaded",function(e){
			// reset label states - no persistence across pages
			$(".labelItem").removeClass('active');
		});
	...
	loadJSON:function(engine){
		...
		for(var x in data.labels){
			var lbl=$.extend(data.labels[x],{type:"labels",tool:self.id});
			self.metaData.push(lbl);
		}
		if(__v) console.log('engine getting '+JSON.stringify(self.metaData)+' in labels');
