...
var TS={
	...
	start:function(engine){
		...
		var getHLite=function(e){
			...
			// engine.setActiveObj();
			$(".line").unbind();
			$(".line").bind('mouseup',{obj:self,engine:engine},function(e){
				var self=e.data.obj;
				self.createHighlight(e.data.engine);
			});
		};
	...
	dataAddedHandle:function(e,o){
		...
		for(var x in o.activeItems){
			...
			for(var y in self.manifest[TILEPAGE]){
				if(self.manifest[TILEPAGE][y].id==id){
					data.push(id);
				}
			}
		}
		...
		self._loadItemsHandle(data,o.engine);
	},
	newActiveHandle:function(e,o){
		...
		var data=[];
		for(var x in o.activeItems){
			if(o.activeItems[x].type!='selections') continue;
			var id=o.activeItems[x].id;
			for(var y in self.manifest[TILEPAGE]){
		...
		// nothing found - stop here
		if(data.length==0) return;
		if(__v) console.log("newactive in textsel: "+JSON.stringify(data));
		self._loadItemsHandle(data,o.engine);
	},
	createHighlight:function(engine){
		var url=TILEPAGE;
		var self=this;
		// de-select all transcript lines
		$(".line_selected").removeClass("line_selected");
	...
	// e : {event}
	// data : {Object}
	_loadItemsHandle:function(data,engine){
		var self=this;
		...
		engine.attachMetadataDialog(vd[0],"span."+vd[0].id+":eq("+($("span."+vd[0].id).length-1)+")");
