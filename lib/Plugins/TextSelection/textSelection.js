...
var TS={
	...
	start:function(json,layout){
		...
		// make sure that listener for mouseup is called off when mousing over
		// a highlight
		$("span[class^='anno']").live("mouseover mousemove",function(e){
			$(".line").unbind("mouseup");
		});	
		$("span[class^='anno']").live("mouseout",function(e){
			$(".line").bind("mouseup",function(e){
				self.createHighlight();
			});
		});
		$("span[class^='anno']").live('mousedown click',function(e){
			e.stopPropagation();
	...
	createHighlight:function(){
		...
		$("body:first").trigger(self.outputCall,[{id:sel.id,type:"selections",display:"..."+$(sel.StartParent).text().substring(0,10)+"...",attachHandle:"."+sel.id+":eq("+($("."+sel.id).length-1)+")",tool:self.id,parentTool:null}]);
	...
	attachButtons:function(sel){
		...
		$("#deleteHLite"+id).bind("click",function(e){
			...
			var id=$(this).attr('id').replace("deleteHLite","");
			var url=$("#srcImageForCanvas").attr('src');
	...
	selectHighlight:function(id){
		...
		if(!h) return;
		if(__v) console.log(h.id);
		
		var handle="span."+h.id+":eq("+($("span."+h.id).length-1)+")";
		
		
		$("body:first").trigger(self.outputCall,[{id:h.id,type:'selections',parentTool:'none',tool:self.id,attachHandle:handle}]);
	},
	...
	// TODO: get rid of this or change it
	...
	// 		
	// 		$("<span class=\"button\">"+ref.display+"<span id=\"del"+sel.id+ref.id+"\" class=\"button\">X</span></span>").appendTo(elem);
	// 		$("#del"+sel.id+ref.id).bind("mousedown click",function(e){
	// 			
	...
	_objChangeHandle:function(e,changes){
		...
		if(changes.type=='color'){
			...
			for(var url in self.manifest){
				for(var sel in self.manifest[url]){
					if(self.manifest[url][sel].id==ref.id){
						self.manifest[url][sel].color=changes.value;
