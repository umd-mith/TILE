	...
	**/
	var TileOCR=function(args){
		// Constructor
			...
			$("<div class=\"az inner\"><div class=\"autorec_toolbar\"><div class=\"toolbar\">Auto Line Recognizer<div class=\"menuitem\">"+
			"<span class=\"btnIconLarge close\"></span></div></div><div id=\"content\" class=\"az\"><div id=\"sidebarHead\"><div id=\"boxSize\">"+
			...
			"<a id=\"autorec_recognize\" class=\"button\">Go</a></div></div>").appendTo($("#az_log"));
	...
	TileOCR.prototype={
		...
		swapCanvas:function(){
			...
			if(self.canvasArea){
				...
				$("body").live("resizestop",function(e){
					if($("#regionBox").length){
						self.CANVAS._resetCanvasImage();
						self.CAR.thresholdConversion();
					}
		...
		_restart:function(transcript){
			...
			if(self.CANVAS) {
				...
				$(".az.main > .az.tool").animate({opacity:0.25,left:n},10,function(e){
					...
					$('#azcontentarea > .az.inner.autoRec').show();
					// self.DOM.show();
		...
		**/
		...
		_recognize:function(){
			...
			//	self.regionList[self.curRegion].images.push($("#srcImageForCanvas").attr('src'));
				...
				if(__v) console.log('scalecorrectx: '+scalecorrectx+' scalecorrecty: '+scalecorrecty);
				//get the current region 
				var _REG=self.regionList;
				
				var left = _REG.left;
		           
		 		var tbarcorrect=($("#azcontentarea > .az.inner.autoRec > .toolbar").innerHeight()+$("#azcontentarea > .az.inner.autoRec").scrollTop());
				...
				if(__v) console.log("alphaTop: "+alphaTop+"  tbarcorrect: "+tbarcorrect);
				...
				for (var i = 0; i < bucket.length; i++) {
					...
					if(__v){
						console.log('autoline values: ');
						console.log("left: "+left+"  top: "+top+"  width: "+_REG.width+"    height: "+height);
					}
					...
					if(self.activeLines[i]){
						...
						ldata.push({"id":"D_"+id+"_shape","type":"rect","_scale":1,"color":"#FFFFFF","posInfo":{"x":(left*scalecorrectx),"y":((top*scalecorrecty)),"width":(_REG.width*scalecorrectx),"height":(height)*scalecorrecty}});
	 ...
	 */
	 var CanvasImage=function(args){
	 	// Constructor
			...
			// attach html
	 		this.loc.append($("<div class=\"az inner autoRec\"><div class=\"toolbar\"><ul class=\"menuitem\"><li>"+
			"<a href=\"#\" id=\"zoomIn\" class=\"btnIconLarge\"></a></li><li><a href=\"#\" id=\"zoomOut\" class=\"btnIconLarge\"></a>"+
			"</li></ul></div><div id=\"canvasHTML"+this.uid+"\" class=\"workspace\"><canvas id=\"canvas\"></canvas></div></div>"));
			...
			$(".az.inner.autoRec > .toolbar > ul > li > a#zoomIn").live('click',function(e){
				e.preventDefault();
				// fire zoom trigger for AR
				$("body:first").trigger("zoomAR",[1]);
			});
			$(".az.inner.autoRec > .toolbar > ul > li > a#zoomOut").live('click',function(e){
				e.preventDefault();
				// fire zoom trigger for AR
				$("body:first").trigger("zoomAR",[-1]);
			});
	...
	CanvasImage.prototype={
		...
		**/
		var RegionBox=function(args){
			// Constructor
				if(!$(".az.inner.autoRec").length) throw "RegionBox cannot be inserted at this point";
				var self=this;
				self.loc=$(args.loc);
				self.DOM=$("<div id=\"regionBox\" class=\"boxer_plainbox\"></div>");
				self.DOM.appendTo($("#azcontentarea > .az.inner.autoRec"));
				//adjust top/left
				var p=$(".az.inner.autoRec").position();
				if(__v) console.log("toolbar height ar: "+$("#azcontentarea > .az.inner.autoRec > .toolbar").innerHeight());
				self.DOM.css({"left":0,"top":(p.top+$("#azcontentarea > .az.inner.autoRec > .toolbar").innerHeight())});
			...
			CanvasAutoRecognizer.prototype={
				...
				thresholdConversion:function(){
					...
					if(this.Region){
						...
						var rt = parseInt(this.Region.css('top'),10)-$('#azcontentarea > .az.inner.autoRec > .toolbar').innerHeight();
		...
		if(!self.__AR__){
			...
			var button={
				...
				click:function(e){
					...
					$("#azcontentarea > .az.inner").hide();
					$("#azcontentarea > .az.inner.autoRec").show();
					// adjust the regionBox
					$("#regionBox").css({"left":0,"top":$("#azcontentarea > .az.inner.autoRec > .toolbar").innerHeight()});
					
					// $("#zoomOut").css("background","blue");
					...
					// 					$(".toolbar > ul > li > a > .imgView").parent().parent().parent().hide();
					// $("#srcImageForCanvas").hide();
