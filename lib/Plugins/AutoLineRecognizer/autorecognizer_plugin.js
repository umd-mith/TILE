	...
	**/
	var TileOCR=function(args){
		// Constructor
			...
			"<a id=\"autorec_recognize\" class=\"button\">Go</a></div></div>").appendTo(self.loc);
	...
	TileOCR.prototype={
		...
		_loadTranscript:function(){
			var self=this;
			var out="";
			if(__v) console.log("autorec transcript: ");
			if(__v) console.log(JSON.stringify(self.transcript));
			if(self.transcript){
	...
	CanvasImage.prototype={
		...
		setUpCanvas: function(url){
			...
			$("#srcImageForCanvas").load(function(e){
				...
				$("#"+self.uid).height($("#azcontentarea").height()-$("#azcontentarea > .az.inner > .toolbar").innerHeight());
				$(this).unbind("load");
				if(__v) console.log($("#regionBox").css("display"));
			});
			...
			CanvasAutoRecognizer.prototype={
				...
				thresholdConversion:function(){
					...
					if(this.Region){
						...
						var rl = $("#azcontentarea").offset().left-parseInt(this.Region.css('left'),10);
						var rt = parseInt(this.Region.css('top'),10)-$("#azcontentarea").offset().top;
						
						var rw = parseInt(this.Region.css('width'),10);
						var rh = parseInt(this.Region.css('height'),10);
						if(__v) console.log("this.Region has: "+rl+", "+rt+", "+rw+", "+rh);
...
var AR={
	...
	start:function(engine){
		...
		if(!self.__AR__){
			...
			var button={
				...
				click:function(e){
					...
					if(__v) console.log("AR running with: "+JSON.stringify(data));
					// if(!data.lines){
					// 						for(var d in data['pages']){
					// 							if(data['pages'][d].url==TILEPAGE){
					// 								data=data['pages'][d].url;
					// 							}
					// 						}
					// 					}
					...
					$("#srcImageForCanvas").hide();
					// $("body:first").trigger("ARrestart",[data]);
					// ARrestart(data);
		...
		// 			// restart function - set up toolbar
		// 			$(".toolbar > ul > li > #pgPrev").hide();
		// 			$(".toolbar > ul > li > #pgNext").hide();
		// 			$(".toolbar > ul > li > #pointer").hide();
		// 			$(".toolbar > ul > li > a > #listView").parent().hide();
		...
		// 			if (!data.lines) {
		// 
		// 				data=data[$("#srcImageForCanvas").attr("src")];
		// 			};
		// 
		// 			//already constructed, initiate main engine restart function
		// 			this.__AR__._restart(data);
		// 		}
	},
