		...
		_recognize:function(){
			...
			//	self.regionList[self.curRegion].images.push($("#srcImageForCanvas").attr('src'));
				...
				var left = _REG.left;
		 		...
		 		var tbarcorrect=($("#azcontentarea > .az.inner.autoRec > .toolbar").height()+$("#azcontentarea > .az.inner.autoRec > .workspace").position().top+$("#azcontentarea > .az.inner.autoRec").scrollTop());
				...
				for (var i = 0; i < bucket.length; i++) {
					...
					if(self.activeLines[i]){
						...
						ldata.push({"id":"D_"+id+"_shape","type":"rect","_scale":TILEIMGSCALE,"color":"#FFFFFF","posInfo":{"x":(left),"y":(top),"width":(_REG.width),"height":(height)}});
	...
	CanvasImage.prototype={
		zoomHandle:function(e,v){
			...
			if($("#regionBox").length){
				...
				var t=$("#azcontentarea > .az.inner.autoRec > .workspace").position().top+5;
				$("#regionBox").width(w);
				$("#regionBox").height(h);
	...
	start:function(engine){
		...
		if(!self.__AR__){
			...
			var button={
				...
				type:'global',
				helptext:'Automatically recognizes lines'
			};
