	 ...
	 */
	 var CanvasImage=function(args){
	 	// Constructor
			...
			this.loc = $("#azcontentarea > .az.inner:eq(1)");
			if(this.loc){
		...
		**/
		var RegionBox=function(args){
			// Constructor
				...
				self.DOM.appendTo($("#azcontentarea > .az.inner:eq(1)"));
				//adjust top/left
			...
			CanvasAutoRecognizer.prototype={
				...
				thresholdConversion:function(){
					...
					if(this.Region){
						...
						var rt = parseInt(this.Region.css('top'),10)-$('.toolbar').innerHeight();
...
var AR={
	...
	start:function(engine){
		...
		if(!self.__AR__){
			...
			if(__v) console.log("AZ LOG IS FRICKIN: "+$("#az_transcript_bar").css('display'));
			...
			var button={
				...
				click:function(e){
					...
					$("#az_activeBox").hide();
					$("#azcontentarea > .az.inner:eq(0)").hide();
					$("#azcontentarea > .az.inner:eq(1)").show();
					...
					$(".toolbar > ul > li > .imgView").parent().parent().hide();
					// $("#srcImageForCanvas").hide();
