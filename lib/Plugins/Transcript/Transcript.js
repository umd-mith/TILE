...
(function ($, R, _) {
	...
	var Trans={
		...
		start:function(engine){
			...
			if(data&&(data.pages)){
				...
				for(var page in data.pages){
					if(data.pages[page].lines){
						text[data.pages[page].url]=[];
						for(var line in data.pages[page].lines){
							text[data.pages[page].url].push(data.pages[page].lines[line]);
						}
					}
				}
			...
			// var text=data.lines;
			self.transcript=new Transcript({text:text,loc:'logbar_list'});
			...
			var button={
				id:'getTrans',
				helptext:'Activate Transcript Mode',
				
				click:clickTrans,
				mousedown:clickTrans
			};
		...
		newActiveHandle:function(e,o){
			...
			$(".line_selected").removeClass("line_selected");
			if(__v) console.log('activehandle reached in transcript: '+JSON.stringify(o.activeItems));
			// check to see if activeItems contain ID for 
			// lines
			var sels=[];
			for(var prop in o.activeItems){
				if(!o.activeItems[prop]) continue;
				if(o.activeItems[prop].type!='lines') continue;
				if(__v) console.log("active handle in transcript: "+JSON.stringify(o.activeItems[prop]));
				if($("#"+o.activeItems[prop].id).length){
					// line in DOM;
					// set line as active 
