	...
	var Trans={
		...
		loadJSON:function(engine){
			...
			if(data&&(data.pages)){
				// parse out data
				for(var page in data.pages){
					if(!data.pages[page]) continue;
					if(data.pages[page].lines){
						if(!text[data.pages[page].url]) text[data.pages[page].url]=[];
						
						for(var line in data.pages[page].lines){
							if(!data.pages[page].lines[line]) continue;
							
							text[data.pages[page].url].push(data.pages[page].lines[line]);
						}
						
						if(__v) console.log('textlines in: '+JSON.stringify(text[data.pages[page].url]));
					}
				}
