...
(function ($, R, _) {
	...
	function Transcript(args){
		...
		if(this.lineArray){
			...
			if(__v) console.log("manifest for Transcript in url "+$("#srcImageForCanvas").attr('src'));
			if(__v) console.log(JSON.stringify(self.lineArray));
		}
		this.curLine=0;
		this.curUrl=null;
	...
	var Trans={
		...
		_trnsClickHandle:function(e,id){
			var self=e.data.obj;
			$("body:first").trigger(self.activeCall,[self.id,{id:id,type:'lines',jsonName:$("#srcImageForCanvas").attr('src'),obj:{id:id,type:'lines'}}]);
