			...
			for(var v in vd){
				...
				for(var sh in self.manifest[url].shapes){
					if(self.manifest[url].shapes[sh].id==id){
						if(__v) console.log(JSON.stringify(self.manifest[url].shapes[sh].posInfo)+"  changed to: "+JSON.stringify(vd[v].posInfo));
...
var IT={
	...
	start:function(json,layout){
		var self=this;
		
		// var id="azcontentarea"; //defaults to TILE page
		this.itagger=new _Itag({loc:"azcontentarea",json:(json)?json:null,layout:layout});
		this.linkManifest=[];
