...
var IT={
	...
	start:function(engine){
		...
		if(!self.itagger){
			...
			if(__v) console.log('setting up engin events');
			// bind ENGINE events
			...
			var button={
				...
				helptext:'Annotate an image using shapes and attach metadata to shapes and transcript lines',
				category:'mode',
				type:'global'
			};
			
			self.activeButton=engine.addToolBarButton(button);
			
			self.activeButton.elem.live('click',function(e){
				e.preventDefault();
				self.activeButton.makeActive();
				// change classes for log area
				$("#az_log").removeClass("tool").addClass("log");
				
				// set up transcript area 
				$("#az_log > .az.inner").hide();
				$("#az_log > #az_transcript_area").show();
				// show active box
				$("#az_activeBox").show();
				// show label browser
				if($("#az_LabelBox").length){
					$("#az_activeBox > .az.inner").hide();
					$("#az_LabelBox").show();
				}
				
				// activate canvas area
	...
	dataAddedHandle:function(e,o){
		if(__v) console.log("DATA ADDED CALLED");
		var self=e.data.obj;
		...
		var vd=[];
		if(__v) console.log('dataadded handle in imagetagger: '+JSON.stringify(o.activeItems));
		var a=false;
		...
		if(__v) console.log("INSERTING NEW SHAPES");
		self.itagger.loadNewShapes(vd);
		// if active shape present, attach metadata dialog
		if(a){
			if(__v) console.log("MAKING A SHAPE ACTIVE");
			self.itagger.raphael.setActiveShape(a);
			if(__v) console.log("ATTACHING METADATA DIALOG");
			var shp={
				id:a.id,
				type:'shapes',
				jsonName:TILEPAGE,
