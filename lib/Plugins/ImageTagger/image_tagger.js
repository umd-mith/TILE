...
(function($){
	...
	_Itag.prototype={
		...
		_restart:function(json){
			...
			if(json.data){
				...
				var shps=[];
				for(var j in json.data.shapes){
					if(json.data.shapes[j]) shps.push(json.data.shapes[j]);
				}
			}
			// send off parsed array to the drawing canvas
			if(self.raphael) self.raphael._restart(shps);
		},
	...
	**/
	var TILEShapeToolBar=Monomyth.Class.extend({
		// Constructor
		init:function(args){
			// this.$super(args);
			this.DOM=$("#"+args.loc);
	...
	**/
	...
	var RaphaelImage=Monomyth.Class.extend({
		init:function(args){
			var self=this;
			//create UID
			var d=new Date();
		...
		setUpCanvas:function(url){
			...
			$("#srcImageForCanvas").bind('load',function(e){
				if(__v) console.log("load event fired for "+$(this).attr('id'));
				if(__v) console.log("img: "+$("#srcImageForCanvas")[0].width+"  "+$("#srcImageForCanvas")[0].height);
				$(this).show();
				$("#srcImageForCanvas").unbind("load");
				if(__v) console.log($(this).attr('src')+"  is loaded in raphael");
				if(!self.drawTool){
				...
				if(__v) console.log("removing loadPage");
		...
		changeShapeColor:function(color,shape){
			...
			for(var sh in self.manifest[url].shapes){
				if(self.manifest[url].shapes[sh].id==shape){
					self.manifest[url].shapes[sh].color=color;
		...
		_shapeActiveHandle:function(e,id){
			if(__v) console.log('shape active called by '+e.target);
			var self=e.data.obj;
			var url=$("#srcImageForCanvas").attr('src');
			$(".shpButtonHolder").remove();
			for(x in self.manifest[url].shapes){
				if(self.manifest[url].shapes[x]&&(self.manifest[url].shapes[x].id==id)){
					...
					if(__v) console.log('foundID; '+foundID);
					...
					if(/^D\_/.test(foundID)){
						...
						// $("#approve_"+foundID).bind("click",function(e){
						// 							var uid=$(this).attr("id").replace("approve_","");
						// 							
						// 							self._approveItemHandle(uid);
						// 							$(this).remove();
						...
						$("body:first").trigger("receiveShapeObj",[{id:foundID,type:'shapes'}]);
						...
						// 								if(!ref) continue;
						// 								// if(__v) console.log("Number of times that "+ref.id+" is on screen: "+$(".shpButtonHolder > div > #delete_"+ref.id).length);
						// 								if($(".shpButtonHolder > div > #delete_"+ref.id).length) continue;
						...
						// 								$("#delete_"+ref.id).bind('mousedown',function(e){
						// 
						// 									// delete shape in manifest
						// 									self._deleteLinkHandle(ref,foundID);
						// 									// delete the link 
		...
		_shapeDrawnHandle:function(e,shpObj){
			...
			if(!self.manifest[url].shapes.push){
				var ag=[];
				for(var t in self.manifest[url].shapes){
					if(self.manifest[url].shapes[t]) ag.push(self.manifest[url].shapes[t]);
				}
				self.manifest[url].shapes=ag;
			}
		...
		_addLinkHandle:function(ref,shape){
			...
			// $("span#delete_"+ref.id).bind("mousedown",function(e){
			// 				// if(__v) console.log("clicked on delete");
			// 				// delete in shape manifest
			// 				self._deleteLinkHandle(ref,shape);
			// 				// for(var i in foundShape[ref.type]){
			// 				// 					if(foundShape[ref.type][i]==ref.id){
			// 				// 						var ac=foundShape[ref.type].split(0,i);
			// 				// 						var bc=foundShape[ref.type].split((i+1));
			// 				// 						foundShape[ref.type]=ac.concat(bc);
			// 				// 					}
			// 				// 				}
			// 				
			// 				// delete the link 
			// 				$("body:first").trigger("deleteShapeLink",[{id:ref.id,type:ref.type,tool:ref.tool,parentObj:shape.id}]);
			// 				$(this).parent().remove();
			// 			});
		},
		...
		_shiftAutoLines:function(id){
			...
			var autoLines=[];
			for(var x in self.manifest[url].shapes){
				
				if(self.manifest[url].shapes[x]&&(self.manifest[url].shapes[x].id==id)){
				
					autoLines.push(self.manifest[url].shapes[x]);
						// store starting point for changing IDs
					n=autoLines.length-1;
					continue;
				} else if(self.manifest[url].shapes[x]){
					if(/^D\_/.test(self.manifest[url].shapes[x].id)){
						autoLines.push(self.manifest[url].shapes[x]);
						continue;
					}
					p.push(self.manifest[url].shapes[x]);
				}
			}
			// new manifest set
			...
			for(var c=n;c<autoLines.length;c++){
				...
				if(c==(autoLines.length-1)){
					// send out deleteLink 
					// $("body:first").trigger("shapeDeleted",[{id:self.manifest[url].shapes[c].id,type:'shapes'}]);
					$("body:first").trigger("shapeDeleted",[autoLines[c].id]);
					// self.manifest[url].shapes[c]=null;
					if(__v) console.log("finishing the line shift; manifest is now: "+JSON.stringify(self.manifest[url].shapes));
					break;
				}
			...
			// 					// send out deleteLink 
			// 					// $("body:first").trigger("shapeDeleted",[{id:self.manifest[url].shapes[c].id,type:'shapes'}]);
			// 					$("body:first").trigger("shapeDeleted",[self.manifest[url].shapes[c].id]);
			// 					self.manifest[url].shapes[c]=null;
			// 					if(__v) console.log("finishing the line shift; manifest is now: "+JSON.stringify(self.manifest[url].shapes));
			// 					break;
			// 				}
			...
			// 					if(id==shape.id){
			// 						if(__v) console.log("erase deleted shape from memory: "+JSON.stringify(self.manifest[url].shapes));
			...
			// 						if(__v) console.log("after deleting extra shape: "+JSON.stringify(self.manifest[url].shapes));
			// 						c+=1;
			// 						continue;
			// 					}
		...
		_restart:function(json){
			...
			for(j in json){
				...
				for(s in self.manifest[url].shapes){
					if(!self.manifest[url].shapes[s]) continue;
				...
				if(!self.manifest[url].shapes.push){
					var ag=[];
					for(var sh in self.manifest[url].shapes){
						ag.push(self.manifest[url].shapes[sh]);
					}
					self.manifest[url].shapes=ag;
				}
			...
			if(__v) console.log(url+" has shapes: "+JSON.stringify(self.manifest[url]));
			if(__v) console.log("setting up canvas in imagetagger _restart:");
...
var IT={
		...
		$("body").bind("deleteShapeLink",{obj:self},function(e,data){
			...
			for(var x in self.linkManifest[url][data.parentObj]){
				if(data.id==self.linkManifest[url][data.parentObj][x].id){
					...
					var ac=self.linkManifest[url][data.parentObj].slice(0,x);
					var bc=self.linkManifest[url][data.parentObj].slice((x+1));
					self.linkManifest[url][data.parentObj]=ac.concat(bc);
					break;
				}
			}
	...
	_receiveShapeObjHandle:function(e,data){
		...
		if(!self.linkManifest[data.id]) {
			self.linkManifest[data.id]=[];
			data.parentTool=null;
		}
		if(!data.attachHandle){
			data.attachHandle="#selBB";
		}
	...
	removeData:function(ref,shape){
		...
		for(var x in self.linkManifest[url][shape]){
			if(self.linkManifest[url][shape][x].id==ref.id){
				var ac=self.linkManifest[url][shape].slice(0,x);
				var bc=self.linkManifest[url][shape].slice((x+1));
				self.linkManifest[url][shape]=ac.concat(bc);
			}
	...
	bundleData:function(j){
		...
		for(o in jcopy){
			...
			if(jcopy[o]){
				...
				jcopy[o].shapes=$.extend(true,{},itagData[o]);
		...
		j=jcopy;
		if(__v){
			console.log("IMAGETAGGER RETURNS:");
