...
// Series of Objects that allow for saving data to the user

// Save - provides functions for force-downloads
// jsonReader - provides functions for reading JSON (Not really used anymore)
...
(function($){
	...
	// Can convert JSON objects or prompt user to download an item
	// 
	// Usage :
	// 
	// new Save({loc: {String}})
	// loc : {String} - Id of parent DOM - should be an element not clearly visible
	...
	var Save=Monomyth.Class.extend({
		...
		// Attaches an iframe to the parent DOM, sets the src to the given location, then 
		// removes the iframe after submitting POST data
		// json : {Object} - Information to save in JSON format 
		userPrompt:function(json){
			...
			console.log(JSON.stringify(json["pages"][3]["lines"][0]));
			$("#uploadData").val(jsonStr);
		...
		// Once document receives a click event, removes the uploadForm if present
		// e : {Event}
