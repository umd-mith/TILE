			...
			for(top in rules.topLevelTags){
				...
				for(t in rules.tags){
					if(rules.tags[t].uid==uid){
						...
						var el=$("<option id=\""+uid+"\">"+tag.name+"</option>").appendTo(this._tagChoices);
						break;
					}
				}
			}
	...
	**/
	var TileLogBar=Log.extend({
		init:function(args){
			...
			//global listeners
			$("body").bind("shapeCommit",{obj:this},this.addLogItem);
		},
		addLogItem:function(e,shape){
			var obj=e.data.obj;
		
			var l=new TileLogItem({
				loc:obj.list,
	...
	var TileLogItem=LogItem.extend({
		init:function(args){
			this.$super(args);
			var self=this;
