...
var LB={
	...
	inputData:function(data){
		...
		if(__v) console.log("number of labeled items on document: "+$("#delete_"+data.id).length); 
		if($("#delete_"+data.id).length) return;
		if(($("#selBB").length)&&($("#delete_"+data.id).length==0)){
			if(__v) console.log("attaching span tags to selBB: "+$("#selBB > .shpButtonHolder")+"  id: "+data.id); 
