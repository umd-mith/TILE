...
Label.prototype={
	...
	lblSelected:function(data){
		...
		for(x in self.manifest[url]){
			if(self.manifest[url][x].id==self.curId){
				if(!self.manifest[url][x].refs) self.manifest[url][x].refs=[];
				if(console) console.log("selected label: "+self.manifest[url][x].id+"  and refs: "+self.manifest[url][x].refs);
				$("body:first").trigger("loadItems",[self.manifest[url][x].refs]);
			}
		}
