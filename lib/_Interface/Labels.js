...
Label.prototype={
	...
	loadLabels:function(data){
		...
		for(d in data){
			...
			$("<div id=\"lbl_"+lbl.id+"\" class=\"labelItem\">"+lbl.name+"</div>").bind("click",function(e){
	...
	_addLinkHandle:function(ref,label){
		...
		if(__v) console.log("addlinkhandle in labels.js: "+label);
		if(($.inArray(label.id,self.checkIds)<0)){ 
			
			self.manifest[self.manifest.length-1]={id:label.id,name:(label.name)?label.name:"Area of interest",refs:[]};
			self.checkIds.push(label.id);
			$("<div id=\"lbl_"+label.id+"\" class=\"labelItem\">"+label.name+"</div>").click(function(){
				if($(this).hasClass("active")) {
					$(this).removeClass("active");
					return;
				}
				...
				var id=$(this).attr('id').replace("lbl_","");
				// $("body:first").trigger("labelSelected",[{type:"labels",id:id}]);
				self.lblSelected({name:$(this).text(),id:label.id});
			}).appendTo(self.labelList);
			// $("#lbl_"+id).addClass("active");
...
var LB={
	...
	getLink:function(){
		...
		// return link value
		return {"id":lbl.attr('id'),type:"labels",tool:self.id};
	...
	bundleData:function(json){
		...
		if(__v) console.log("LABELS OUTPUT DATA: "+JSON.stringify(data));
		var temp=$.extend(true,{},data);
		json.labels=temp;
		if(__v) console.log("LABELS REPLACED json.labels: "+JSON.stringify(json.labels));
