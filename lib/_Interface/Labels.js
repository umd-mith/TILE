...
Label.prototype={
	...
	_deleteLinkHandle:function(ref){
		...
		for(x in self.manifest){
			if(self.manifest[x].id==id){
				if(console) console.log("manifest has: "+self.manifest[x].id+"  "+self.manifest[x].refs+" need to find: "+ref.id);
				for(r in self.manifest[x].refs){
					if(self.manifest[x].refs[r]==ref.id){
						...
						if(self.manifest[x].refs.length>1){
							...
							self.manifest[x].refs=[];
							if(console) console.log("deleted ref "+self.manifest[x].refs);
						}
					}
				}
				break;
...
var LB={
	...
	inputData:function(data){
		...
		if(console) console.log("number of labeled items on document: "+$("#delete_"+data.id).length); 
		if($("#delete_"+data.id).length) return;
		if(($("#selBB").length)&&($("#delete_"+data.id).length==0)){
