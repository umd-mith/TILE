	...
	**/
	...
	var ImportDialog=Dialog.extend({
		...
		init:function(args){
			...
			console.log("importDialog setting up it's stuff: "+$("#importDataFormInputMulti"));
		...
		handleMultiForm:function(e){
			...
			console.log('handleMultiForm is: '+obj+"  and obj has: "+obj.closeB+"  "+obj.multiFileInput);
	...
	var TILE_ENGINE=Monomyth.Class.extend({
		...
		getSchema:function(e,file){
			...
			// if(/<([A-Za-z_0-9]*)|[\(\)]*/.test(obj.imageFile)){
			// 				// security problem - aborting
			// 				window.location="http://www.google.com";
			// 				return;
			// 		 	}
			
			// passed test - ok to go
			if(/json$|JSON$|txt$|TXT$|JS$|js$/.test(obj.imageFile)){
				// js file - attach to header
				
				// var el=$("<script type=\"text/javascript\" src=\""+obj.imageFile+"\"></script>").appendTo($("head"));
		...
		_parseOutJSON:function(){
			...
			//organize manifest by url
			for(key in self.json.pages){
				console.log('parsing JSON in tile1.0: '+key+": "+self.json.pages[key].url);
				if(!self.manifest[self.json.pages[key].url]){
					self.manifest[self.json.pages[key].url]=$.extend(true,{},self.json.pages[key]);
				} else {
					//duplicate url - 
			...
			if(this.toolSet){
				...
				function recTool(t){
					if(t>=self.toolSet.length) {
						...
						console.log("restarting imagetagger with: "+m);
						self.toolSet[0].restart(m);
						// $("body:first").trigger("loadImageList",[self.manifest]);
						return;
					}
