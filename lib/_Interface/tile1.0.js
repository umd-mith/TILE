	...
	**/
	...
	var ExportDialog=Dialog.extend({
		...
		_expStrDoneHandle:function(e,str){
			...
			if(__v)  console.log(self.srcXML.val());
	...
	var TILE_ENGINE=Monomyth.Class.extend({
				...
				for(tool in obj.toolSet){
					if(obj.toolSet[tool].name==toolname){
						...
						if(!obj.toolSet[tool].constructed){
							...
							obj.toolSet[tool].start(null,self._base,(obj.manifest)?obj.manifest:null,(args)?args:null); //give place where it opens into
		...
		_toolDoneHandle:function(e,args){
			...
			if(args){
				if(args.data){
					// replace current manifest lines with this data
					// but first, correct the height offsets of shapes
					// var yval=["y","width","cy","ry"];
					// 					var ycorrect=$("#azcontentarea > .az.inner > .toolbar").innerHeight();
					// 					for(d in args.data){
					// 						if(args.data[d].shapes){
					// 							for(x in args.data[d].shapes){
					// 								$.each(args.data[d].shapes[x].posInfo,function(i,o){
					// 									if($.inArray(i,yval)>-1){
					// 										args.data[d].shapes[x].posInfo[i]-=ycorrect;
					// 									}
					// 								});
					// 							}
					// 						}
		...
		_saveSettingsHandle:function(e){
			...
			var exfest=self._log.bundleData(self.manifest);
			//merge manifest data with json data
			for(p in self.json.pages){
			
				for(x=0;x<self.json.pages[p].lines.length;x++){
					if(exfest[self.json.pages[p].url].lines[x]){
						
						self.json.pages[p].lines[x]=exfest[self.json.pages[p].url].lines[x];
					}
					
				}
				var aa=exfest[self.json.pages[p].url].lines.slice((self.json.pages[p].lines.length));
				exfest[self.json.pages[p].url].lines=aa;
				
				
				// var url=self.json.pages[p].url;
				// 				if(exfest[url]){
				// 					// combine lines that are in this page
				// 					for(y in self.exfest[url].lines){
				// 						var mline=exfest[url].lines[y].text;
				// 						for(aa in self.json.pages[p].lines){
				// 							if(self.json.pages[p].lines[aa].text==mline){
				// 								self.json.pages[p].lines[aa]=exfest[url].lines[y];
				// 								break;
				// 							}
				// 						}
				// 					}
