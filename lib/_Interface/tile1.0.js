	...
	PluginController.prototype={
		...
		initTools:function(json){
			...
			if(self.toolSet){
				...
				function recTool(t){
					...
					if(__v) console.log(ct.name+" toolIds: "+toolIds[t]);
		...
		_toolOutputHandle:function(e,data){
			...
			if(self.activeTool.inputData(data)){
				...
				if(__v) console.log("store data of link into manifest: "+self.activeTool.id+"  "+data.id+'  '+data.type);
				if(!self.manifest[self.activeTool.id]) self.manifest[self.activeTool.id]=[];
				self.manifest[self.activeTool.id].push(data.id);
