	...
	**/
	...
	var ExportDialog=Dialog.extend({
		...
		_expStrDoneHandle:function(e,str){
			...
			$("#exportDataForm").attr("action","http://localhost:8888/TILE/postJuly/importWidgets/exportXML.php");
			console.log($("#exportDataForm").attr("action"));
			self.expData.val(str);
			iframe.appendTo($("#azglobalmenu"));
			// console.log("EXPDATA VAL: \n\n"+self.expData.val());
			//
				...
				for(tool in obj.toolSet){
					if(obj.toolSet[tool].name==toolname){
						...
						if(!obj.toolSet[tool].constructed){
							...
							// if(obj.toolSet[tool]._outputData) $("body").bind(obj.toolSet[tool]._ouputData,)
						} else {
							//tool already constructed, restart tool
							obj.curTool=obj.toolSet[tool];
						
							obj.curTool.restart((obj.json)?obj.manifest[$("#srcImageForCanvas").attr("src")]:null,(args)?args:null);
						}
						break;
					}
				}
