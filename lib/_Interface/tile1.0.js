		...
		sendLabels:function(){
			...
			for(var x in lbls){
				...
				if(!self._labels[lbls[x]]){
					var id=Math.random()*100+lbls[x];
					
					self._labels[lbls[x]]={id:id,name:lbls[x],tool:"LB1000"};
	...
	**/
	...
	var TILE_ENGINE=Monomyth.Class.extend({
		init:function(args){
			...
			$("body").bind("toolSetUpDone",{obj:self},function(e){
				...
				$.ajax({
					...
					success:function(x){
						self.dialogJSON=x;
						if(__v) console.log("calling dialog boxes");
						self.addDialogBoxes();
						if(__v) console.log("setting up the system");
						self.setUp();
					}
				});
			});
		...
		_saveSettingsHandle:function(e){
			...
			for(var p in self.json){
				if(p=='pages'){
					for(var page in self.json[p]){
						for(var m in self.manifest){
							if(m==self.json[p][page].url){
								self.json[p][page]=$.extend(true,{},self.manifest[m]);
							}
						}
					}
					...
					for(var m in self.manifest){
						if(m==p){
							var exfest=$.extend(true,{},self.manifest[m]);
							self.json[p]=exfest;
						}
					}
				}
			...
			// 				if(!self.json.pages[p]||(self.json.pages[p]=="")) continue;
			// 				
			...
			// 					$.extend(true,exfest,self.manifest[self.json.pages[p].url]);
			// 				}
			// 				self.json.pages[p]=self.manifest[self.json.pages[p].url];
			// 				
			// 				
			// 				// var shps=[];
			// 				
	...
	PluginController.prototype={
		...
		setUpTool:function(toolname,json){
			...
			if(!self.curTool){
				...
				// console.log("no previous tool selected: "+self.curTool);
			...
			$("body").bind(self.curTool._close,{obj:self},function(data){
				...
				self.toolSet[self.startTool].restart(m,data);
			});
			var m=(json)?json[$("#srcImageForCanvas").attr("src")]:null;
		...
		_toolOutputHandle:function(e,data){
			...
			if(((!self.activeTool)||(self.activeTool.id==data.tool))&&(!data.parentTool)){
				...
				if(__v) console.log("attachHandle in tooloutput: "+data.attachHandle);
			...
			if(self.activeTool.inputData){
				...
				self.activeTool.inputData({ref:data,obj:link});
				if(__v) console.log("going to put "+link+" into tool "+data.tool);
			...
			// 				// go to defaultTool
			// 				self.activeTool=self.defaultTool;
			// 				self.activeTool.inputData(data);
			// 				
			// 				// store data of link into manifest
			// 				
			// 				if(!self.manifest[self.activeTool.id]) self.manifest[self.activeTool.id]=[];
			// 				if($.inArray(data.id,self.manifest[self.activeTool.id])) self.manifest[self.activeTool.id].push(data.id); 
			// 				
			// 				if(!self.manifest[data.tool]) self.manifest[data.tool]=[];
			// 				
			// 				self.manifest[data.tool].push(data.id);
			// 				var link=self.activeTool.getLink();
			// 				if(link&&(self.toolSet[data.tool].inputData)) self.toolSet[data.tool].inputData(link);
			// 				return true;
			// 			}
			return false;
		},
		...
		_deleteLinkHandle:function(e,ref){
			...
			// TODO: if no other link with ref.id and ref.type, then erase original object
			// FOR NOW: if an object doesn't link to anything, then it will not exist after another tool becomes activated
			// var refs=[];
			// 		for(s in self.manifest){
			// 			if((s!=self.activeTool.id)&&(s!=ref.tool)){
			// 				if($.inArray(ref.id,self.manifest[s])>=0){
			// 					refs.push(s);
			// 					break;
			// 				}
			// 			}
			// 		}
			// 		if(refs.length==0) {
			// 			if(self.toolSet[ref.parentTool].deleteSelf) self.toolSet[ref.parentTool].deleteSelf(ref.parentObj);
			// 			// if(ref.tool&&ref.objID&&ref.objType) self.toolSet[ref.tool].removeData({id:ref.objID,type:ref.objType});
			// 		}
			
			// if(!self.activeTool){
			// 				// delete all references
			// 				for(s in self.manifest){
			// 					var x=$.inArray(ref.id,self.manifest[s]);
			// 					if(x>=0){
			// 						// delete in tool
			// 						
			// 						if((self.toolSet[s].removeData)) self.toolSet[s].removeData(ref);
			// 						if(self.manifest[s].length>1){
			// 							// delete in manifest
			// 							var ac=self.manifest[s].slice(0,x);
			// 							var bc=self.manifest[s].slice((x+1));
			// 							self.manifest[s]=ac.concat(bc);
			// 						} else {
			// 							self.manifest[s]=[];
			// 						}
			// 					}
			// 				}
			// 			} else {
			// 				// activeTool present - make sure to check all tools
			// 				// for reference
			// 				var refs=[];
			// 				for(s in self.manifest){
			// 					if((s!=self.activeTool.id)&&(s!=ref.tool)){
			...
			// 					if(ref.tool&&ref.objID&&ref.objType) self.toolSet[ref.tool].removeData({id:ref.objID,type:ref.objType});
			// 				}
			// 				self.activeTool.removeData({id:ref.objID,type:ref.objType});
			// 			}
		},
