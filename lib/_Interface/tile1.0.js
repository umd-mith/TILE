	...
	PluginController.prototype={
		setUpToolData:function(){
			...
			function recLoad(i){
				...
				if(self.toolSet[i].output){
					$("body").bind(self.toolSet[i].output,{obj:self},self._toolOutputHandle);
				}
		...
		_setActiveTool:function(e,id){
			var self=e.data.obj;
			if(self.activeTool){
				// don't do anything if this is the same tool
				if(self.activeTool.id==id) return;
				
				
				self.activeTool.unActive();
			}
			...
			if(__v) console.log("setActiveTool activates: "+self.activeTool.id);
		...
		_toolOutputHandle:function(e,data){
			if(__v) console.log("tooloutputhandle in tile1.0 reached "+e.data.obj.activeTool);
			var self=e.data.obj;
			if(!self.activeTool||(!(self.activeTool.inputData))){ 
				self.activeTool=self.defaultTool;
				if(__v) console.log("tool: "+self.defaultTool.id);
			};
