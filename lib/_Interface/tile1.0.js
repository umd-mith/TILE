	...
	**/
	...
	$.extend(FloatingDiv.prototype, {
		...
		init: function(myID, labels) {
			...
			'<li class="label_formLI cloneMe">'+
			'<span class="label_formLabel">Data already attached: </span><br/><div id="attachDataList" class="form_attachedList"></div>'+
			'</ol></fieldset>';
		...
		createDialog:function(myID) {
			...
			this._attachDataList=$("#attachDataList");
		...
		sendLabels:function(){
			...
			for(var x in lbls){
				...
				html+="<div id=\""+self._labels[lbls[x]].id+"\" class=\"button shape\">"+lbls[x]+"<span id=\"del_"+id+"\" class=\"button shape delete formLink\">X</span></div>";
