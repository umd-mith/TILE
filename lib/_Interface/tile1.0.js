	...
	$.extend(FloatingDiv.prototype, {
		...
		deleteLinkHandle:function(id){
			...
			if(n==0){
				...
				var ac=lb.refs.slice(0,n);
				var bc=lb.refs.slice((n+1));
				lb.refs=ac.concat(bc);
			}
	...
	**/
	...
	PluginController.prototype={
		...
		_toolOutputHandle:function(e,data){
			...
			if(data.inputTool){
				if(__v) console.log(data.inputTool);
				if(self.manifest[data.inputTool]&&self.manifest[data.inputTool].inputData){
					if(__v) console.log("data passed inputTool "+data.inputTool);
					self.manifest[data.inputTool].inputData(data.payload);
				}
