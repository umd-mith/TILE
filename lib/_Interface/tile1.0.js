	...
	**/
	...
	PluginController.prototype={
			...
			if(self.toolSet){
				...
				function recTool(t){
					if(t>=toolIds.length) {
						...
						$("body").trigger("floatDivAppear",{obj:self},self._attachFloatDiv);
		...
		_toolOutputHandle:function(e,data){
			if(__v) console.log("toolOutputHandle called by: "+e.type);
			var self=e.data.obj;
			
			if(!self.activeTool||(!(self.activeTool.inputData))){
				self.activeTool=self.defaultTool;
			};
			if(!self.activeTool.inputData) return;
		
			if(self.activeTool.inputData(data)){
				...
				if(__v) console.log("adding data to "+self.activeTool.id);
				...
				if(data.attachHandle){
					self._attachFloatDiv(data.attachHandle);
				}
				return true;
			} else {
				// go to defaultTool
		...
		_attachFloatDiv:function(id){
			var self=this;
			// id represents element to attach to
			if(id==null) return;
			var pos=$(id).offset();
			if(__v) console.log("attaching "+$("#tilefloat_floatingDiv").parent()+"  at: "+pos.left+", "+pos.top);
			pos.left=pos.left+$("#azcontentarea:first").position().left;
			$("#tilefloat_floatingDiv").parent().css("left",pos.left+'px');
