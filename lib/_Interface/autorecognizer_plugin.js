		...
		_loadTranscript:function(){
			...
			if(self.transcript){
				for(t in self.transcript){
					...
					out+="<div class='trLine selLine' id='trLine"+t+"'>"+self.transcript[t].text+"</div>";
				}
			}else {
				for(u in URL_LIST){
		...
		_recognize:function(){
			...
			//	self.regionList[self.curRegion].images.push($("#srcImageForCanvas").attr('src'));
				...
				for (var i = 0; i < bucket.length; i++) {
					...
					self.regionList.shapes.push({"id":id,"type":"rect","scale":self.CANVAS._scale,"posInfo":{"x":left,"y":top,"width":_REG.width,"height":height}});
					//update assoc. transcript tag
					if(self.transcript[i]){
						self.regionList.lines.push(self.transcript[i]);
						self.regionList.links.push({"shape":id,"line":self.transcript[i].uid});
					}
		...
		_outputData:function(){
			...
			$(".az.main > .az.tool").animate({opacity:0.25,left:n},200,function(e){
				...
				//	var items=$.merge(self.lineManifest,self.regionList);
					...
					$("body").trigger("addLogBarItem",[self.regionList]);
...
var AR={
	start:function(id,schema,data){
		...
		if(!id) id="az_log";
		this.__AR__=new TileOCR({loc:id,transcript:trans});
	},
	name:"autorecognizer",
