	...
	**/
	var TileOCR=Monomyth.Class.extend({
		...
		_setUp:function(e,jsonhtml){
			...
			//self.DOM.bind("transcriptLineSelected",{obj:self},self._lineClickHandle);
			//self.DOM.bind("transcriptLineDeSelected",{obj:self},self._lineClickHandle);
		...
		_closeOutAR:function(){
			...
			var shapes=self.exportLines();
			var items=[];
			for(g in self.regionList){
				if(self.regionList[g]){
		...
		_lineClickHandle:function(e,index){
			var self=e.data.obj;
		...
		//export line data as JSON object
		exportLines:function(){
			var self=this;
			if(!self.regionList) return;
			var lineJSON=[];
			for(reg in self.regionList){
				if((!self.regionList[reg])||(!self.regionList[reg].etc)) continue;
				for(tag in self.regionList[reg].etc.tags){
					if(self.regionList[reg].etc.tags[tag].type=='transcript') continue;
					var shape=self.regionList[reg].etc.tags[tag];
					if(shape.region&&shape.shape){
						var l=(shape.region.DOM.offset().left-self.CANVAS.canvas.offset().left);
						var t=((shape.region.DOM.offset().top-self.CANVAS.canvas.offset().top));
						var w=shape.region.DOM.width();
						var h=shape.region.DOM.height();
						shape.shape.posInfo.x=l;
						shape.shape.posInfo.y=t;
						shape.shape.posInfo.width=w;
						shape.shape.posInfo.height=h;
						var sdata=[];
						
						lineJSON.push(shape.shape);
					}
				}
			}
			return lineJSON;
		...
		_loadTranscript:function(){
			...
			if(self.transcript){
				...
				for(t in self.transcript){
					...
					var el=$("<div class='trLine selLine' id='trLine"+t+"'>"+self.transcript[t].text+"</div>");
					self.transcriptArea.append(el);
					el.click(function(e){
						if($(this).hasClass("selected")){
							$(this).removeClass("selected");
							//$(this).trigger("transcriptLineDeSelected",[parseInt($(this).attr('id'),10)]);
						} else {
							$(this).addClass("selected");
							//$(this).trigger("transcriptLineSelected",[parseInt($(this).attr('id'),10)]);
						}
					});
				}
			} // else {
		...
		_recognize:function(){
			...
			//	self.regionList[self.curRegion].images.push($("#srcImageForCanvas").attr('src'));
				...
				for (var i = 0; i < bucket.length; i++) {
					...
					if(self.transcript[self.regionList.lines[i]]){
						//add data to the session's JSON object
						self.transcript[self.regionList.lines[i]].shapes.push({"id":id,"type":"rect","scale":self.CANVAS._scale,"posInfo":{"x":left,"y":top,"width":_REG.width,"height":height}});
					}
					...
					// 					"uri":$("#srcImageForCanvas").attr("src"),
					// 					"type":"shape",
					// 					"shape":{"id":id,"type":"rect","scale":self.CANVAS._scale,"posInfo":{"x":left,"y":top,"width":_REG.width,"height":height}},
