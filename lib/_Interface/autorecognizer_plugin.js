	...
	**/
	var TileOCR=Monomyth.Class.extend({
		...
		_setUp:function(e,jsonhtml){
			...
			self.nextB=$("#"+self.DOM.attr('id')+" > #content > div.step > div.buttondiv");
			self.prevB=$("#"+self.DOM.attr('id')+" > #content > div.buttons > div > #prevButton");
		...
		_closeOutAR:function(){
			...
			$("body").trigger("VD_ADDSHAPE",[self.lineManifest[$("#srcImageForCanvas").attr("src")]]);
			$(".lineBox").remove();
		...
		**/
		...
		_recognize:function(){
			...
			$.merge(self.lineManifest[$("#srcImageForCanvas").attr('src')],ldata);
