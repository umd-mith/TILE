	...
	var TileOCR=Monomyth.Class.extend({
		...
		**/
		_capture:function(){
			...
			var curLineId = "cl"+(regionList.length-1); 
		    var curList = $("#regionListBox").html() + "<div id='"+curLineId+"'><div class='deleteLine'>x</div>" +  self.curRegion + "," + dims.left + "," + dims.top + "," + dims.width + "," +
		    dims.height +
		    "," +
		    rgb +
		    "</div>";
		...
		var RegionBox=Monomyth.Class.extend({
			...
			_zoomHandle:function(e,v){
				...
				if(v>0){
					//zooming in
					var d=self._getDims();
					self.DOM.css({"left":(d.left*2),"top":(d.top*2),"width":(d.width*2),"height":(d.height*2)});
				} else if(v<0){
					var d=self._getDims();
					self.DOM.css({"left":(d.left/2),"top":(d.top/2),"width":(d.width/2),"height":(d.height/2)});
				}
