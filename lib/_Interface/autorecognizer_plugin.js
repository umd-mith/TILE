			...
			//	self.regionList[self.curRegion].images.push($("#srcImageForCanvas").attr('src'));
				...
				for (var i = 0; i < bucket.length; i++) {
					...
					if(self.transcript.lines[i]&&self.transcript.lines[i].shapes){
						...
						self.transcript.lines[i].shapes.push({"button1":"<span id=\"delete"+id+"\" class=\"deleteKey button\">DELETE</span>","[Automatically Recognized Line]":i,"order":"l"+i,"id":id,"type":"rect","_scale":1,"color":"#33FF00","posInfo":{"x":(left*scalecorrectx),"y":(top-tbarcorrect)*scalecorrecty,"width":(_REG.width*scalecorrectx),"height":(height)*scalecorrecty}});
					}
					
		        }
				// adjust lines
				$(".lineBox").each(function(e){
					$(this).width($(this).width()*scalecorrectx);
					$(this).height($(this).height()*scalecorrecty);
					var left=parseInt($(this).css("left"),10)*scalecorrectx;
					var top=(parseInt($(this).css("top"),10)-tbarcorrect)*scalecorrecty;
	 ...
	 */
	 var CanvasImage=TILEImage.extend({
		...
		setUpCanvas: function(url){
			var self = this;
			// if(__v)  console.log("autorec canvas image is loading canvas: "+$("#srcImageForCanvas")[0].src+" "+url);
			$("#srcImageForCanvas").load(function(e){
				...
				// if(__v)  console.log("autorec loading the canvas: "+$("#srcImageForCanvas")[0].width+" "+$("#srcImageForCanvas")[0].src);
		...
		var RegionBox=Monomyth.Class.extend({
			...
			_zoomHandle:function(e,v){
				...
				if(v>0){
					...
					self.DOM.css({"left":(d.left*1.25)+'px',"top":(top*1.25)+'px',"width":(d.width*1.25),"height":(d.height*1.25)});
				} else if(v<0){
					var d=self._getDims();
					var top=(d.top*0.75);
					if(top<(self.DOM.parent().position().top+$("#azcontentarea > .az.inner > .toolbar").innerHeight())){
