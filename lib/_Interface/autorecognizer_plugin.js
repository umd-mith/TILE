		...
		_recognize:function(){
			...
			//	self.regionList[self.curRegion].images.push($("#srcImageForCanvas").attr('src'));
				...
				for (var i = 0; i < bucket.length; i++) {
					...
					// if(i==0) {top-=(tbarcorrect+(25));
					// 						} else {
					// 							top=lastTop+height;
					// 						}
						
						//add data to the session's JSON object
						self.transcript.lines[i].shapes.push({"id":"D_"+id,"type":"rect","_scale":1,"color":"#FFFFFF","posInfo":{"x":(left*scalecorrectx),"y":((top*scalecorrecty)),"width":(_REG.width*scalecorrectx),"height":(height)*scalecorrecty}});
					}
		...
		_outputData:function(){
			...
			$(".az.main > .az.tool").animate({opacity:0.25,left:n},200,function(e){
				...
				$(this).animate({opacity:1,left:0},200,function(e){
					...
					}).attr("src",url.substring(url.indexOf('http')));
	 ...
	 */
	 var CanvasImage=TILEImage.extend({
		...
		setUpCanvas: function(url){
			...
			if(/localhost/.test(url)==false){
				url="PHP/RemoteImgRedirect.php?uimg="+url;
			}
			$("#srcImageForCanvas")[0].src=url;
		},
		_remoteImageHandle:function(src){
			var self=this;
			var url="PHP/RemoteImageRedirect.php?uimg="+src;
			$.ajax({
				url:url,
				success:function(){
