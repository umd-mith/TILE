		...
		**/
		...
		_recognize:function(){
			...
			//	self.regionList[self.curRegion].images.push($("#srcImageForCanvas").attr('src'));
				numOfLines = self.transcript.length;
				...
				for (var i = 0; i < bucket.length; i++) {
					...
					self.transcript[i].shapes.push({"id":id,"type":"rect","scale":self.CANVAS._scale,"posInfo":{"x":left,"y":top,"width":_REG.width,"height":height}});
					//update assoc. transcript tag
					if(self.transcript[i]){
						//self.transcript[i].lines.push(self.transcript[i]);
						//self.transcript.links.push({"shape":id,"line":self.transcript[i].uid});
					}
