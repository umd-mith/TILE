	...
	**/
	var TileOCR=Monomyth.Class.extend({
		...
		_closeOutAR:function(){
			...
			$(".az.main > .az.tool").animate({opacity:0.25,left:n},400,function(e){
				...
				//	var items=$.merge(self.lineManifest,self.regionList);
					...
					for(g in self.regionList){
						//go through and update all the items
						for(area in self.regionList[g].etc){
							var o=self.regionList[g].etc[area];
							var l=(bb.DOM.position().left/imgdims.w);
							var t=((bb.DOM.offset().top-self.CANVAS.DOM.parent().offset().top)/imgdims.h);
							var w=bb.DOM.width()/imgdims.w;
							var h=bb.DOM.height()/imgdims.h;
							$.extend(o,{
			...
			// 				self.DOM.trigger("addLogBarItem",[{"items":self.lineManifest,"group":"AutoRec"}]);
			// 			},1,self);
			...
			if(!($("#srcImageForCanvas").attr('src') in self.regionList[self.curRegion].images)) {
				...
				//	if(i==0) alert(imgdims.w+"  	"+(bb.DOM.position().left/imgdims.w)+" "+bb.DOM.position().top); 
					...
					ldata.push({
						...
						"group":self.regionList[self.curRegion].uid,
						"transcript":$("#trLine"+i).text(),
						"region":bb
					});
...
var AR={
	start:function(id){
		this.__AR__=new TileOCR({loc:id});
	},
	name:"autorecognizer",
