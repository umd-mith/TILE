		...
		**/
		...
		_recognize:function(){
			...
			//	self.regionList[self.curRegion].images.push($("#srcImageForCanvas").attr('src'));
				...
				console.log("scalecorrectx: "+scalecorrectx+"  scalecorrecty: "+scalecorrecty+"  tbarcorrect: "+tbarcorrect);
				...
				for (var i = 0; i < bucket.length; i++) {
					...
					if(self.transcript.lines[i]&&self.transcript.lines[i].shapes){
						...
						self.transcript.lines[i].shapes.push({"id":"D_"+id,"type":"rect","_scale":1,"color":"#FFFFFF","posInfo":{"x":(left*scalecorrectx),"y":(((top)*scalecorrecty)-tbarcorrect),"width":(_REG.width*scalecorrectx),"height":(height)*scalecorrecty}});
					}
