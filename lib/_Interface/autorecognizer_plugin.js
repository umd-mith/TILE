		...
		**/
		...
		_recognize:function(){
			...
			//	self.regionList[self.curRegion].images.push($("#srcImageForCanvas").attr('src'));
				...
				var left = _REG.left;
		        var rtop = _REG.top;   
		 		var tbarcorrect=$("#azcontentarea > .az.inner > .toolbox").innerHeight();
				...
				for (var i = 0; i < bucket.length; i++) {
					...
					if(self.transcript.lines[i]&&self.transcript.lines[i].shapes){
						...
						self.transcript.lines[i].shapes.push({"id":id,"type":"rect","_scale":1,"posInfo":{"x":(left*scalecorrectx),"y":((top*scalecorrecty)-tbarcorrect),"width":(_REG.width*scalecorrectx),"height":(height*scalecorrecty)-tbarcorrect}});
					}
