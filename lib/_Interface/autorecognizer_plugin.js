		...
		**/
		...
		_recognize:function(){
			...
			//	self.regionList[self.curRegion].images.push($("#srcImageForCanvas").attr('src'));
				...
				if(__v)  console.log("_REG values: "+left+"  "+rtop+"  "+_REG.width);
				if(__v)  console.log("_REG CORRECT: "+scalecorrectx+"  "+scalecorrecty+"  l: "+(left*scalecorrectx)+" rtop: "+(rtop*scalecorrecty));
				if(__v)  console.log("tbarcorrect: "+tbarcorrect);
	 ...
	 var CanvasImage=TILEImage.extend({
		...
		init: function(args){
			...
			$(window).resize(function(e){
				if(__v)  console.log("resize autorec: "+self.uid);
				if($("#"+self.uid).length){
			...
			var CanvasAutoRecognizer=AutoRecognizer.extend({
				...
				thresholdConversion:function(threshold){
					...
					if(this.Region){
						...
						if(__v)  console.log("conversion threshold: "+threshold);
						...
						if(__v)  console.log("converting: "+this.context);
