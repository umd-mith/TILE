			...
			//	self.regionList[self.curRegion].images.push($("#srcImageForCanvas").attr('src'));
				...
				for (var i = 0; i < bucket.length; i++) {
					...
					if(self.transcript.lines[i]&&self.transcript.lines[i].shapes){
						...
						self.transcript.lines[i].shapes.push({"[Automatically Recognized Line]":i,"order":i,"id":id,"type":"rect","_scale":1,"color":"#33FF00","posInfo":{"x":(left*scalecorrectx),"y":(top-tbarcorrect)*scalecorrecty,"width":(_REG.width*scalecorrectx),"height":(height)*scalecorrecty}});
					}
