	...
	**/
	var TileOCR=Monomyth.Class.extend({
		...
		_setUp:function(e,jsonhtml){
			...
			self.nextB=$("#"+self.DOM.attr('id')+" > #content > div.step > div.buttondiv").click(function(e){
				self._next();
			});
		...
		_closeOutAR:function(){
			...
			$("body").trigger("VD_ADDSHAPE",[self.lineManifest,{"group":"AutoRec"}]);
			$(".lineBox").remove();
		...
		**/
		_capture:function(){
			...
			var curLineId = "cl"+(regionList.length-1); 
		    var curList = self.regionListBoxDiv.html()+"<div id='"+curLineId+"'><div class='deleteLine button'>x</div>" +  
			            self.curRegion+","+dims.left+","+dims.top+","+dims.width+","+dims.height+",<br/>"+rgb+"</div>";
