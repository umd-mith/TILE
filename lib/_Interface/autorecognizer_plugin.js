	...
	var TileOCR=Monomyth.Class.extend({
		...
		**/
		...
		_recognize:function(){
			...
			//	self.regionList[self.curRegion].images.push($("#srcImageForCanvas").attr('src'));
				...
				var left = _REG.left;
		 		...
		 		var tbarcorrect=($("#azcontentarea > .az.inner > .toolbar").innerHeight()+$("#azcontentarea > .az.inner").scrollTop()+$("#azcontentarea > .az.inner > .workspace").offset().top);
				// tbarcorrect+=(_REG.top-tbarcorrect);
		    
				var rtop = _REG.top;
				var alphaTop=_REG.top-(tbarcorrect+15);
				...
				for (var i = 0; i < bucket.length; i++) {
					...
					if(self.transcript.lines[i]&&self.transcript.lines[i].shapes){
						...
						self.transcript.lines[i].shapes.push("D_"+id);
						self.transcript.shapes.push({"id":"D_"+id,"type":"rect","_scale":1,"color":"#FFFFFF","posInfo":{"x":(left*scalecorrectx),"y":((top*scalecorrecty)),"width":(_REG.width*scalecorrectx),"height":(height)*scalecorrecty}});
						// self.shapesFound.push({"id":"D_"+id,"type":"rect","_scale":1,"color":"#FFFFFF","posInfo":{"x":(left*scalecorrectx),"y":((top*scalecorrecty)),"width":(_REG.width*scalecorrectx),"height":(height)*scalecorrecty}});
					}
		...
		_outputData:function(){
			...
			if($(".lineBox").length) $("body:first").trigger("outputAutoRecData",[{data:self.transcript}]);
			...
			$("#srcImageForCanvas").attr("src",url.substring(url.indexOf('http')));
			console.log('closed and finished');
			...
			$(".az.main > .az.tool").animate({opacity:0.25,left:n},200,function(e){
				...
				$(this).animate({opacity:1,left:0},200,function(e){
					...
					self.canvasArea.animate({opacity:0},200,function(){
						...
						$("#srcImageForCanvas").attr("src",url.substring(url.indexOf('http')));
						$("body").trigger("closeOutAutoRec",[{scale:self.CANVAS._scale,data:self.transcript}]);
						console.log('closed and finished animations');
					});
	 ...
	 var CanvasImage=TILEImage.extend({
		...
		setUpCanvas: function(url){
			...
			if((/file::/.test(url)==false)&&(/PHP\//.test(url)==false)){
				url="PHP/RemoteImgRedirect.php?uimg="+url;
			}
