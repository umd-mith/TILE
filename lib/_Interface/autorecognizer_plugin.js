	...
	var TileOCR=Monomyth.Class.extend({
		...
		_paginate:function(dir){
			...
			if(self.regionList[self.curRegion]){
				for(l in self.regionList[self.curRegion].etc){
					var t=self.regionList[self.curRegion].etc[l];
		...
		canvasReadyHandle:function(e){
			...
			self._loadTranscript(self.transcript);
			
		},
		_loadTranscript:function(tscript){
			var self=this;
			self.transcript=tscript;
			var out="";
			if(self.transcript){
