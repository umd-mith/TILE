	...
	**/
	var TileOCR=Monomyth.Class.extend({
		...
		_setUp:function(e,jsonhtml){
			...
			//get buttons
			self.showBoxB=$("#"+self.DOM.attr('id')+" > #content > div > #showBox");
			self.captureB=$("#"+self.DOM.attr('id')+" > #content > div > #captureButton");
			self.nextB=$("#"+self.DOM.attr('id')+" > #content > div > #nextButton");
			self.prevB=$("#"+self.DOM.attr('id')+" > #content > div > #prevButton");
			self.recognizeB=$("#"+self.DOM.attr('id')+" > #content > div > #autorec_recognize");
		...
		**/
		...
		_recognize:function(){
			...
			for (var i = 0; i < bucket.length; i++) {
				...
				var l=(self.regionBox.DOM.position().left/imgdims.l);
				var t=(($(jsel).position().top-self.regionBox.DOM.position().top)/imgdims.t);
			...
			if(!self.linesManifest[$("#srcImageForCanvas").attr('src')]) self.linesManifest[$("#srcImageForCanvas").attr('src')]=[];
			self.linesManifest[$("#srcImageForCanvas").attr('src')]
