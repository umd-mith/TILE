		...
		**/
		...
		_recognize:function(){
			...
			//	self.regionList[self.curRegion].images.push($("#srcImageForCanvas").attr('src'));
				...
				var left = _REG.left;
		        var rtop = _REG.top;   
		 		var tbarcorrect=$("#azcontentarea > .az.inner > .toolbar").innerHeight();
				if(console) console.log("_REG values: "+left+"  "+rtop+"  "+_REG.width);
				if(console) console.log("_REG CORRECT: "+scalecorrectx+"  "+scalecorrecty+"  l: "+(left*scalecorrectx)+" rtop: "+(rtop*scalecorrecty));
				if(console) console.log("tbarcorrect: "+tbarcorrect);
				...
				for (var i = 0; i < bucket.length; i++) {
					...
					if(self.transcript.lines[i]&&self.transcript.lines[i].shapes){
						//add data to the session's JSON object
						self.transcript.lines[i].shapes.push({"id":id,"type":"rect","_scale":1,"posInfo":{"x":(left*scalecorrectx),"y":(top-tbarcorrect)*scalecorrecty,"width":(_REG.width*scalecorrectx),"height":(height)*scalecorrecty}});
					}
		...
		_outputData:function(){
			...
			$(".az.main > .az.tool").animate({opacity:0.25,left:n},200,function(e){
				...
				$(this).animate({opacity:1,left:0},200,function(e){
					$(".lineBox").remove();
					$("body").trigger("closeOutAutoRec",[{scale:self.CANVAS._scale,data:self.transcript}]);
	 ...
	 var CanvasImage=TILEImage.extend({
		...
		init: function(args){
			...
			$(window).resize(function(e){
				if(console) console.log("resize autorec: "+self.uid);
				if($("#"+self.uid).length){
					$("#"+self.uid).width($("#azcontentarea").width());
					$("#"+self.uid).height($("#azcontentarea").height());
				}
			});
		...
		zoomHandle:function(e,v){
			...
			if(v>0){
				...
				self._scale=parseFloat(1.25*self._scale);
				...
				self.canvas[0].width=w;
				self.canvas[0].height=h;
				self._scale=parseFloat(0.75*self._scale);
		...
		setUpCanvas: function(url){
			var self = this;
			if(console) console.log("autorec canvas image is loading canvas: "+$("#srcImageForCanvas")[0].src+" "+url);
			$("#srcImageForCanvas").load(function(e){
				...
				if(console) console.log("autorec loading the canvas: "+$("#srcImageForCanvas")[0].width+" "+$("#srcImageForCanvas")[0].src);
		...
		var RegionBox=Monomyth.Class.extend({
			...
			_zoomHandle:function(e,v){
				...
				if(v>0){
					...
					var d=self._getDims();
					var top=(d.top*1.25);
					if(top<(self.DOM.parent().position().top+$("#azcontentarea > .az.inner > .toolbar").innerHeight())){
						top=(self.DOM.parent().position().top+$("#azcontentarea > .az.inner > .toolbar").innerHeight());
					}
			...
			var CanvasAutoRecognizer=AutoRecognizer.extend({
				...
				thresholdConversion:function(threshold){
					...
					if(this.Region){
						...
						if(console) console.log("conversion threshold: "+threshold);
						...
						if(console) console.log("converting: "+this.context);
...
var AR={
	...
	start:function(id,b,data,args){
		...
		// $("#az_activeBox").animate({"display":"none"},200);
	...
	restart:function(json,args){
		...
		$(".toolbar > ul > li > a > #listView").parent().hide();
		$(".toolbar > ul > li > #rect").parent().parent().hide();
		// make sure we have correct json data
		if(!json.lines) json=json[$("#srcImageForCanvas").attr("src")];
