	...
	**/
	var TileOCR=Monomyth.Class.extend({
		...
		_setUp:function(e,jsonhtml){
			...
			self.prevB=$("#"+self.DOM.attr('id')+" > #content > div.buttons > div > #prevButton").click(function(e){
				$(this).trigger("turnPage",[-1]);
			});
		...
		_closeOutAR:function(){
			...
			for(g in self.regionList){
				...
				for(area in self.regionList[g].etc){
					var o=self.regionList[g].etc[area];
					if(o.region){
						var l=(o.region.DOM.position().left/imgdims.w);
						var t=((o.region.DOM.offset().top-self.CANVAS.DOM.parent().offset().top)/imgdims.h);
						var w=o.region.DOM.width()/imgdims.w;
						var h=o.region.DOM.height()/imgdims.h;
						$.extend(o,{
							"args":[l,t,w,h], //all percentages of length, width, x, y
							"x":l,
							"y":t,
							"width":w,
							"height":h
				...
				$.merge(items,self.regionList[g].etc);
				self.regionList[g].done=false;
				self.regionList[g].etc=[];
			}
			...
			$(".az.main > .az.tool").animate({opacity:0.25,left:n},400,function(e){
				...
				if(self.canvasArea){ 
					...
					self.canvasArea.animate({opacity:0},400,function(){
				...
				//	var items=$.merge(self.lineManifest,self.regionList);
					...
					$("body").trigger("addLogBarItem",[{"items":items}]);
						$(".lineBox").remove();
				});
			});
		...
		_next:function(){
			...
			if(self.curRegion==self.regionList.length){
				//see if there's another image
				$("#srcImageForCanvas").load(function(e){
					$(this).unbind("load");
					if(!($(this).attr("src") in self.url)){
						self.url.push($(this).attr("src"));
					}
					$(".clline").removeClass("selected");
					self.curRegion=0;
					$("#cl-"+self.curRegion).addClass("selected");
					
					//remove the current lines
					$(".lineBox").remove();
					self._resetLines();
					
					var newScale=self.DOM.width()/$(this)[0].width;
					//change the regionBox scale
					self.regionBox._adjustSize(newScale);
				});
				
				self.DOM.trigger("turnPage",[1]);
				
				// if(URL_LIST){
				// 					var uri=null;
				// 					if(URL_LIST[self.curRegion]){
				// 						uri=URL_LIST[self.curRegion].uri;
				// 					} else {
				// 						self.curRegion=0;
				// 						uri=URL_LIST[self.curRegion].uri;
				// 					}
				// 					$("#srcImageForCanvas").attr("src",uri);
				// 				}
			}else{
				$(".clline").removeClass("selected");
				$("#cl-"+self.curRegion).addClass("selected");
		...
		_previous:function(){
			...
			if(self.curRegion<1){
				...
				$("#srcImageForCanvas").load(function(e){
					...
					$(".lineBox").remove();
					self._resetLines();
					var newScale=$(this)[0].width/self.DOM.width();
					//change the regionBox scale
					self.regionBox._adjustSize(newScale);
				});
				...
				$(".clline").removeClass("selected");
				$("#cl-"+self.curRegion).addClass("selected");
				//remove the current lines and add ones from current region
				$(".lineBox").remove();
				self._resetLines();
			}
		},
		_resetLines:function(){
			...
			var self=this;
			var scalew=self.CANVAS.canvas.width()/$("#srcImageForCanvas")[0].width;
			var scaleh=self.CANVAS.canvas.height()/$("#srcImageForCanvas")[0].height;
			var url=$("#srcImageForCanvas").attr('src');
			for(l in self.lineManifest){
				var line=self.lineManifest[l];
				if((line.uri==url)&&(self.curRegion==line.region)){
					//part of current image and region
					line.lineBox.DOM.appendTo(self.CANVAS.DOM.parent());
					var l=line.lineBox.DOM.position().left*scalew;
					var t=line.lineBox.DOM.position().top*scaleh;
					var w=line.lineBox.DOM.width()*scalew;
					var h=line.lineBox.DOM.height()*scaleh;
					line.lineBox.css({"left":l,"top":t,"width":w,"height":h});
				}
			}
		...
		canvasReadyHandle:function(e){
			...
			if(self.transcript){
				for(t in self.transcript){
					...
					out+="<div class='trLine selLine' id='trLine"+t+"'>"+self.transcript[t]+"</div>";
				}
			}else {
		...
		**/
		...
		_recognize:function(){
			...
			if(!($("#srcImageForCanvas").attr('src') in self.regionList[self.curRegion].images)) {
				...
				self.regionList[self.curRegion].etc=ldata;
				// //send to Logbar
			...
			//	$.merge(self.lineManifest,ldata);
				...
				$("body").bind("lineClicked",function(e,data){
					self.DOM.unbind("keypress");
					var num = data.substring(8);
					var id = "recLine"+num;
					$(".recLine").removeClass("selected");
					$("#"+id).addClass("selected");
					self.DOM.keypress(function(e){
						e.preventDefault();
						if(e.keyCode==27){
							$("#"+id).remove();
						}
		...
		**/
		...
		var lineBox=Monomyth.Class.extend({
			...
			zoomHandle:function(e,v){
				var self=e.data.obj;
				if(v<0){
					var w=self.DOM.width()*2;
					var h=self.DOM.height()*2;
					var l=self.DOM.position().left*2;
					var t=self.DOM.position().top*2;
...
var AR={
	start:function(id,schema){
		this.__AR__=new TileOCR({loc:id,transcript:schema.transcript});
	},
	name:"autorecognizer",
