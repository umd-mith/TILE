	...
	**/
	var TileOCR=Monomyth.Class.extend({
		...
		_setUp:function(e,jsonhtml){
			...
			self.showBoxB=$("#"+self.DOM.attr('id')+" > #content > div.buttons > div > #showBox");
			self.captureB=$("#"+self.DOM.attr('id')+" > #content > div.buttons > div > #captureButton");
			self.nextB=$("#"+self.DOM.attr('id')+" > #content > div.buttons > div > #nextButton");
			self.prevB=$("#"+self.DOM.attr('id')+" > #content > div.buttons > div > #prevButton");
			self.recognizeB=$("#"+self.DOM.attr('id')+" > #content > div.buttons > div > #autorec_recognize");
		...
		**/
		...
		_recognize:function(){
			...
			for (var i = 0; i < bucket.length; i++) {
				...
				var l=(self.regionBox.DOM.position().left*imgdims.w);
				var t=(($(jsel).position().top)*imgdims.h);
				var w=$(jsel).width()/imgdims.w;
				var h=$(jsel).height()/imgdims.h;
	 ...
	 var CanvasImage=Image.extend({
		...
		setUpCanvas: function(url){
			...
			_getPerc:function(){
				...
				var t=(dp.top-rp.top);
				var w=(self.canvas.width()/$("#srcImageForCanvas")[0].width);
				var h=(self.canvas.height()/$("#srcImageForCanvas")[0].height);
