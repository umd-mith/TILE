...
//bugs:
// needs to be modular - must be able to use alone in stand-alone environment
// zooming in: container width and height shrink, but image gets larger
// vd-container doesn't extend enough outside size of image - need to make unscrollable?
// exporting JSON: all information must be in JSON:
	// exporting ID and position info
	// all shapes must be contained in image object w/ uri and orig height and width
// offset positions actually refer to a percentage in shape details
// image:{
       // uri:
       //    origHeight
       //    origWidth
       ...
       //  }
       // ]
//autorec should spit out shape JSON: export shapes
//user-testing
...
(function($){
	...
	**/
    ...
    pic_real_width = $("#srcImageForCanvas").width;
    pic_real_height = $("#srcImageForCanvas").height;
	
});

var src = $("#srcImageForCanvas").attr("src");
$("#srcImageForCanvas").attr({
	"src": ""
});
$("#srcImageForCanvas").attr({
	"src": src
		...
		zoomHandle:function(e,n){
			...
			if(self.drawTool&&($("#srcImageForCanvas").width()!=0)){
				...
				//change the height and width of the actual svg container
				$("svg").width(parseFloat($(".vd-container").width()));
				$("svg").height(parseFloat($(".vd-container").height()));
