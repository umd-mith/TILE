...
(function($){
	...
	_Itag.prototype={
		...
		inputData:function(data){
			var self=this;
			self.raphael._addLinkHandle(data);
		},
	...
	var RaphaelImage=TILEImage.extend({
		...
		_shapeActiveHandle:function(e,id){
			...
			for(x in self.manifest[url].shapes){
				if(self.manifest[url].shapes[x].id==id){
					// notify that this has been clicked
					$("body:first").trigger("receiveShapeObj",[{"id":id,"type":"shapes"}]);
					if($("#selBB > .shpButtonHolder").length) return;
					// attach delete button
					$("<div class=\"shpButtonHolder\"><div id=\""+self.manifest[url].shapes[x].id+"\" class=\"button\">Delete</div></div>").click(function(){
						// if(console) console.log("delete got hit too");
						self.drawTool.deleteShape(self.manifest[url].shapes[x].id);
						$("body:first").trigger("shapeDeleted",[self.manifest[url].shapes[x].id]);
					...
					if(/^D_/.test(id)){
						...
						// Approve
						// Approve All
						$("<div id=\"approve_"+self.manifest[url].shapes[x].id+"\" class=\"button\">Approve</div>").click(function(e){
							e.stopPropagation();
							var id=$(this).attr("id").replace("approve_","");
							// if(console) console.log("clicked on shape "+id+" to approve it");
							self._approveItemHandle(id);
						}).appendTo($("#selBB > .shpButtonHolder"));
						$("<div id=\"approveAllButton\" class=\"button\">Approve All</div>").click(function(e){
							e.stopPropagation();
					...
					// if(console) console.log("no of items in ref: "+self.manifest[url].shapes[x].labels);
					for(item in self.manifest[url].shapes[x]){
						if((!(/posInfo|id|color|type|_scale/.test(item)))&&(self.manifest[url].shapes[x][item]!="")){
							...
							$("#delete_"+self.manifest[url].shapes[x][item]).click(function(e){
								if(console) console.log("clicked: "+self.manifest[url].shapes[x][item]);
								// delete the link
		...
		_shapeDrawnHandle:function(e,shpObj){
			...
			$("<div class=\"shpButtonHolder\"><div id=\""+shpObj.id+"\" class=\"button\">Delete</div></div>").click(function(e){
			...
			$("body:first").trigger("receiveShapeObj",[{"id":shpObj.id,"type":"shapes"}]);
		},
		...
		_approveItemHandle:function(id){
			...
			if(console) console.log("Approving a line: "+id);
			//clear all shapes
			self.drawTool.clearShapes();
			...
			$("body:first").trigger("deleteLink",[{"id":id,"type":"shapes"}]);
			$("body:first").trigger("addLink",[{id:id.replace("D_",""),type:"shapes"}]);
			// var sendShps=[];
		...
		_loadShapesHandle:function(e,json){
			...
			// clear the canvas
			self.drawTool.clearShapes();
			if(console) console.log("loadShapes receives json: "+json);
			for(j in json){
				var shid=json[j];
		...
		_addLinkHandle:function(ref){
			...
			var id=$("#selBB").children("div.button:eq(0)").attr("id");
			var url=$("#srcImageForCanvas").attr("src");
			for(sh in self.manifest[url].shapes){
				if(self.manifest[url].shapes[sh].id==id){
					// does this reference already exist?
					if(!self.manifest[url].shapes[sh][ref.type]) {
						self.manifest[url].shapes[sh][ref.type]=[];
					} else {
						for(x in self.manifest[url].shapes[sh][ref.type]){
							if(self.manifest[url].shapes[sh][ref.type][x]==ref.id) {
								return;
								break;
							}
						}
					...
					// $("#selBB").append($("<div class=\"button\">"+ref.id+"</div><span id=\"delete_"+ref.id+"\" class=\"button\">X</span>"));
					// 				self.manifest[url].shapes[sh][ref.type].push(ref.id);
					...
					// 					$(this).parent().remove();
					// 				});
					// send addLink to listening objects
					// $("body:first").trigger("addLink",[{id:self.manifest[url].shapes[sh].id,type:'shapes'}]);
					break;
				}
			}
			...
			for(sh in self.manifest[url].shapes){
				...
				for(x in shape[ref.type]){
					if(shape[ref.type][x]==ref.id){
						if(console) console.log("found: "+shape[ref.type][x]);
...
var IT={
	...
	_receiveShapeObjHandle:function(e,data){
		var self=e.data.obj;
		if(console) console.log("receiveObj "+data);
		$("body:first").trigger(self.outputCall,[data]);
	},
	_shapeDeletedHandle:function(e,data){
		var self=e.data.obj;
		$("body:first").trigger(self.deleteCall,[{"id":data,"type":"shapes"}]);
	},
