	...
	var RaphaelImage=Image.extend({
		...
		addUrl:function(url){
			...
			if((url!=null)){
				...
				for(u in url.pages){
					...
					var cu=(url.pages[u].uri||url.pages[u].url);
					var lines=(url.pages[u].lines)?url.pages[u].lines:[];
					// var title = url[u].title;
					// if(!url.pages[u].uid){
					// 						var _d=new Date();
					// 						var uid=_d.getTime("milliseconds")+"img";
					// 					}
					if((cu.length>1)){
						//var cleanurl=url[u].uri.replace(/^[A-Za-z0-9]+|[\t\r]+/,"");
					
						this.url.push(url.pages[u]);
						//Master Array ---
						//formatted to be output as a JSON
						this.manifest[url.pages[u].url]=url.pages[u];
					} else if(this.manifest[url.pages[u].url]){
						//duplicate already exists; add info and lines to this image
						$.each(this.manifest[url.pages[u].url].info,function(i,o){
							if(i in url.pages[u].info){
								if($.isArray(o)){
									o.push(url.pages[u].info[i]);
								} else {
						...
						$.merge(this.manifest[url.pages[u].url].lines,url.pages[u].lines);
					}
					this.pageNum=0;
				}
			}
		...
		setUpCanvas:function(url,max,min){
			...
			$("#srcImageForCanvas").load(function() {
				...
				//make sure canvas is set to right width and height
				if(self._imgScale!=self.drawTool.scale)	self.drawTool.scale(self._imgScale);
		...
		_loadShapesHandle:function(e,json){
			...
			for(p in json){
				...
				if(json[p]._scale!=self._imgScale){
					diff=json[p].scale-self._imgScale;
			...
			if(diff<0){
				...
				alert(self.drawTool._scale);
			self.drawTool._scale=self._imgScale;
		},
		// handles {Event} 'zoom'
		// e : {Event}
