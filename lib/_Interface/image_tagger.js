	...
	**/
	...
	//		this.DOM.css({width:this.srcImage.width(),height:$("#scroller").position().top});
		...
		setUpDrawTool:function(){
			...
			self.drawTool=new VectorDrawer({"overElm":$("#azcontentarea > .az.inner > #raphworkspace_"),"initScale":1});
			//set up triggers
		...
		_loadShapesHandle:function(e,json){
			var self=e.data.obj;
			self.drawTool.importShapes(json); //send to drawTool
		},
		...
		showImg:function(e,val){
			...
			if((val>0)){
				...
				if(obj.drawTool){
					obj.manifest[obj.curUrl]=obj.drawTool.exportShapes();
				}
...
var IT={
	...
	start:function(id,base,json){
		this.itagger=new _Itag({loc:id,base:base,json:(json)?json.images:null});
	},
	restart:function(){
