	...
	**/
	...
	var RaphaelImage=TILEImage.extend({
		...
		_addLinkHandle:function(ref,shape){
			if(__v) console.log('addlinkhandle for: '+shape.id);
			...
			$("span#delete_"+ref.id).bind("click",function(e){
				if(__v) console.log("delete clicked on "+ref.id);
				$(this).prev().remove();
				$(this).remove();
...
var IT={
	...
	bundleData:function(j){
		...
		for(o in j){
			...
			if(j[o]){
				if(__v) console.log("bundleData imagetagger: j["+o+"]="+JSON.stringify(j[o]));
				j[o].shapes=$.extend(true,{},itagData[o]);
				// if(__v) console.log("SHAPE DATA: "+JSON.stringify(j[o].shapes));
			}
		}
		return j;
	},
