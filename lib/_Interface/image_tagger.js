...
(function($){
	...
	var _Itag=function(args){
		...
		var ingestImages=function(e,manifest){
			var obj=e.data.obj;
			//send image file data to canvas
			//obj.raphael.manifest=manifest;
	...
	var ImageList=Monomyth.Class.extend({
		...
		show:function(data){
			...
			for (var i=0;i<data.length;i++){
				 $("<li><a href='"+data[i].url+"'>"+data[i].title+"<\/a></li>").appendTo(".az.inner.textImageList > .list");	
			}
	...
	**/
	...
	var RaphaelImage=Image.extend({
		...
		setUpCanvas:function(url,max,min){
			...
			//	self.loc.width(pic_real_width);
				
				self.loc.height(pic_real_height);
