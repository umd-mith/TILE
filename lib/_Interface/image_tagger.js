	...
	**/
	...
	var RaphaelImage=TILEImage.extend({
		...
		setUpCanvas:function(url){
			...
			$("#srcImageForCanvas").load(function(e){
				...
				if($("#raphworkspace_").width()>=$("#azcontentarea:first").width()){
					...
					diff= $("#azcontentarea > .az.inner > .toolbar").innerHeight()-$("#azcontentarea > .az.inner > .toolbox").height(); // $("#azcontentarea:first").innerHeight()-$("#raphworkspace_").innerHeight();
					$("#raphworkspace_").height($("#raphworkspace_").height()-diff);
		...
		_loadShapesHandle:function(e,json){
			var self=e.data.obj;
			for(s in json){
				if((json[s]._scale==1)&&(json[s]._scale!=self._imgScale)){
