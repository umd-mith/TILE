	...
	**/
	...
	var RaphaelImage=TILEImage.extend({
			...
			for(x in self.manifest[url].shapes){
				if(self.manifest[url].shapes[x].id==id){
					...
					if(/^D_/.test(id)){
						...
						$("<div id=\"approveAllButton\" class=\"button\">Approve All</div>").click(function(){
					...
					for(item in self.manifest[url].shapes[x]){
						if(!(/posInfo|id|color|type|_scale/.test(item))){
							$("<div id=\"\" class=\"button\">"+self.manifest[url].shapes[x][item]+"<span id=\"delete_"+self.manifest[url].shapes[x][item]+"\" class=\"button\"></span></div>").appendTo($("#selBB"));
		...
		_shapeDrawnHandle:function(e,shpObj){
			...
			$("<div id=\""+shpObj.id+"\" class=\"button\">Delete</div>").click(function(){
		...
		_addLinkHandle:function(e,ref){
			...
			for(sh in self.manifest[url].shapes){
				if(self.manifest[url].shapes[sh].id==id){
					...
					$("#selBB").append($("<div class=\"button\">"+ref.id+"<span id=\"delete_"+ref.id+"\" class=\"button\">X</span></div>"));
