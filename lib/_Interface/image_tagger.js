	...
	**/
	...
	var RaphaelImage=TILEImage.extend({
		...
		_shapeActiveHandle:function(e,id){
			...
			for(x in self.manifest[url].shapes){
				if(self.manifest[url].shapes[x]&&(self.manifest[url].shapes[x].id==id)){
					...
					if($("#selBB > .shpButtonHolder > #"+foundID).length) return;
					$("#selBB").html("");
					
					// attach delete button
					$("<div class=\"shpButtonHolder\"><div id=\""+foundID+"\" class=\"button\">Delete</div></div>").appendTo($("#selBB"));
					if(__v) console.log("attached a delete button for: "+$("#"+foundID));
		...
		_shapeDrawnHandle:function(e,shpObj){
			...
			$("<div class=\"shpButtonHolder\"><div id=\""+shpObj.id+"\" class=\"button\">Delete</div></div>").appendTo($("#selBB"));
			
			$(".shpButtonHolder > #"+shpObj.id).click(function(e){
				e.stopPropagation();
