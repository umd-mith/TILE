	...
	**/
	...
	var RaphaelImage=TILEImage.extend({
		init:function(args){
			...
			$(window).resize(function(e){
				if($("#raphworkspace_").length){
					...
					$("#raphworkspace_").width($("#azcontentarea").width())-12;
					$("#raphworkspace_").height($("#azcontentarea").height())-8;
				}
			});
			console.log("wrkspace width: "+$("#raphworkspace_").width()+"   az inner width: "+$("#azcontentarea:first").width());
			if($("#raphworkspace_").width()>=$("#azcontentarea:first").width()){
				// adjust width
				var diff=$("#azcontentarea:first").innerWidth()-$("#raphworkspace_").width();
				$("#raphworkspace_").width($("#raphworkspace_").width()-diff);
				diff=$("#azcontentarea > .az.inner > .toolbar").innerHeight(); // $("#azcontentarea:first").innerHeight()-$("#raphworkspace_").innerHeight();
				$("#raphworkspace_").height($("#raphworkspace_").height()-8);
		...
		_shapeChangeHandle:function(e){
			...
			console.log("shape is changed: "+vd);
			...
			for(s in json){
				...
				if((json[s]._scale!=self._imgScale)){
					...
					$.each(json[s].posInfo,function(i,o){
						if($.inArray(i,xval)>-1){
							json[s].posInfo[i]*=corx;
						} else if($.inArray(i,yval)>-1){
							json[s].posInfo[i]*=cory;
						}
					});
				}
			}
			console.log("raphael loading shapes:  "+JSON.stringify(json));
		...
		zoomHandle:function(e,n){
			...
			if(self.drawTool&&($("#srcImageForCanvas").width()!=0)){
				...
				if(n>0){
					...
					$(".vd-container").width(1.25*parseFloat($(".vd-container").width())-12);
					$(".vd-container").height(1.25*parseFloat($(".vd-container").height())-8);
