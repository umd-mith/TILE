	...
	**/
	...
	var RaphaelImage=TILEImage.extend({
		...
		_shapeActiveHandle:function(e,id){
			...
			for(x in self.manifest[url].shapes){
				if(self.manifest[url].shapes[x].id==id){
					if(__v) console.log("shape "+id+" clicked on. found: "+self.manifest[url].shapes[x].id);
					var foundID=id;
					...
					$("<div class=\"shpButtonHolder\"><div id=\""+foundID+"\" class=\"button\">Delete</div></div>").click(function(){
						// if(__v) console.log("delete got hit too");
						if(/^D\_/.test(foundID)){
					...
					$("body:first").trigger("receiveShapeObj",[{"id":id,"type":"shapes",attachHandle:"#selBB > .shpButtonHolder"}]);
					if(/^D\_/.test(foundID)){
		...
		_shapeDrawnHandle:function(e,shpObj){
			...
			$("body:first").trigger("receiveShapeObj",[{"id":shpObj.id,"type":"shapes",attachHandle:"#selBB > .shpButtonHolder"}]);
		},
		...
		_loadShapesHandle:function(e,json){
			...
			for(j in json){
				var shid=json[j];
				if(__v) console.log("loadShapes shid: "+shid);
				for(sh in self.manifest[url].shapes){
					if(self.manifest[url].shapes[sh]&&(self.manifest[url].shapes[sh].id==shid)){
						if(__v) console.log("loadShapes creating vd, adding: "+self.manifest[url].shapes[sh].id);
		...
		_addLinkHandle:function(ref){
			...
			var url=$("#srcImageForCanvas").attr("src");
			if(__v) console.log("addLinkHandle image_tagger.js: "+ref.id);
			$("#selBB > .shpButtonHolder").append($("<div class=\"button\">"+ref.id+"</div><span id=\"delete_"+ref.id+"\" class=\"button\">X</span>"));
...
var IT={
	...
	start:function(id,json,layout){
		...
		$("body").bind("deleteShapeLink",{obj:self},function(e,data){
			...
			if(__v) console.log("deleteShapeLink found: "+shid);
			data.objID=shid;
			data.objType="shapes";
			
			if(__v) console.log("deleteShapeLink called in imagetagger");
	...
	_shapeDeletedHandle:function(e,data){
		...
		$("body:first").trigger(self.deleteCall,[{"id":data,"type":"shapes","objID":data,"objType":"shapes",tool:self.id}]);
	},
