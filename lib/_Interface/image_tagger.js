	...
	**/
	...
	var AutoRecLine=Shape.extend({
		init:function(args){
			this.$super(args);
			this._obj=args.data;
			this.DOM=$("#lineBox_"+this._obj.order);
		...
		resizeData:function(newScale){
			var self=this;
			var wx=($("#srcImageForCanvas")[0].width*self._obj._scale)/($("#srcImageForCanvas")[0].width*newScale);
			var wy=($("#srcImageForCanvas")[0].height*self._obj._scale)/($("#srcImageForCanvas")[0].height*newScale);
			self.DOM.css({"left":(parseFloat(self.DOM.css("left"))*wx),"top":(parseFloat(self.DOM.css("top"))*wy)});
