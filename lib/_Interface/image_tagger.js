	...
	var RaphaelImage=Image.extend({
		...
		showImg:function(e,val){
			...
			if((val>0)){
				...
				if(obj.drawTool){
					// var shapes=obj.drawTool.exportShapes();
					// 					//must organize into id:{shape} array
					// 					var orderedShapes=[];
					// 					for(id in shapes){
					// 						orderedShapes[shapes[id].id]=shapes[id];
					// 					}
		...
		goToPage:function(n){
			var self=this;
			// if(self.curUrl){
			// 			self.manifest[self.curUrl].shapes=self.drawTool.exportShapes();
			// 		}
			self.pageNum=n;
