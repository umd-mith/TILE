...
(function($){
	...
	var _Itag=function(args){
		...
		var self=this;
		//this.imageListHTML = args.imageListHTML;
	...
	_Itag.prototype={
		...
		showImageList:function(e){
			...
			// 			}
			// 			
			// 			$("#azcontentarea > div.az").hide();
			// 			
			// 			$("#azcontentarea").append(e.data.obj.imageList);
			// 			
			// 			data = obj.raphael.url;
			// 			for (var i=0;i<data.length;i++){
			// 				 $("<li class='imageItem'><a href='#'><span class='imageTitle'>"+data[i].title+"</span> <span class='imgURI'>"+data[i].uri+"</span><\/a></li>").appendTo("#imgListUL");	
			// 			}
			// 			
			// 			$(".imageItem").mouseover(function(e){
			// 				
			// 				uri = ($(this).find("span:eq(1)").text());
			// 				$("#imagePreviewArea").html("<img src='"+uri+"' width=100>");
			// 			}).click(function(e){
			// 				uri = ($(this).find("span:eq(1)").text());
			// 				$("#textImageList").remove();
		...
		**/
		_restart:function(){
			...
			//self.raphael._restart();
		//var w=	$("#srcImageForCanvas")[0].width;
		//var h= $("#srcImageForCanvas")[0].height;
		//$("#srcImageForCanvas").width(w).height(h);
			//self.srcImage.css({"width":self.srcImage[0].width,"height":self.srcImage[0].height});
	...
	var ImageList=Monomyth.Class.extend({
		...
		show:function(data){
			...
			//data = $("#srcImageForCanvas").attr('src');
			for (var i=0;i<data.length;i++){
			...
			$(".az.inner.textImageList > .list > li > a").mouseover(function(e){
				...
				$("body").trigger("switchImage",[uri]);
				//go back to canvas
				$("#azcontentarea > .az.inner.textImageList").remove();
				$("#azcontentarea > div.az").show();
	...
	**/
	...
	var RaphaelDrawTool=DrawTool.extend({
		...
		//can add either array of objects or single object
		_addExtraShape:function(e,shape,args){
			var self=e.data.obj;
			args["items"]=[];
			for(x in shape){
				var o=shape[x];
				...
				$.extend(o,{
					cur:null,
					id:o.uid,
					uri:(o.uri)?o.uri:$("#srcImageForCanvas").attr("src"),
					shapeChangeCall:"shapeChangeCall"+o.uid,
					anchor:false,
					_display:false,
					deactivate:function(){
						this.cur.hide();
				...
				//adjust size/position data
				
				$.map(["x","width","cx","rx"],function(p,n){
					if(p in o){
						
						o[p]=(o[p]*self._over.elm.width());
					
					}
				});
				
				$.map(["y","height","cy","rh"],function(p,n){
					if(p in o){
						o[p]=(o[p]*self._over.elm.height());
					}
				});
				//update args variable
				switch(o.con){
					case 'rect':
						o.args=[o.x,o.y,o.width,o.height];
						break;
					case 'elli':
						o.args=[o.cx,o.cy,o.rw,o.rh];
						break;
					case 'poly':
						break;
				}
				self._manifest[o.uri.replace(/^[ \t\r\n]+|[ \t\r\n]+$/,"")].push(o);
				o.type='shape';
				//pass on the shape and any args along to listeners
				args.items.push(o);
				//self._loc.trigger("shapeCommit",[o,args]);
			}
			...
			self._allObjs=self._manifest[$("#srcImageForCanvas").attr('src')];
			...
			setTimeout(function(obj,w,h,args){
				obj._buildCanvas(w,h);
				$("body").trigger("addLogBarItem",[args]);
			},1,self,newSize.width,newSize.height,args);
		},
		...
		/*zoom:function(e,val){
			...
			if((val>0)&&(obj.zoomLevel<obj.maxZoom)){
				...
				alert("+ "+obj.zoomLevel/obj.maxZoom);
				$("srcImageForCanvas").width=(obj.zoomLevel/obj.maxZoom)*$("srcImageForCanvas").width;
				$("srcImageForCanvas").height=(obj.zoomLevel/obj.maxZoom)*$("srcImageForCanvas").height;
			} else if((val<0)&&(obj.zoomLevel>obj.minZoom)){
				obj.zoomLevel--;
				alert("- "+obj.zoomLevel/obj.maxZoom);
				obj.drawer.scale((obj.zoomLevel/obj.maxZoom));
				$("srcImageForCanvas").width($("srcImageForCanvas").width/(obj.zoomLevel/obj.maxZoom));
				$("srcImageForCanvas").height($("srcImageForCanvas").height/(obj.zoomLevel/obj.maxZoom));
				alert()
			}
		},*/
	...
	**/
	...
	//		this.DOM.css({width:this.srcImage.width(),height:$("#scroller").position().top});
		...
		setUpDrawTool:function(){
			...
			self.drawTool=new VectorDrawer({"overElm":$("#azcontentarea > .az.inner > #raphworkspace_ > #raphael"),"initScale":1});
			//set up triggers
		...
		zoomHandle:function(e,n){
			...
			if(self.drawTool&&($("#srcImageForCanvas").width()!=0)){
				...
				if(n>0){
					...
					$("#srcImageForCanvas").width(1.25*$("#srcImageForCanvas").width());
					$("#srcImageForCanvas").height(1.25*$("#srcImageForCanvas").height());
							$(".vd-container").width(1.25*$(".vd-container").width());
					$(".vd-container").height(1.25*$(".vd-container").height());
					//self.srcImage.css({"width":$(".vd-container").width()+'px',"height":$(".vd-container").height()+"px"});
					
				} else if(n<0){
					$("#srcImageForCanvas").width(0.75*$("#srcImageForCanvas").width());
					$("#srcImageForCanvas").height(0.75*$("#srcImageForCanvas").height());
		...
		//	$("#srcImageForCanvas").css({"width":$(".vd-container").width()+'px',"height":$(".vd-container").height()+"px"});
		//	self.setUpCanvas(self.curUrl);
		},
