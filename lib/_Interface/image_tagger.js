...
(function($){
	...
	**/
	...
	var ImageList=Monomyth.Class.extend({
		...
		//Takes the imagelist away from the DOM
		back:function(e){
			var self=e.data.obj;
			$("#"+self.loc+" > .az.inner.textImageList").remove();
			$("#"+self.loc+" > div.az").show();
	...
	**/
	...
	var TILEShapeToolBar=SideToolBar.extend({
		...
		// type is and activates that shape choice
		// changes DOM styles
	...
	**/
	...
	var RaphaelImage=Image.extend({
		...
		_loadShapesHandle:function(e,json){
			...
			for(p in json){
				if(json[p].scale!=self._imgScale){
			...
			if(diff<0){
				//need to scale it up
				var c=Math.ceil(Math.abs(diff)/1.25);
				for(x=1;x<c;x++){
					self.drawTool.scale(1.25);
				}
			} else {
				//need to scale it down
				var c=Math.ceil(Math.abs(diff)/0.75);
				for(x=1;x<c;x++){
