	...
	**/
	...
	var RaphaelImage=TILEImage.extend({
		...
		_shapeDrawnHandle:function(e,shpObj){
			...
			self.manifest[$("#srcImageForCanvas").attr('src')].shapes=vd;
		...
		_approveItemHandle:function(e,id){
			...
			if(elx!=null) {
			 	self.drawTool.importShapes(JSON.stringify([self.manifest[url].shapes[elx]]));
 			}
		...
		_deleteItemHandle:function(e,id){
			var self=e.data.obj;
			for(obj in self.manifest[$("#srcImageForCanvas").attr('src')].shapes){
				if(self.manifest[$("#srcImageForCanvas").attr('src')].shapes[obj].id==id){
					self.manifest[$("#srcImageForCanvas").attr('src')].shapes[obj]=null;
				}
			}
