	...
	**/
	...
	var RaphaelImage=TILEImage.extend({
		...
		_shapeActiveHandle:function(e,id){
			...
			for(x in self.manifest[url].shapes){
				if(self.manifest[url].shapes[x].id==id){
					...
					$("<div id=\""+self.manifest[url].shapes[x].id+"\" class=\"button\">Delete</div>").click(function(){
		...
		_shapeDrawnHandle:function(e,shpObj){
			...
			$("<div id=\""+shpObj.id+"\" class=\"button\">Delete</div>").click(function(e){
				self.drawTool.deleteShape(shpObj.id);
			}).appendTo($("#selBB"));
			
			
			// take shape data and pass to listening objects
			$("body:first").trigger("addLink",[{"id":shpObj.id,"type":"shapes"}]);
		},
		...
		_addLinkHandle:function(e,ref){
			...
			for(sh in self.manifest[url].shapes){
				if(self.manifest[url].shapes[sh].id==id){
					...
					if(console) console.log("delete_"+ref.id);
					$("#delete_"+ref.id).click(function(e){
						
						// delete the link 
						if(console) console.log("ref: "+ref.id+" "+ref.type);
		...
		_deleteLinkHandle:function(e,ref){
			if(console) console.log("got to deleteLink: "+ref.id);
			if(ref.type=='shapes') return;
			var self=e.data.obj;
