	...
	**/
	...
	var RaphaelImage=TILEImage.extend({
		...
		_loadShapesHandle:function(e,json){
				...
				if(json[j].type=='autorect'){
					...
					if($(".lineBox").length!=self._autoLines.length){
						...
						$(".lineBox").each(function(i,o){
							...
							// vd shapes
							// var el=json[j];
							// var data=$.extend(true,json[j],{
							// 								"imgScale":self._imgScale
							// 							});
							console.log("line: "+$(o).attr('id')+'  '+self._autoLines[$(o).attr('id').replace("lineBox_","")]);
							if(self._autoLines[$(o).attr('id').replace("lineBox_","")]){
								var x=parseFloat($(o).css("left"));
								var y=parseFloat($(o).css("top"));
								var data={'id':$(o).attr('id').replace("lineBox_",""),'type':'autorect','_scale':self._autoLines[$(o).attr("id").replace("lineBox_","")]._obj._scale,'posInfo':{'x':x,'y':y,'width':$(o).width(),'height':$(o).height()}};
								var arl=new AutoRecLine({initialTop:y,initialBottom:y+$(o).height(),foundOnRow:i,data:data});
								self._autoLines[data.id]=arl;
								console.log('autolines['+data.id+'] = '+self._autoLines[data.id]);
							}
						});
					...
					if(self._autoLines[json[j].id]) {
						console.log("received id for "+json[j].id+" which is an autoline with: "+self._autoLines[json[j].id].DOM);
			...
			self.drawTool.importShapes(JSON.stringify(shpsLoad));
		},
		setAutoLines:function(data){
			var self=this;
			
			self._autoLines=[];
			for(d in data){
				for(sh in data[d].shapes){
					if(data[d].shapes[sh].type=='autorect'){
						var arl=new AutoRecLine({initialTop:data[d].shapes[sh].posInfo.y,initialBottom:data[d].shapes[sh].posInfo.y+data[d].shapes[sh].posInfo.height,foundOnRow:data[d].shapes[sh].order,data:data[d].shapes[sh]});
						self._autoLines[data[d].shapes[sh].id]=arl;
						console.log('autolines['+data[d].shapes[sh].id+'] = '+self._autoLines[data[d].shapes[sh].id]);
					}
				}
		...
		_approveItemHandle:function(e,id){
			...
			if(self._autoLines[id]){
				...
				// var allShapes=self._drawTool.exportShapes();
				$.extend(true,allShapes,data);
				// self._drawTool.importShapes(allShapes);
				// send to current transcript line
				$("body:first").trigger("receiveShapeObj",[data]);
			}
		},
		_deleteItemHandle:function(e,id){
			var self=e.data.obj;
			if($("#lineBox_"+id).length){
				self._shiftAutoLines(id);
				if(self._autoLines[id]) self._autoLines[id]=null;
			} else {
				$("body:first").trigger("removeShapeObj",[id]);
			}
		},
		_shiftAutoLines:function(id){
			...
			var line=self._autoLines[id].DOM;
			// for all of the deleted line's siblings, each sibling
			// gets the previous siblings' id
			var plines=line.nextAll('.lineBox').each(function(i,o){
				$(o).attr('id',$(o).prev('.lineBox').attr('id'));
			});
			line.remove(); 
			
			
			// 			line.remove();
			// 			$(".lineBox").each(function(e){
			// 				var t=parseFloat($(this).css("top"));
			// 				if(t>top){
			// 					if(self._autoLines[$(this).attr("id").replace("lineBox_","")]){
			// 						self._autoLines[id].DOM=self._autoLines[$(this).attr("id").replace("lineBox_","")].DOM;
			// 						id=$(this).attr('id').replace("lineBox_","");
			// 						top=t;
			// 					}
			// 				}
			// 			});
		...
		_restart:function(json){
			...
			// parse new shape data
			self.setAutoLines(json.lines);
	...
	**/
	...
	var AutoRecLine=Shape.extend({
		...
		convertToJSON:function(){
			var self=this;
			var left=parseInt(self.DOM.css('left'),10);
			var top=parseInt(self.DOM.css('top'),10);
			var posInfo={'x':left,'y':top,'width':self.DOM.width(),'height':self.DOM.height()};
			
			var json={'id':self.uid,'type':'rect','_scale':self._obj._scale,'color':"#FFFFFF",'posInfo':posInfo};
			return json;
