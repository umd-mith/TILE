	...
	**/
	...
	var RaphaelImage=TILEImage.extend({
		...
		setUpCanvas:function(url){
			...
			newImg.src = url;
			var prh = newImg.height;
			var prw = newImg.width;
			$("#srcImageForCanvas").load(
				function(self,prw,prh){
					
					
					
					$("#srcImageForCanvas").width(prw);
					$("#srcImageForCanvas").height(prh);
					if(!self.drawTool){
					...
					$(".vd-container:first").width(prw);
					$(".vd-container:first").height(prh);
					if(self.manifest[url]){
							self.manifest[url].origWidth=prw;
							self.manifest[url].origHeight=prh;
					}
					self._imageScale = 1;
