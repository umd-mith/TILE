	...
	var RaphaelImage=TILEImage.extend({
		init:function(args){
			...
			$(window).resize(function(e){
				if($("#raphworkspace_").length){
					...
					$("#raphworkspace_").width($("#azcontentarea:first").innerWidth());
					diff=$("#azcontentarea > .az.inner > .toolbar").innerHeight();
					$("#raphworkspace_").height($("#azcontentarea").height()-diff);
				}
			});
		...
		setUpCanvas:function(url){
			...
			$("#srcImageForCanvas").load(function(e){
				...
				// if(console) console.log("vdcontainer:first is : "+$("#srcImageForCanvas")[0].width+"  by "+$("#srcImageForCanvas")[0].height);
		...
		_shapeChangeHandle:function(e){
			...
			if(console) if(console) console.log("shape is changed: "+vd);
		...
		_loadShapesHandle:function(e,json){
			...
			// 				}
			// 			}
			// 			if(console) console.log("raphael loading shapes:  "+JSON.stringify(json));
		...
		zoomHandle:function(e,n){
			...
			if(self.drawTool&&($("#srcImageForCanvas").width()!=0)){
				...
				if(console) console.log("imagetagger scale: "+self._imgScale+"  height: "+$("#srcImageForCanvas").height()+"  src: "+$("#srcImageForCanvas").attr('src'));
		...
		_restart:function(){
			...
			//$("body").bind("zoom",{obj:self},self.zoomHandle);
			if(console) console.log("restarting raphael: "+self._imgScale);
		},
