	...
	**/
	...
	var RaphaelImage=TILEImage.extend({
		...
		_loadShapesHandle:function(e,json){
			...
			for(j in json){
				if(json[j].order!=null){
					...
					var arl=new AutoRecLine({initialTop:data.posInfo.y,initialBottom:(data.posInfo.y+data.posInfo.height),foundOnRow:json[j].order,data:data});
		...
		_shiftAutoLines:function(id){
			...
			var temp=[];
			var pline=null;
			var line=$("#lineBox_"+id);
			var top=parseFloat(line.css("top"));
			...
			if(self._autoLines[id]){
				self._autoLines[id]=null;
				// var fo=self._autoLines[id].foundOnRow;
				// 				var d={"id":id,'type':'rect','color':"#FFFFFF",'posInfo':{'x':parseFloat($(this).css("left")),'y':parseFloat($(this).css('top')),'width':$(this).width(),'height':$(this).height()}};
				// 				self._autoLines[id]=new AutoRecLine({initialTop:d.posInfo.x,initialBottom:(d.posInfo.y+d.posInfo.height),foundOnRow:fo,data:d});
			}
			// for(a in self._autoLines){
			// 				if(a==id){
			// 					self._autoLines[a].destroy();
			// 				} else {
			// 				
			// 					temp.push(self._autoLines[a]);
			// 				}
			// 			}
			// 			var c=0;
			// 			for(a in self._autoLines){
			// 			
			// 				self._autoLines[a]=(temp[c])?temp[c]:null;
			// 				
			// 				if(temp[c]) console.log(a+" is now "+temp[c].uid);
			// 				c++;
			// 			}
