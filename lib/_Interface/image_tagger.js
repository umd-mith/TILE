...
(function($){
	...
	_Itag.prototype={
		...
		createShapeToolBar:function(e){
			...
			var w=$("#"+obj.loc).width();
			var h=$("#"+obj.loc).height()-obj.ShapeToolBar.DOM.height()-20;
		...
		show:function(data){
			...
			for (var i=0;i<data.length;i++){
				...
				if(!title){
				 ...
				 $("<li><a href='"+data[i].url+"'>"+title+"<\/a></li>").appendTo(".az.inner.textImageList > .list");	
			}
	...
	var RaphaelImage=Image.extend({
		...
		setUpCanvas:function(url){
			...
			$("#srcImageForCanvas").load(function() {
				...
				if(self.manifest[src]){
					self.manifest[src].origWidth=pic_real_width;
					self.manifest[src].origHeight=pic_real_height;
				}
			
				$("#srcImageForCanvas").width((pic_real_width*self._imgScale));
				$("#srcImageForCanvas").height((pic_real_height*self._imgScale));
				//make sure container element is height and width of real image
				$(".vd-container").width((pic_real_width*self._imgScale));
				$(".vd-container").height((pic_real_height*self._imgScale));
				
				$("body:first").trigger("newPageLoaded",[src]);
		...
		_loadShapesHandle:function(e,json){
			...
			var diff=0;
			var dscale=0;
			//set images to current scale
			for(p in json){
				
				if((json[p]._scale-self._imgScale)!=0){
					console.log("Scale is different for shape "+json[p].id+" scale: "+json[p]._scale+"  imgScale: "+self._imgScale+"  drawTool scale: "+self.drawTool._scale);
					diff=json[p]._scale-self._imgScale;
					dscale=json[p]._scale;
					if(diff<0){
						console.log("Diff is less than 0");
						//need to scale the shape upwards
						while(diff<=0){
							$.each(json[p].posInfo,function(i,o){
								o*=1.25;
							});
							diff+=1.25;
						}
						...
						console.log("Diff is greater than 0");
						//need to scale the shape down
					
						while(diff>=0){
							var oldX = parseFloat(json[p].attr("x"));
							var oldY = parseFloat(json[p].attr("y"));
							json[p].scale((0.75*json[p]._scale),(0.75*json[p]._scale));
							json[p]._scale=(0.75*json[p]._scale);
							newX = oldX * 0.75;
							newY = oldY * 0.75;
							json[p].attr({
								"x": newX,
			...
			//determine if shapes need to be re-scaled
			// **shapes from JSON object are stored and not updated;
			// so if user zooms out/in, these shapes have to be updated
			
			// if(diff<0){
			// 			self.drawTool._scale=dscale;
			// 			//need to scale it up
			// 			
			// 			while(self.drawTool._scale<self._imageScale){
		...
		zoomHandle:function(e,n){
			...
			if(self.drawTool&&($("#srcImageForCanvas").width()!=0)){
				...
				if(n>0){
					...
					self._imgScale*=1.25;
					console.log("imgscale: "+self._imgScale+"  drawTool scale: "+self.drawTool._scale);
				} else if(n<0){
...
var IT={
	...
	restart:function(json,args){
		...
		if(args&&args.scale){
			//resize the canvas to match the previous tool's canvas
			$("#srcImageForCanvas").width(args.scale*parseFloat($("#srcImageForCanvas").width()));
			$("#srcImageForCanvas").height(args.scale*parseFloat($("#srcImageForCanvas").height()));
			$(".vd-container").width(args.scale*parseFloat($(".vd-container").width()));
			$(".vd-container").height(args.scale*parseFloat($(".vd-container").height()));
			// self.drawTool.scale(1.25); //zooming in
			// 		self._imgScale*=1.25;
			var diff=this.itagger.raphael._imgScale-args.scale;
			if(diff<0){
				//need to scale up
				while(diff<0){
					this.itagger.raphael.drawTool.scale(1.25);
					diff+=1.25;
				}
			} else if(diff>0){
				//need to scale down
				while(diff>0){
					this.itagger.raphael.drawTool.scale(0.75);
					diff-=0.75;
				}
			}
			
			this.itagger.raphael._imgScale=args.scale;
