	...
	**/
	...
	var RaphaelImage=TILEImage.extend({
		...
		_shiftAutoLines:function(id){
			...
			var lastLine=$(".lineBox:last-child");
			var toplim=parseFloat(line.css("top"));
			lastLine.prevAll('.lineBox').each(function(i,o){
				if(parseFloat($(o).css("top"))>toplim){
					console.log("changing: "+$(o).attr('id')+"  to: "+$(o).prev('.lineBox').attr('id'));
					$(o).attr('id',$(o).prev('.lineBox').attr('id'));
				}
			});
			line.remove(); 
			var cauto=self._autoLines[id];
			self._autoLines.length=1;
			
			var curvg=self.drawTool.exportShapes();
			self.drawTool.clearShapes();
			$("body:first").trigger("loadShapes",[curvg.push(self._autoLines[id]._obj)]);
	...
	**/
	...
	var AutoRecLine=Shape.extend({
		...
		resizeData:function(newScale){
			var self=this;
			console.log("from scale: "+self._obj._scale+" to "+newScale);
