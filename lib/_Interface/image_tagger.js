	...
	**/
	...
	var RaphaelImage=TILEImage.extend({
		...
		_shapeActiveHandle:function(e,id){
			if(__v) console.log(e.type+"  was fired");
			
				if(__v) console.log(e.which);
			...
			for(x in self.manifest[url].shapes){
				if(self.manifest[url].shapes[x]&&(self.manifest[url].shapes[x].id==id)){
					...
					$("<div class=\"shpButtonHolder\"><div id=\""+foundID+"\" class=\"button\">Delete</div></div>").appendTo($("body"));
					$(".shpButtonHolder").css("left",$("#selBB").offset().left+'px');
					$(".shpButtonHolder").css("top",$("#selBB").offset().top+'px');
					
					
					if(__v) console.log("attached a delete button for: "+$("#"+foundID)+"  "+foundID);
		...
		_shapeDrawnHandle:function(e,shpObj){
			...
			$("<div class=\"shpButtonHolder\"><div id=\""+shpObj.id+"\" class=\"button\">Delete</div></div>").appendTo($("body"));
			$(".shpButtonHolder").css("left",$("#selBB").offset().left+'px');
			$(".shpButtonHolder").css("top",$("#selBB").offset().top+'px');			
			$("#"+shpObj.id).click(function(e){
				e.stopPropagation();
		...
		_addLinkHandle:function(ref){
			...
			$(".shpButtonHolder").append($("<div class=\"button\">"+ref.id+"</div><span id=\"delete_"+ref.id+"\" class=\"button\">X</span>"));
		...
		bundleData:function(){
			...
			for(url in self.manifest){
				...
				for(sh in self.manifest[url]){
					if(self.manifest[url][sh]){
						cleanManifest[url].push(self.manifest[url][sh]);
					}
				}
...
var IT={
	...
	_shapeDeletedHandle:function(e,data){
		var self=e.data.obj;
		if(__v) console.log("shapeDeleted called in image_tagger wrapper");
