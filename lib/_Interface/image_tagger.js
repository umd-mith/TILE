...
(function($){
	...
	var _Itag=function(args){
		...
		var ingestImages=function(e,json){
			var obj=e.data.obj;
			//send image file data to canvas
			obj.raphael.addUrl(json);
			// obj.schemaFile=file;
	...
	**/
	...
	var RaphaelImage=Image.extend({
			...
			if((url!=null)){
				...
				for(u in url.pages){
					...
					if((cu.length>1)){
						...
						this.manifest[url.pages[u].info.facs]=url.pages[u];
					}
					this.pageNum=0;
				}
			}
		...
		setPageInfo:function(url){
			...
			if((this.curUrl==url)&&(this.manifest[this.url[this.pageNum].info.facs])){
