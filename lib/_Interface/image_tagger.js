	...
	var ImageList=Monomyth.Class.extend({
		...
		show:function(data){
			...
			for (var i=0;i<data.length;i++){
				 $("<li><a href='"+data[i].url+"'>"+data[i].info.facs+"<\/a></li>").appendTo(".az.inner.textImageList > .list");	
			}
		...
		//Takes the imagelist away from the page
		back:function(e){
			var self=e.data.obj;
			$("#"+self.loc+" > .az.inner.textImageList").remove();
			$("#"+self.loc+" > div.az").show();
	...
	**/
	...
	var RaphaelImage=Image.extend({
		...
		_loadShapesHandle:function(e,json){
			...
			//self.drawTool.scale(self._imgScale); //set images to current scale
		},
