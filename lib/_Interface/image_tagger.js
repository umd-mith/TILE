	...
	**/
	...
	var RaphaelImage=TILEImage.extend({
		...
		_shapeActiveHandle:function(e,id){
			...
			for(x in self.manifest[url].shapes){
				if(self.manifest[url].shapes[x].id==id){
					if(console) console.log("shape "+id+" clicked on. found: "+self.manifest[url].shapes[x].id);
					var foundID=id;
					...
					$("<div class=\"shpButtonHolder\"><div id=\""+foundID+"\" class=\"button\">Delete</div></div>").click(function(){
					...
					if(/^D_/.test(foundID)){
						...
						// Approve
						// Approve All
						if(console) console.log("putting approve button for: "+foundID);
						$("<div id=\"approve_"+foundID+"\" class=\"button\">Approve</div>").click(function(e){
							e.stopPropagation();
		...
		_approveItemHandle:function(id){
			...
			$("body:first").trigger("shapeDeleted",[{"id":id,"type":"shapes"}]);
...
var IT={
	...
	_receiveShapeObjHandle:function(e,data){
		var self=e.data.obj;
		if(console) console.log("receiveObj "+data.id);
