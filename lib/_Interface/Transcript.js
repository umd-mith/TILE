...
(function ($, R, _) {
	...
	$.extend(Transcript.prototype, {
		...
		_shapeDrawnHandle:function(e,data){
			...
			// 				if(self.lineArray[self.curLine].shapes[s].id==data.id){
			// 					self.lineArray[self.curLine].shapes[s]=data;
			// 					c=true;
			// 					break;
			// 				}
			// 			}
			// 			if(c==false) self.lineArray[self.curLine].shapes.push(data);
			// 		} else {
		...
		_updateItemHandle:function(e,shapes){
			...
			if(__v) console.log("receiving shapes: "+shapes);
		...
		_lineSelected:function(index){
			...
			if(self.lineArray[index]) {
				if(self.curLine!=index){
					...
					if(self.lineArray[self.curLine].shapes.length>0){
						if(__v) console.log("loading transcript line "+index+" shapes: "+self.lineArray[self.curLine].shapes);
