	...
	$.extend(Transcript.prototype, {
		...
		_setAutoLines:function(){
			...
			if(!self.manifest[self.curUrl].autoLines){
				self.manifest[self.curUrl].autoLines=[];
			}
			...
			for(l in self.lineArray){
				...
				for(sh in self.lineArray[l].shapes){
					if(self.lineArray[l].shapes[sh].id.substring(0,1)=='D'){
						...
						self.manifest[self.curUrl].autoLines.push(self.lineArray[l].shapes[sh]);
			...
			if(self.manifest[self.curUrl].autoLines){
				...
				for(x in self.manifest[self.curUrl].autoLines){
					if(self.manifest[self.curUrl].autoLines[x].id==id){
						...
						self.manifest[self.curUrl].autoLines[x]={};
		...
		_shiftAutoLines:function(id){
			...
			for(x in self.manifest[self.curUrl].autoLines){
				...
				if(self.manifest[self.curUrl].autoLines[x].id==id){
					...
					var bc=self.manifest[self.curUrl].autoLines.slice((x+1));
					self.manifest[self.curUrl].autoLines=ac.concat(bc);
					break;
				}
				// if(check){
				// 					sid=self.manifest[self.curUrl].autoLines[x].id;
				// 					self.manifest[self.curUrl].autoLines[x]=self.manifest[self.curUrl].autoLines[x+1];
				// 					for(sh in self.lineArray[x].shapes){
				// 						if(!self.manifest[self.curUrl].autoLines[x+1]) continue;
				// 						if(self.lineArray[x].shapes[sh].id==sid){
				// 							self.lineArray[x].shapes[sh]=self.manifest[self.curUrl].autoLines[x+1];
				// 							
				// 						}
				// 					}
				// 				}
				...
				for(sh in self.lineArray[l].shapes){
					if(self.lineArray[l].shapes[sh].id.substring(0,1)=="D"){
						if(self.manifest[self.curUrl].autoLines[l]){
							console.log("from: "+self.lineArray[l].shapes[sh].id+"  to : "+self.manifest[self.curUrl].autoLines[l].id);
							self.lineArray[l].shapes[sh]=self.manifest[self.curUrl].autoLines[l];
						} else {
							var ac=self.lineArray[l].shapes.slice(0,sh);
							var bc=self.lineArray[l].shapes.slice(sh+1);
		...
		_lineSelected:function(index){
			...
			if(self.lineArray[index]) {
				...
				if(self.lineArray[self.curLine].shapes.length>0){
					// var shps=$.merge([],self.lineArray[self.curLine].shapes);
					// 						
					// 						if(self.manifest[self.curUrl].autoLines[self.curLine]){
					// 							
					// 							shps.push(self.manifest[self.curUrl].autoLines[self.curLine]);
					// 						}
