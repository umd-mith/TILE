...
(function ($, R, _) {
	...
	$.extend(Transcript.prototype, {
		...
		_addLinkHandle:function(ref,line){
			...
			if(__v) console.log("Transcript addLinkhandle:  id: "+ref.id+" type: "+ref.type);
			// deactivate self
			// $(".line_selected").removeClass("line_selected");
				...
				for(x in self.lineArray[self.curLine][ref.type]){
					...
					if(item&&(item==ref.id)){
						...
						if(self.lineArray[self.curLine][ref.type].length>1){
							...
							if(__v) console.log("erasing: "+ref.id+" now the array's all up in : "+self.lineArray[self.curLine][ref.type]);
						} else {
							self.lineArray[self.curLine][ref.type]=[];
							if(__v) console.log("erasing: "+ref.id+" now the array's all up in : "+self.lineArray[self.curLine][ref.type]);
	...
	var Trans={
		...
		unActive:function(){
			var self=this;
			$("#transcript_toolbar").append("<p>Double Click to Activate Lines Again.</p>");
