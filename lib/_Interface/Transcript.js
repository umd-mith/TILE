...
(function ($, R, _) {
	...
	$.extend(Transcript.prototype, {
		...
		_addLinkHandle:function(ref){
			...
			if(__v) console.log("Transcript addLinkhandle"+ref.id+" "+ref.type);
			// deactivate self
			// $(".line_selected").removeClass("line_selected");
		...
		_addLinkDiv:function(id,ref){
			...
			$("#TILE_Transcript_"+id+" > #delete_"+ref.id).click(function(e){
				...
				$("body:first").trigger("removeTransLink",[{id:ref.id,type:ref.type}]);
				$(this).remove();
				$("#"+ref.id).remove();
			});
		},
		_deleteLinkHandle:function(ref){
			if(__v) console.log("ref: "+ref.id+"  "+ref.type);
			if(ref.type=="lines") return;
			// if(!$(".line_selected").length) return;
			var self=this;
			if(__v) console.log("deleteLinkHandle Transcript is: "+$(".line_selected").attr('id'));
			var id=$(".line_selected").attr('id');
			for(x in self.lineArray){
				if(self.lineArray[x].id==id){
					self.curLine=x;
					break;
				}
			}
			...
			if(self.curLine==null) return;
			if(__v) console.log("lineArray[curline]["+ref.type+"]="+self.lineArray[self.curLine][ref.type]);
			if(self.lineArray[self.curLine][ref.type]){
				for(x in self.lineArray[self.curLine][ref.type]){
					var item=self.lineArray[self.curLine][ref.type][x];
					if(__v) console.log("item: "+item+" ref: "+ref.id);
					if(item&&(item==ref.id)){
	...
	var Trans={
		...
		getLink:function(){
			...
			return {"type":"lines","id":line};
		},
		inputData:function(data){
			var self=this;
			if(!$(".line_selected").length) return false;
			self.transcript._addLinkHandle(data);
			return true;
		},
		removeData:function(data){
			var self=this;
			if(__v) console.log("Transcript removing data: "+data);
			self.transcript._deleteLinkHandle(data);
		},
		restart:function(){
			var self=this;
