...
(function ($, R, _) {
	...
	function Transcript(args){
		...
		if(this.lineArray){
			for(url in this.lineArray){
				...
				self.manifest[url]=self.lineArray[url].lines;
				for(x in self.manifest[url]){
					if(!self.manifest[url][x].id){
						var id="";
						...
						self.manifest[url][x].id=id;
					}
				}
				// if(__v) console.log("lines present in self.manifest["+url+"]: "+self.manifest[url]);
	...
	$.extend(Transcript.prototype, {
		_newPageLoadedHandle:function(e,url){
			var self=e.data.obj;
			self._addLines(self.manifest[url],url);
		},
		...
		_drawText: function(){
			var self=this;
			if(__v) console.log("drawText: "+self.lineArray);
		...
		_addLinkHandle:function(ref,line){
			...
			for(x in self.lineArray){
				if(self.lineArray[x].id==$(".line_selected").id){
					self.curLine=x;
					break;
				}
			}
								...
								if(item&&(item==ref.id)){
									...
									if(self.lineArray[n][ref.type].length>1){
										...
										for(sh in self.lineArray[n][ref.type]){
											if(self.lineArray[n][ref.type][sh]!=ref.id){
					...
					if(item&&(item==ref.id)){
						...
						if(self.lineArray[self.curLine][ref.type].length>1){
							...
							for(sh in self.lineArray[self.curLine][ref.type]){
								if(self.lineArray[self.curLine][ref.type][sh]!=ref.id){
		...
		_lineSelected:function(id){
			...
			if(self.lineArray[index]) {
				if(__v) console.log("lineArray selected: "+index+" and it has: "+self.lineArray[index].shapes);
				//clear all shapes
	...
	var Trans={
		...
		start:function(e,data,layout){
			...
			// set up linkManifest
			for(var url in data){
				self.linkManifest[url]=data[url].lines;
			}
		...
		unActive:function(){
			...
			$("#transcript_toolbar").dblclick(function(e){
		...
		_trnsClickHandle:function(e,id){
			var self=e.data.obj;
			if(__v) console.log("trnsClickHandle called with: "+id);
		...
		getLink:function(){
			...
			return {"type":"lines","id":line,tool:self.id};
		},
		...
		removeData:function(data,line){
			var self=this;
			if(__v) console.log("Transcript removing data: "+data);
		...
		bundleData:function(json){
			var self=this;
			// go through linkManifest and return added values
			for(var url in self.linkManifest){
				var cL=self.linkManifest[url];
				for(var x in cL){
					for(var j in json[url].lines){
						if(json[url].lines[j].id==cL[x].id){
							// add line data to existing manifest data
							for(var ref in cL[x]){
								if(!json[url].lines[j][cL[x][ref].type]) json[url].lines[j][cL[x][ref].type]=[];
								json[url].lines[j][cL[x][ref].type].push(cL[x][ref]);
							}
			...
			json=self.transcript.bundleData(json);
			return json;
		}
	};