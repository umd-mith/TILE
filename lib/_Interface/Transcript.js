	...
	$.extend(Transcript.prototype, {
			...
			for(l in self.lineArray){
				...
				for(sh in self.lineArray[l].shapes){
					if(self.lineArray[l].shapes[sh].id.substring(0,1)=='D'){
						...
						// if(!self.manifest[self.curUrl].autoLines[l]) self.manifest[self.curUrl].autoLines[l]=[];
						self.manifest[self.curUrl].autoLines[l]=self.lineArray[l].shapes[sh];
						// var arl=new AutoRecLine({initialTop:data[d].shapes[sh].posInfo.y,
						// 						initialBottom:data[d].shapes[sh].posInfo.y+data[d].shapes[sh].posInfo.height,foundOnRow:data[d].shapes[sh].order,data:data[d].shapes[sh]});
		...
		_shiftAutoLines:function(id){
			...
			for(x in self.manifest[self.curUrl].autoLines){
				...
				if(check){
					var sid=self.manifest[self.curUrl].autoLines[x].id;
					self.manifest[self.curUrl].autoLines[x]=self.manifest[self.curUrl].autoLines[x+1];
					for(sh in self.lineArray[x].shapes){
						if(!self.manifest[self.curUrl].autoLines[x+1]) break;
						if(self.lineArray[x].shapes[sh].id==sid){
							console.log(sid+'  now: '+self.manifest[self.curUrl].autoLines[x+1].id);
