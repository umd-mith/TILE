			...
			if($.inArray(obj.n,self.showTags)>-1){
				...
				switch(obj.type){
					...
					case 'group':
						...
						if(obj.etc.tags.length>0){
							for(f in obj.etc.tags){
								var id=obj.etc.tags[f];
							
								obj.o.pushItem(self.manifest.tags[id]);
							}
						}
					...
					case 'group':
						if(!self.curItem.o._copy){
							self.curItem.o._copy=new GroupCopy({
								html:"<div id=\"emptygroup\" class=\"group\">"+self.curItem.o.DOM.html()+"</div>",
								uid:self.curItem.uid,
								loc:self.logInsertPoint.attr("id"),
								name:self.curItem.o.name
							});
		...
		//	var copy=item.clone(self.dropInsertArea.attr('id'));
			...
			if(item.type=='group'){
				...
				var copy=new TagCopy({
					loc:self.dropInsertArea.attr("id"),
					name:item.name,
					html:"<div id=\"\" class=\"tag "+item.type+"\">"+item.DOM.html()+"</div>",
					uid:item.uid
				});
			}
