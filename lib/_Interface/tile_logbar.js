	...
	**/
	...
	var TagCopy=LogItem.extend({
		init:function(args){
			...
			//fire first event to the logbar
			self.DOM.click();
		...
		updateCopy:function(e,updates){
			...
			$("#"+this.DOM.attr('id')+" > div.tagcontent > div.tagattr > .nameValue > .button").hide(); //schema-defined tags
		}
	...
	var TileLogShape=LogItem.extend({
		init:function(args){
			...
			self.collapseIcon=$("#"+this.DOM.attr('id')+" > .tagname > a.btnIcon").hide();// click(function(e){
			...
			this.editSelectB=$("#"+this.DOM.attr('id')+" > div.tagname > div.tagmenu > #ddown > div.menuitem.ddown > span").show();
			this.editSelect=$("#"+this.DOM.attr('id')+" > div.tagname > div.tagmenu > #ddown > div.menuitem.ddown > select");
	...
	**/
	var TileTranscriptTag=LogItem.extend({
		init:function(args){
			...
			this._copy=new TagCopy({
				loc:this.copyloc,
				html:"<div id=\"\" class=\"tag transcript\">"+this.DOM.html()+"</div>",
				uid:this.uid,
				name:this.name
			});
		},
	...
	**/
	var TileLogTag=LogItem.extend({
		...
		//	self.shapeId=(args.shapeId)?args.shapeId:null;
			...
			self.DOM=$("#emptytag").attr('id',"tag_"+self.uid).addClass("selected");
		...
		//	self.DOM.bind("toggleTagState",{obj:this},self.toggleTagState);
			...
			//create a copy for the loglist
			self._copy=new TagCopy({
				html:"<div id=\"\" class=\"tag\">"+self.DOM.html()+"</div>",
				uid:self.uid,
				loc:self.copyloc,
				name:self.name
			});
