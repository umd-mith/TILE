	...
	var TILE_ENGINE=Monomyth.Class.extend({
		...
		//$("body").bind("saveAllSettings",{obj:this},this.saveSettings);		
			...
			if(this.json){
				...
				URL_LIST=this.json.pages;
				
				
				
			//	$("body:first").trigger("loadImageList");
			...
			$.ajax({
				...
				success:function(d){
					...
					for(key in obj.json.pages){
						if(!urls[obj.json.pages[key].url]){
							urls[obj.json.pages[key].url]=obj.json.pages[key];
						} else {
							//duplicate url - 
							$.each(urls[obj.json.pages[key].url].info,function(i,o){
							...
							$.merge(urls[obj.json.pages[key].url].lines,obj.json.pages[key].lines);
						}
					}
					...
					$("body:first").trigger("loadImageList",[obj.json]);
				...
				for(tool in obj.toolSet){
					if(obj.toolSet[tool].name==toolname){
						...
						if(!obj.toolSet[tool].constructed){
							...
							obj.toolSet[tool].start(null,self._base,(obj.json)?obj.json:null); //give place where it opens into
		...
		addNewTool:function(e,toolname){
			...
			//replace the main log area with whatever 
			toolname=toolname.toLowerCase().replace(" ","");
			if(toolname==obj.curTool.name) return;
			// //NEW: accessing autorecognizer tools
			// 		//AUTORECOGNIZER
			// 		$("body").bind("ARStart",{obj:obj},function(e){
			// 			//stop the imagetagger from listening
				...
				for(tool in obj.toolSet){
					if(obj.toolSet[tool].name==toolname){
						...
						if(!obj.toolSet[tool].constructed){
							...
							obj.toolSet[tool].start(null,obj.schemaData,obj._log.exportLines()); //give place where it opens into
							obj.toolSet[tool].constructed=true;
							obj.curTool=obj.toolSet[tool];
							$("body").bind(obj.toolSet[tool].done,{obj:obj,tool:obj.toolSet[tool]},obj._toolDoneHandle);
							if(obj.toolSet[tool].outputData) $("body").bind(obj.toolSet[tool].outputData,{obj:obj},obj._toolOutputHandle);
						} else {
							//tool already constructed, restart tool
							obj.curTool=obj.toolSet[tool];
							obj.toolSet[tool].restart(obj._log.exportLines());
						}
						break;
					}
				}
		...
		TRSetHandle:function(e,c){
			...
			$.ajax({
				...
				success:function(d){
					...
					for(tr in d){
						if(d[tr].transcript){
							for(line in d[tr].transcript){
								//create an item for manifest.tags
								if(d[tr].transcript[line].length>0){
									tlines.push({
										'type':'transcript',
										'uri':d[tr].uri,
										'name':(d[tr].transcript[line].substring(0,5)+"..."),
										'uid':'t'+dt.getTime('milliseconds')+tlines.length,
										'etc':{"text":d[tr].transcript[line]}
									});
								}
							}
						}
		...
		_newPageHandle:function(e,url){
			...
			for(t in self.json.pages){
				if(self.json.pages[t].url==url){
					page=self.json.pages[t];
					break;
				}
			}
			...
			if(page){
				...
				self._log._addLines(page.lines,url);
			}
		},
		...
		_saveSettingsHandle:function(e){
			...
			self.json=self.curTool.bundleData(self.json);
			if(!self.save) self.save=new Save({loc:"azglobalmenu"});
			self.json=self._log.bundleData(self.json);
			self.save.userPrompt(self.json);
			// j.schema=self.jsonReader.schemaFile;
			// 			//get elements from plugins
			// 			for(b in self.toolSet){
			// 				j=self.toolSet[b].bundleData(j);
			// 			}
			// 			
			// 			if(!self.save) self.save=new Save({loc:"azglobalmenu"});
...
TILE.LoadTags=LoadTags;
	...
	var ExportDialog=Dialog.extend({
		...
		submitHandle:function(e){
			...
			//create an iframe and attach to azglobalmenu
			//(area of page not really effected)
			var iframe=$("<iframe src=\""+file+"\"></iframe>");
			this.submitB[0].submit();
			$("iframe").remove();
		}
	});
