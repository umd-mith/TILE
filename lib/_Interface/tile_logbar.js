...
(function($){
	...
	//JS for creating drop-down tables
	function _start_Drop_Down(){
		...
		var i = 1;
		while ($("#ddown"+i).length) {
			heights.push($("#ddown"+i).height() + 8);
			$("#ddown"+i).height(startingHeight);
			$("#ddown"+i).mouseover(function () {
				$(this).stop().animate({height:heights[this.id.substr(5)-1]},{queue:false, duration:speed});
			});
			$("#ddown"+i).mouseout(function () {
				$(this).stop().animate({height:startingHeight+'px'}, {queue:false, duration:speed});
			});
			i++;
		}
		$("#ddown ul").css("visibility", "visible");
	}
	...
	**/
	var TileLogBar=Log.extend({
		...
		setUpToolBar:function(){
			...
			self.toolSelect.css({visibility:"visible","z-index":"1000","position":"absolute","left":tx.left}).height(self.startingHeight);
			self.tagSelect.css({visibility:"visible","z-index":"1000","position":"absolute","left":(tx.left+self.toolSelect.width()+8)}).height(self.startingHeight);
		...
		setChoices:function(){
			...
			$.each(c,function(i,o){
				...
				var el=$("<a href=\"\">"+o+"</a>").val(o).appendTo(elwrap);
				el.click(function(e){
					e.preventDefault();
	...
	var TileLogShape=LogItem.extend({
		init:function(args){
			...
			this.shapeIcon=$("#"+this.DOM.attr('id')+" > div.menuitem > span.btnIconLarge.hand");
		...
		**/
		...
		setEdit:function(){
			...
			if(this.editSelect){
				...
				self.editSelect.css({"visibility":"visible","position":"absolute","left":tx.left,"top":tx.top+20}).height(self.startingHeight);
	...
	var TileLogGroup=LogItem.extend({
		init:function(args){
			...
			this.groupNameInput=$("#"+this.DOM.attr("id")+" > div.groupname > a > input").val(this.groupNameSpan.text());
			...
			this.DOM.bind("removeItem",{obj:this},this.removeItem);
		},
		_dropp:function(){
			this.DOM.droppable({
				drop:this.addItem
		...
		addItem:function(e,o){
			...
			if($("#"+$(this).attr('id')+" > #"+(m.attr('id')+"copy")).length){
				return;
			} else {
				//copy all of the tag HTML into DOM
				var n=$("#"+m.attr('id')+"copy"+n).length;
				item=m.clone().css("top","0px").attr('id',m.attr('id')+"copy");
			
				item.appendTo($(this));
				$("#"+item.attr('id')+" > div.tagname > p").text($("#"+item.attr('id')+" > div.tagname > input").val()).show();
				$("#"+item.attr('id')+" > div.tagname > input").hide();
				//if it is a transcript, hide the buttons and textarea
				if($("#"+item.attr('id')+" > div.tagcontent > form").length){
					$("#"+item.attr('id')+" > div.tagcontent > form").hide();
					$("#"+item.attr('id')+" > div.tagcontent > p").text($("#"+item.attr('id')+" > div.tagcontent > form > textarea").val()).show();
				}
				$("#"+item.attr('id')+" > div.tagmenu > div.menuitem > span.btnIconLarge.hand").hide();
				var id=item.attr('id');
				$("#"+item.attr('id')+" > div.tagmenu > #ddown > div.menuitem > ul > li").each(function(i,o){
					$(o).unbind("click");
					$(o).click(function(e){
						e.preventDefault();
		...
		pushItem:function(item){
			...
			if($("#"+this.DOM.attr('id')+" > #"+(item.attr('id')+"copy")).length){
				return;
			} else {
				var n=$("#"+item.attr('id')+"copy").length;
				item.attr("id",item.attr('id')+"copy"+n).css("top","0px");
				...
				$("#"+item.attr('id')+" > div.tagmenu > div.menuitem > span.btnIconLarge.hand").hide();
			//	$("#"+item.attr('id')+" > div.tagmenu > #ddown").hide();
				var id=item.attr('id');
				$("#"+item.attr('id')+" > div.tagmenu > #ddown > div.menuitem > ul > li").each(function(i,o){
					$(o).unbind("click");
					$(o).click(function(e){
	...
	var TileTranscriptTag=LogItem.extend({
		...
		setEdit:function(){
			if(this.menuSelect){
				...
				self.speed=300;
				//make visible
				var tx=self.menuSelect.position();
				self.menuSelect.css({visibility:"visible",zindex:"1000","position":"absolute","left":tx.left}).height(self.startingHeight);
