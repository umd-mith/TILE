			...
			self.groupSelect.children().each(function(i,o){
				if(!$(o).attr('id')=='null'){
					$(o).click(function(e){
						...
						$(this).trigger("itemDropped",[$(this).parent().children("div.tag:visible").attr("id").substring(1),$(this).val()]);
					});
				}
			});
		...
		**/
		_addItemR:function(e,json){
			...
			if(json.items.length){
						...
						switch(item.type){
							case "shape":
								...
								_keepers.push({
									...
									// setTimeout(function(self,n,item,j){
									// 										self.addShape(n,item,'g'+j);
									// 										self.addTranscript(item.transcript,'g'+j);
									// 									},1,self,e,item,j);
								// } else {
								// 									_keepers.push({
								// 										shape:item,
				...
				if(_keepers.length>6){
					//only add the last
					var a=_keepers.slice((_keepers.length-7));
					self._addRecursive(a,0); 
					
					//prompt the user to add elements
					//self.logActiveArea.hide();
					// var _goOn=$("<div class=\"logitem prompt\"><span class=\"menuitem\"><a class=\"button\">Load In Items</a></span></div>").click(function(e){
					// 						e.preventDefault();
					// 						self._addRecursive(_keepers,0);
					// 					});
					// 					self.logInsertPoint.append(_goOn);
		...
		_overFlowHandle:function(){
			...
			for(s in self.manifest.tags){
				if(ok==0){
					next++;
					if(next==self.tagCount){
						ok=1;
					}
				}else{
					self.manifest.tags[s].cur._copy.DOM.appendTo(self.logInsertPoint);
					ok++;
					if(ok==6) break;
		...
		addShape:function(e,shape,args){
			var obj=e.data.obj;
			if(args){
				var gid=args.group;
			...
			if(!obj.manifest.tags[shape.id]){
				...
				if(gid){
					if(!obj.manifest.groups[gid]){
						...
						setTimeout(function(obj,l,gid){
							var loc=(obj.GroupId)?(obj.manifest.groups[obj.GroupId].o.dropInsertArea):obj.logInsertPoint;
							var b=new TileLogGroup({
								loc:loc,
								html:obj.html.taggroup,
								name:"Group"+gid
							});
							obj.manifest.groups[gid]={o:b,tags:[],shapes:[]};
							if(!obj.GroupId) obj.GroupId=gid;
							b.pushItem(l);
						},1,obj,l,gid);
					
					} else {
						obj.manifest.groups[obj.groupId].o.pushItem(l);
					}
				} else
				if(obj.GroupId&&obj.manifest.groups[obj.GroupId]){
				...
				var l=obj.manifest.tags[shape.id];
				...
				if(!obj.manifest.groups[gid]){
					...
					setTimeout(function(obj,l){
						obj.manifest.groups[obj.GroupId].o.pushItem(l);
					},1,obj,l);
				
				}
			}
			if(obj.logInsertPoint.children("div").length>5){
				obj._overFlowHandle();
			}
		},
