...
(function($){
	...
	var TILE_ENGINE=Monomyth.Class.extend({
		...
		setUp:function(d){
			...
			this._log=new TileLogBar({html:d});
	...
	**/
		...
		**/
		_draggableDroppable:function(){
			$("#logbar_list").sortable({
				items:".logitem" //restrict to only tags, transcripts, and shapes
				//handle:'.moveTag'
			});
		},
		...
		addTag:function(){
			...
			if(this.rules){
				var l=new TileTag({
					...
					tagRules:this.rules,
					html:this.logitemhtml
				});
				//create manifest object for tag
				this.manifest.tags.push({
	...
	var TileLogShape=LogItem.extend({
		init:function(args){
			...
			this.shapeName=$("#"+this.DOM.attr('id')+" > p.shapename").attr("id",this.DOM.attr('id')+"_shapeName").hide();
			this.inputTitle=$("#"+this.DOM.attr('id')+" > input").attr("id",this.DOM.attr('id')+"_titleInput").val(this.shapeName.text());
