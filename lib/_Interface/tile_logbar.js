	...
	var TileLogBar=Log.extend({
		...
		**/
		_addItemR:function(e,json){
			...
			if(json.items){
				...
				while(pages>0){
					var el=$("<span class=\"paginateInfo\">"+pages+"</span>").click(function(e){
						$(this).trigger("paginateTo",[parseInt($(this).text(),10)]);
					});
					$("#az_log > .az.inner > #logNav").append(el);
		...
		**/
		...
		_paginate:function(dir){
			...
			//animate 
			self.logInsertPoint.animate({opacity:0.01,background:"red"},200,function(){
				...
				self.logInsertPoint.animate({"opacity":1,"background":"white"},200);
			});
	...
	var TileLogShape=LogItem.extend({
		...
		**/
		...
		_tagUpdate:function(){
			...
			$("#"+obj.uid+"copy").removeClass("selected");
