		...
		**/
		...
		tagSelectedHandle:function(e,tid){
			...
			if(self.manifest.tags[tid]){
				...
				t.act.appendTo(self.logActiveArea);
				self.curItem=t;
			}
		},
		/**
						...
						switch(item.type){
							case "shape":
								...
								if(!item.n){
									...
									if(o.group&&(!self.manifest.tags[o.group])){
										_keepers.push({
											cur:null,
											o:null,
											uid:o.group,
										...
										self.manifest.tags[o.group].etc.push(o);
									}
		...
		**/
		...
		_paginate:function(dir){
			...
			if(dir<0){
				...
				//go forward
				var lastitem=self.showTags[(self.showTags.length-1)];
				self.showTags=[];
				for(x=lastitem;x<(lastitem+self.maxTags);x++){
			...
			var items=$.grep(self.manifest.tags,function(o,n){
				return ($.inArray(o.n,self.showTags)>-1);
			});
			self.logInsertPoint.empty();
			for(it in items){
				self._addItem(items[it]);
				self._createListItem();
			}
			// for(g in self.manifest.tags){
			// 			if($.inArray(self.manifest.tags[g].n,self.showTags)>-1){
			// 				self._addItem(self.manifest.tags[g]);
			// 				self._createListItem();
			// 			}
			// 		}
	...
	**/
	...
	var TileLogGroup=LogItem.extend({
		...
		//	var copy=item.clone(self.dropInsertArea.attr('id'));
			...
			//if($.inArray(item.uid,self.etc.tags)==-1) self.etc["tags"].push(item.uid);
			//self.DOM.droppable("enable");
		},
