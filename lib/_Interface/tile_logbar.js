	...
	var TileLogBar=Log.extend({
		...
		**/
		_addRecursive:function(items,n){
			...
			switch(obj.type){
				...
				case 'shape':
					...
					break;
				case 'transcript':
					self.addTranscript(obj.transcript,{list:true,group:obj.group});
		...
		addTag:function(id){
			...
			self.manifest.tags[logtag.uid]={o:logtag,act:logtag.DOM,cur:logtag._copy.DOM,type:'schema',n:self.tagCount};
			self.tagCount++;
			if(self.logInsertPoint.children("div").length>self.maxTags){
		...
		addShape:function(shape,args){
			...
			if(!obj.manifest.tags[shape.id]){
				...
				obj.manifest.tags[shape.id]={cur:l.DOM,o:l,type:'shape',n:obj.tagCount};
				obj.tagCount++;
				if(gid){
			...
			//if logbar has gone over recommended length, re-format
			if(obj.logInsertPoint.children("div").length>self.maxTags){
				obj._overFlowHandle();
			}
		},
		...
		addGroup:function(args){
			...
			//if logbar has gone over recommended length, re-format
			if(this.logInsertPoint.children("div").length>this.maxTags){
				this._overFlowHandle();
			}
		},
		addTranscript:function(item,gid){
			...
			if(this.rules){
				...
				//add to master manifest
				this.manifest.tags[l.uid]={o:l,act:l.DOM,cur:l._copy.DOM,n:self.tagCount};
				self.tagCount++;
				if(gid){
