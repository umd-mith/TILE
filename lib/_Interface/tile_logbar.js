	...
	**/
	var TileLogBar=Log.extend({
		init:function(args){
			...
			this.logInsertPoint=$("#logbar_list").height((this.DOM.height()-hadjust)).css("overflow","auto");
		...
		**/
		...
		addShape:function(e,shape){
			var obj=e.data.obj;
			//must be a new shape
			if(!obj.manifest.shapes[shape.id]){
				...
				obj.manifest.shapes[shape.id]=l;
				if(obj.GroupId&&obj.manifest.groups[obj.GroupId]){
	...
	var TagCopy=LogItem.extend({
		init:function(args){
			...
			$("#"+this.DOM.attr('id')+" > div.tagname > p").show();
		}
	...
	var TileLogShape=LogItem.extend({
		init:function(args){
			...
			this.name="Shape"+$("div.tag.shape").length;
	...
	**/
	var TileTranscriptTag=LogItem.extend({
		init:function(args){
			...
			this.transName=$("#"+this.DOM.attr('id')+" > div.tagname > p.transname").css("display","inline-block").hide();
