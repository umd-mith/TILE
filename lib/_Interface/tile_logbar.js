			...
			if(self.curItem){
				...
				switch(self.curItem.type){
					...
					case 'group':
						...
						//Remove from emptytags and put in manifest
						self.manifest.tags[self.curItem.uid]=self.curItem;
	...
	**/
	...
	var TagCopy=LogItem.extend({
		init:function(args){
			...
			$("#"+this.DOM.attr('id')+" > div.tagname > p").text((args.name)?args.name:"copy");
			...
			if($("#"+this.DOM.attr('id')+" > div.tagcontent > p")){
				...
				$("#"+this.DOM.attr('id')+" > div.tagcontent > form").remove();
			}
	...
	var TileLogShape=LogItem.extend({
		...
		//	self.setEdit();
			...
			//global bind for when the tag is updated
			//in logbar
			$("body").bind("tagAdded",{obj:self},self._tagAddedHandle);
		...
		**/
		...
		_tagAddedHandle:function(e){
			var obj=e.data.obj;
			obj.shapeName.show().text(obj.inputTitle.val());
			obj.inputTitle.hide();
			obj.DOM.trigger("tagUpdate"+obj.uid,[{".tagname > p.shapename":obj.shapeName.html()}]);
		},
	...
	var TileLogGroup=LogItem.extend({
		...
		//	$("#"+self.loc).height($("#"+self.loc).height()+self.DOM.height());
			...
			//global bind for when user edits this tag in logbar
			$("body").bind("addedTag",{obj:self},self._tagAddedHandle);
		},
		_dropp:function(){
			var self=this;
	...
	var GroupCopy=Monomyth.Class.extend({
		init:function(args){
			...
			self._dropp();
			$("body").bind("tagUpdate",{obj:self},self._tagUpdateHandle);
		},
		_dropp:function(){
			var self=this;
			//also draggable
		...
		_tagUpdateHandle:function(e,args){
			var self=e.data.obj;
			for(a in args){
				$("#"+self.DOM.attr('id')+" > "+a).html(args[a]);
			}
		}
	});
	...
	**/
	var TileTranscriptTag=LogItem.extend({
		init:function(args){
			...
			//global bind for when user clicks 'done'
			$("body").bind("addedTag",{obj:this},this._addedTagHandle);
		},
		_dragg:function(){
			this.DOM.draggable({
