						...
						if(d[tr].transcript){
							for(line in d[tr].transcript){
								...
								tlines.push({
									...
									'uid':dt.getTime('milliseconds')+tlines.length+'t',
				...
				for(tool in obj.toolSet){
					if(obj.toolSet[tool].name==toolname){
						if(!obj.toolSet[tool].constructed){
							...
							obj.toolSet[tool].start("az_log",self._base,(obj.json)?obj.json:null); //give place where it opens into
				...
				for(tool in obj.toolSet){
					if(obj.toolSet[tool].name==toolname){
						if(!obj.toolSet[tool].constructed){
							...
							obj.toolSet[tool].start("az_log",obj.schemaData,obj._log.manifest.tags); //give place where it opens into
							obj.toolSet[tool].constructed=true;
							obj.curTool=obj.toolSet[tool];
							$("body").bind(obj.toolSet[tool].done,{obj:obj,tool:obj.toolSet[tool]},obj._toolDoneHandle);
						} else {
							//tool already constructed, restart tool
							obj.curTool=obj.toolSet[tool];
							obj.toolSet[tool].restart(obj._log.manifest.tags);
						}
						break;
					}
				}
	...
	**/
	var TileLogBar=Log.extend({
		...
		**/
		...
		//objects sent here first when being created in activearea - not yet created
		//activearea object; must check and see if amount of items in loginsert area is not full
		_overFlowHandle:function(obj){
			...
			if((self.logInsertPoint.children("div.tag").length+self.logInsertPoint.children("div.group").length)==self.maxTags){
				...
				if($("#"+self.pageSelectTool.attr('id')+" > .pageChoice").length>0){
					var ops=parseInt($("#"+self.pageSelectTool.attr('id')+" > option:last-child").text(),10);
					var el=$("<option class=\"pageChoice\">"+(ops+1)+"</option>");
					self.pageSelectTool.append(el);
				} else {
					
					var el=$("<option class=\"pageChoice\">1</option>");// .click(function(e){
					// 						$(this).trigger("paginateTo",[parseInt($(this).text(),10)]);
					// 					});
					self.pageSelectTool.append(el);
					var el=$("<option class=\"pageChoice\">2</option>");// .click(function(e){
					// 						$(this).trigger("paginateTo",[parseInt($(this).text(),10)]);
					// 					});
					self.pageSelectTool.append(el);
				}
		...
		_paginate:function(dir){
			...
			// 					if(parseInt(self.manifest.tags[it].n,10)==i){
			// 						self.showTags.push(parseInt(self.manifest.tags[it].n,10));
