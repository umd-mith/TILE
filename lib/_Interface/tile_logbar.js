	...
	**/
	var TileLogBar=Log.extend({
		init:function(args){
			...
			var hadjust=(this.toolbar.height()+((this.toolbar.css("border-width")+this.toolbar.css("padding"))*2)+2);
		...
		addTag:function(id){
			...
			self.manifest.tags[logtag.uid]=logtag;
		},
		/**
		...
		**/
		_addItemR:function(e,json){
			...
			if(json.items){
				//see if these belong to a group
				if(json.group){
		...
		addTranscript:function(item){
			if(this.rules){
				var l=new TileTranscriptTag({
					html:this.html.tagtranscript,
					loc:this.logActiveArea.attr("id"),
					copyloc:this.logInsertPoint.attr("id")
				});
				//add to master manifest
	...
	var TagCopy=LogItem.extend({
		init:function(args){
			...
			$("#"+this.DOM.attr('id')+" > div.tagmenu > #ddown").remove();
			$("#"+this.DOM.attr('id')+" > div.tagname > input").remove();
			$("#"+this.DOM.attr('id')+" > div.tagname > p").text(args.name).show();
			//in case this is a transcript tag
			$("#"+this.DOM.attr('id')+" > div.tagcontent > p").show();
			...
			//universal items
			self.collapseIcon=$("#"+self.DOM.attr('id')+" > div.tagname > a.open.btnIcon");
	...
	var TileLogShape=LogItem.extend({
		init:function(args){
			...
			self.collapseIcon=$("#"+this.DOM.attr('id')+" > .tagname > a.btnIcon").click(function(e){
			...
			this.shapeIcon=$("#"+this.DOM.attr('id')+" > div.tagname > div.tagmenu > div.menuitem > span.btnIconLarge.poly");
			//configure the name for this log item
			this.name="Shape"+$("#"+self.loc+" > .tag.shape").length;
			
			this.shapeName=$("#"+this.DOM.attr('id')+" > div.tagname > p.shapename").attr("id",this.DOM.attr('id')+"_shapeName");
			this.shapeName.text(this.name);
			this.shapeName.css("display","inline-block");
			...
			this.editSelectB=$("#"+this.DOM.attr('id')+" > div.tagmenu > #ddown > div.menuitem.ddown > span").show();
			this.editSelect=$("#"+this.DOM.attr('id')+" > div.tagmenu > #ddown > div.menuitem.ddown > select");
			...
			//create copy to be put below active bar
			this._copy=new TagCopy({
				html:"<div id=\"\" class=\"tag shape\">"+this.DOM.html()+"</div>",
				loc:this.copyloc,
				uid:this.shapeId,
				name:this.name
			});
		...
		**/
		...
		setEdit:function(){
			if(this.editSelect){
				...
				// 					$(this).stop().animate({height:self.startingHeight+'px'}, {queue:false, duration:self.speed});
				// 				});
				//set up the trigger for showing the select element
				// self.DOM.bind("showMenuSelect",)
				self.editSelectB.click(function(e){
					$(this).hide();
					$(this).parent().children("select").mousedown();
				});
	...
	**/
	var TileTranscriptTag=LogItem.extend({
		init:function(args){
			...
			//active element - just added
			$(".tag").removeClass("selected");
			self.DOM.addClass("selected");
			
			//get elements
			self.collapseIcon=$("#"+self.DOM.attr('id')+" > div.tagname > a.btnIcon").click(function(e){
				if($(this).hasClass("open")){
					$(this).removeClass("open").addClass("closed");
			...
			this.menuSelectB=$("#"+this.DOM.attr('id')+" > div.tagmenu > #ddown > div.menuitem.ddown > span").show();
			this.menuSelect=$("#"+this.DOM.attr('id')+" > div.tagmenu > #ddown > div.menuitem.ddown > select").attr("id","ddown_"+this.uid);
			...
			this._dragg();
			//create copy to be put in logbar list
			this._copy=new TagCopy({
				loc:this.copyloc,
				html:this.html,
				uid:this.uid,
