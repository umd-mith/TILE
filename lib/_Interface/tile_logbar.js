		...
		**/
		...
		_createListItem:function(){
			...
			if(self.curItem){
				...
				if((check==false)&&(self.curItem.type!='shape')){
					...
					if(self.curItem.type=='shape') self.curItem.o.DOM.remove();
					self.curItem=null;
				}
			}
		...
		**/
		...
		_paginateToHandle:function(e,p){
			...
			for(i in self.manifest.tags){
				...
				if((item.n>mt)&&(item.n<t)){
					self.showTags.push(item.n);
					self._addItem(item);
					self._createListItem();
				}
			}
		...
		_paginate:function(dir){
			...
			//animate 
			self.logInsertPoint.animate({opacity:0.01},200,function(){
