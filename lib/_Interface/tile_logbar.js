		...
		**/
		...
		addShape:function(shape,args){
			...
			if(!obj.manifest.tags["s"+shape.id]){
				...
				// 			});
				l={
					uid:"s"+shape.id,name:(args&args.name)?args.name:("shape_"+shape.id),html:obj.html.tagshape,cur:null,o:null,act:null,etc:(obj.etc||null),type:'shape',shape:shape,n:obj.tagCount
				};
				...
				if(l.etc&&l.etc.groups){
					for(g in l.etc.groups){
						...
						if(!self.manifest.tags[guid]){
							...
							self.manifest.tags[guid].etc.push(l.uid);
						}
					}
		...
		addTranscript:function(item,args){
			...
			if(this.rules){
				...
				// 				}
				//add item or correct loglist space
				self._overFlowHandle(self.manifest.tags[l.uid]);
			}
	...
	**/
	var TileTranscriptTag=LogItem.extend({
		init:function(args){
			...
			//get elements
			self.collapseIcon=$("#"+self.DOM.attr('id')+" > div.tagname > a.btnIcon").hide();
			...
			$("body").bind("addedTag",{obj:this},this.addedTagHandle);
		},
		_dragg:function(){
			this.DOM.draggable({
