	...
	**/
	...
	var TileLogShape=LogItem.extend({
		init:function(args){
			...
			//global
			$("body").bind(this.shape.shapeChangeCall,{obj:this},this.shapeChangeHandle);
			//local
		...
		**/
		...
		shapeChangeHandle:function(e,_newShape){
			...
			obj.shapeSpan.empty();
		//	var libag=[];
			$.map(["scale","width","height","x","y","rx","ry","rw","rh"],function(v){
				if(v in self.shape){
			...
			//go through all copies of this particular shape log and update them
			// var i=0;
			// 			while($("#"+self.DOM.attr('id')+"copy"+i).attr('id')){
			// 				$("#"+self.DOM.attr('id')+"copy"+i+" > div.tagcontent > ul").empty();
			// 				for(li in libag){
			// 					$("#"+self.DOM.attr('id')+"copy"+i+" > div.tagcontent > ul").append(libag[li]);
			// 				}
			// 				i++;
			// 			}
	...
	var TileLogGroup=LogItem.extend({
		init:function(args){
			...
			this.groupNameSpan=$("#"+this.DOM.attr("id")+" > div.groupname > span").hide().click(function(e){
				$(this).hide();
				$(this).parent().children("input").show();
			});
			// this.groupNameSpan.hide();
			//input for making group name
			this.groupNameInput=$("#"+this.DOM.attr("id")+" > div.groupname > input").blur(function(e){
				$(this).hide();
			...
			this.groupNameInput.val(this.groupNameSpan.text());
			this._dropp();
		...
		addItem:function(e,o){
			...
			if($("#"+$(this).attr('id')+" > #"+(m.attr('id')+"copy")).length){
				...
				// 				var id=item.attr('id');
				// 				var startH=21;
				// 				$("#"+item.attr('id')+" > div.tagmenu > #ddown > div.menuitem > ul").mouseover(function(e){
				// 					$(this).height(78);
				// 				});
				// 				$("#"+item.attr('id')+" > div.tagmenu > #ddown > div.menuitem > ul").mouseout(function(e){
				// 					$(this).height(startH);
				// 				});
		...
		pushItem:function(item){
			...
			if($("#"+this.DOM.attr('id')+" > #"+(item.DOM.attr('id')+"copy")).length){
				...
				var copy=item.clone(this.DOM.attr('id'));
