...
var TileTabBar=TabBar.extend({
	...
	addTag:function(e){
		...
		if(obj.imageLoaded){
			...
			// 			}
			var tag=new TileTag({loc:obj.taggingArea.attr("id")});
			obj.docData[obj.currPage].tags.push(tag);
		}
	},
	setOpenTag:function(e,id){
		var obj=e.data.obj;
		if(obj.docData[obj.currPage]&&obj.docData[obj.currPage].tags){
			for(p in obj.docData[obj.currPage].tags){
				if(id!=obj.docData[obj.currPage].tags[p].index){
					obj.docData[obj.currPage].tags[p].closeTagState();
				}
			}
		}
	},
