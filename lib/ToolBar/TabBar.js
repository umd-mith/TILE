...
var TileTabBar=TabBar.extend({
	...
	loadJSONTags:function(data,curUrl){
		...
		for(j in data){
			if(data[j].uri.length>1){
				...
				for(t in data[j].tags){
					...
					var cd=new TileTag({loc:this.tagHolder.attr("id"),htmlindex:tg.uid,title:tg.name,values:tg.values,tagRules:this.tagRules,shapeId:tg.shapeId});
		...
		this.curUrl=curUrl;//alert("curUrl: "+this.curUrl+" manifest: "+this.manifest[this.curUrl].length+" manifest2: "+this.manifest[this.curUrl.replace(/^[ \t\r\n]+|[ \t\r\n]+$/,"")].length);
		this.tags=[];
