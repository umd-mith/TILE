...
var TILEShapeToolBar=SideToolBar.extend({
	...
	customWidgetClick:function(e){
		...
		if(obj.widt){
			...
			if((x<(obj.colorWidgetB.position().left+obj.colorWidgetB.width()))&&(x>obj.colorWidgetB.position().left)&&(y>obj.colorWidgetB.position().top)&&(y<(obj.colorWidgetB.position().top+obj.colorWidgetB.height()))){
	...
	onCPSubmit:function(hsb,hex,rgb){
		...
		$(this).trigger("shapeColorSubmit",[hex]);
	},
