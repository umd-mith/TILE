...
var TileNameValue=NameValue.extend({
	init:function(args){
		...
		this.DOM=$("<select></select>").attr("id",this.htmlId).addClass("nameValue").appendTo(this.loc);
		this.valueEl=null;
		this.items=[];
	...
	setUpTopLevel:function(){
		for(t in this.rules.topLevelTags){
			...
			if(T){
				...
				var o=$("<option></option>").attr("id",id).val(uid).text(T.name).appendTo(this.DOM);
				o.click(function(){
		...
		// 			$(this).trigger("nameSelect",[$(this).val()]);
		// 		});
		 this.DOM.bind("nameSelect",{obj:this},this.nameSelectHandle);
	},
		...
		if(T){
			...
			switch(T.valueType){
				...
				case 1:
					...
					obj.loc.append(obj.valueEl);
					break;
				case 2: 
				//reference to a url or other tag
					obj.valueEl=$("<input type=\"text\"></input>").attr("id",function(){
						return $(".nameValue").length+"_input";
					}).val("").appendTo(obj.loc);
