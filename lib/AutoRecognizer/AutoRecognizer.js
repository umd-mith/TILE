...
var CanvasAutoRecognizer=AutoRecognizer.extend({
	...
	thresholdConversion:function(imageEl,bg){
		if(this.Region){
			...
			if(!this.regionData){
				...
				var nw = (imageEl.width*(1000*Math.pow(2,zoom)))/imageEl.height;
				var	nh = (imageEl.height*(1000*Math.pow(2,zoom)))/imageEl.width;
	...
	zoom:function(e,val){
		var obj=e.data.obj;
		var zoom=obj.canvasImage.getZoomLevel();
		if(obj.Region){
			if((val>0)&&(zoom<5)){
				obj.Region.ox*=2;
				obj.Region.oy*=2;
