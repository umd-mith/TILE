			...
			for(s=0;s<this.shapes.length;s++){
				if(this.shapes[s]){
					...
					if(jQuery.inArray(shape.index,this.lineBreaks)>0){
						...
						var top=(this.Region.oy+shape.Bottom+this.canvas.position().top);
						var height=10;
						//create blob box
						bb=new BlobBox({
							width:this.Region.w,
	...
	createLineBreaks:function(){
		...
		if (this.shapes.length > 3) {
			for (var n = 1; n < this.shapes.length; n++) {
				if ((this.shapes[(n-1)])&&(this.shapes[n].compare(this.shapes[(n-1)]))){
					if((lineBreaks.length<=1)||((this.shapes[n].Top-this.shapes[lineBreaks[lineBreaks.length-1]].Bottom)>10)){	
