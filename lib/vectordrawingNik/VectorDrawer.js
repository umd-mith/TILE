...
//NOTE: see changeINIT_ATTRColor()
     ...
     jQuery.extend(VectorDrawer.prototype, {
		...
		setState:function(e,state){
			...
			if(state){
				...
				//obj._uninstallHandlers();
				obj._canvas.elm.hide();
				
				//obj._over.elm.hide();
				obj._over.elm.css({width:200,height:200});
				// obj._over.elm.hide();
				//obj._buildCanvas(0,0);
			} else {
				//colorpicker is closed - return canvas to normal size
				obj._canvas.elm.show();
				obj._over.elm.css({width: (obj._over.startWidth*obj._scale),height: (obj._over.startHeight*obj._scale)});
				//obj._installOneShotHandlers();
			}
		},
	...
	//At mouseup, that's when the user has finished drawing a shape (Click and Drag and Done)
                 ...
                 if (!self._obj) return;
				 if(!self._start) return; //some firebug errors resulting from 'self._start' null
                 e.preventDefault();
