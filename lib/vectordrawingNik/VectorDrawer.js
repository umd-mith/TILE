...
//NOTE: see changeINIT_ATTRColor()
	...
	function changeINIT_ATTRColor(e,hex){
     ...
     var VectorDrawer = function (initDrawMode, initScale, initObjs, startsize, overElm) {
		...
		$("body").bind("colorPickerIsToggled",{obj:this},self.setState);
         self._over = {
             elm: $(overElm),
...
//triggered whenever the ColorPicker is opened  
setState:function(e,state){
	var obj=e.data.obj;
	if(state){
		//colorpicker is open
		//set canvas to 0,0
		// obj._over.elm.remove();
		// 	obj._buildCanvas(0,0);
		obj._paper.attr({width:0,height:0});
	} else {
		//colorpicker is closed - return canvas to normal size
		obj._over.elm.remove();
		obj._buildCanvas();
	}
		...
		loadMasterObjList:function(list){
			...
			for(l in list){
				...
				//alert("shapes length: "+list[l].uri+":  "+list[l].shapes.length);
				for(sh in list[l]){
					var o=list[l][sh];
					if(!this._manifest[o.uri]) this._manifest[o.uri]=[];
					var args=null;
