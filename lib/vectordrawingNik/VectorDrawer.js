...
//NOTE: see changeINIT_ATTRColor()
	...
	function changeINIT_ATTRColor(e,hex){
     ...
     var VectorDrawer = function (initDrawMode, initScale, initObjs, startsize, overElm) {
		 ...
		 $("body").bind("VD_REMOVESHAPE",{obj:self},self.destroySingleShape);
     ...
     jQuery.extend(VectorDrawer.prototype, {
		...
		//NEW: can load objects on the fly
		loadObjs:function(objs){
			var self=this;
			jQuery.merge(self._allObjs,objs);
			self._canvas.elm.remove();
			self._buildCanvas(null,null);
			self.scale(self._scale);
		//	self._canvas.elm.css({width:self._over.origWidth,height:self._over.origHeight});
		},
		...
		changeOverElm:function(args){
			...
			if(args.startsize){
				self._over.origWidth=args.startsize[0]*self._scale;
				self._over.origHeight=args.startsize[1]*self._scale;
				self._over.startWidth=args.startsize[0];
				self._over.startHeight=args.startsize[1];
					...
					*@activate:
					 ...
					 **/
                     var metaObj = {
						...
						destroy:function(){
							this.cur.trigger("VD_REMOVESHAPE",[this.id]);
							this.cur.remove();
					...
					var dRand=new Date();
                     ...
                     var o = self._obj;
					 if (o && o.newO) {
						self._canvas.elm.trigger(o.shapeChangeCall,[{scale:o.newO.scale,x:o.newO.x,y:o.newO.y,width:o.newO.width,height:o.newO.height,cx:o.newO.cx,cy:o.newO.cy,rx:o.newO.rx,ry:o.newO.ry}]);
