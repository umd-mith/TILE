     ...
     jQuery.extend(VectorDrawer.prototype, {
		...
		//NEW: controls the zoom level of the current image object
		 ...
		 displayObjControl:function(e,mode,id){
			...
			//ra: releaseAnchor
			//get Shape
			var S=_.grep(self._allObjs,function(i,val){
				return (val.id==id);
			});
			S=S[0];
			if(S){
		...
		loadMasterObjList:function(list){
			...
			for(sh in list.shapes){
				var o=list.shapes[sh];
				$.extend(o,{
					cur:null,
					shapeChangeCall:"shapeChangeCall_"+o.id,
					anchor:false,
					_display:true,
					deactivate:function(){
						this.hide();
				...
				this._manifest[list.uri].push(o);
			}alert(this._manifest)
			this._buildCanvas(null,null);
			//each object has ID - need it to create shapeChangeCall
			// this._manifest = _.map(list.Shapes,function(o,i){
			// 				$.extend(o,{
			// 					cur:null,
			// 					shapeChangeCall:"shapeChangeCall_"+o.id,
			// 					anchor:false,
