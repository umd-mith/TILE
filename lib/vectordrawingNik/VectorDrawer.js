...
//NOTE: see changeINIT_ATTRColor()
	...
	//NEW: function for changing the stroke color of a shape
	//sets INIT_ATTRS to have a different stroke-color value
	//BIND declared above
	function changeINIT_ATTRColor(e,hex){
		
		INIT_ATTRS.stroke="#"+hex;
	}	
      ...
      *   Can be a DOM element, string, etc. Defaults to "img" (the first image in the page)
		...
		NOTE: SOME Parts of this object were changed from their original state - 
		added
		startsize - {Array} Initial width & height values to start the SVG width (totals max width and height of image)

      */
     var VectorDrawer = function (initDrawMode, initScale, initObjs, startsize, overElm) {
         var self = this;
		 ...
		 self._manifest=[];
         self._start = self._obj = self._points = self._shapeData = null;
		...
		$("body").bind("shapeColorSubmit",changeINIT_ATTRColor);
         self._over = {
             elm: $(overElm),
     ...
     jQuery.extend(VectorDrawer.prototype, {
		...
		//NEW: controls the zoom level of the current image object
         scale: function(newScale) {
			...
			if(newSize.height>$("#scroller").position().top) newSize.height=(newSize.height-($("#scroller").position().top+self._over.elm.offset().top));
		...
		loadMasterObjList:function(list){
			// list format: {uri}:{Object}
			this._manifest = _.map(list,function(o,i){
				$.extend(o,{
					cur:null,
					anchor:false,
					_display:true,
		...
		sendSingleShape:function(obj){
			...
			//send the object
			// $.extend(obj,{
			// 			initData:{scale:obj.scale,x:obj.cur.x,y:obj.cur.y,width:obj.cur.width,height:obj.cur.height,points:obj.cur.points,cx:obj.cur.cx,cy:obj.cur.cy,rx:obj.cur.rx,ry:obj.cur.ry}
			// 		});
		...
		changeOverElm:function(args){
			...
			//resize
			if(args.startsize){
			//		alert(self._over.elm.attr('src')+" is width: "+self._over.elm[0].width+", "+self._over.elm[0].height);
			...
			//self._over.elm.css({width:self._over.startWidth,height:self._over.startHeight});
			self.scale(self._scale);
			//self._buildCanvas(self._over.origWidth,self._over.origHeight);
         ...
         _buildCanvas: function(w,h) {
			...
			if((w==null)||(h==null)){
         		self._paper = R(self._over.offset.left, self._over.offset.top,
		                 self._over.origWidth, self._over.origHeight);
			...
			if(self._manifest[self._over.elm.attr("src")]){
				//alert('manifest: '+self._manifest[self._over.elm.attr("src")]);
					...
					*@activate:
					...
					var dRand=new Date();
                     if (self._drawMode == 'r') {
						 ...
						 var id=dRand.getTime()+"_"+self._allObjs.length;
                         $.extend(metaObj, {
							 id:id,
							 shapeChangeCall:"shapeCoordUpdate"+id,
                             con: "rect",
                     ...
                     var o = self._obj;
					 if (o && o.newO) {
						self._canvas.elm.trigger(o.shapeChangeCall,[{scale:o.newO.scale,x:o.newO.x,y:o.newO.y,width:o.newO.width,height:o.newO.height,points:o.newO.points,cx:o.newO.cx,cy:o.newO.cy,rx:o.newO.rx,ry:o.newO.ry}]);
