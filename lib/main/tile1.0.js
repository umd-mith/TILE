	...
	**/
	...
	TILE_ENGINE.prototype={
		...
		changePage:function(val){
			...
			if(__v) console.log("Curpage is now: "+curPage);
	...
	PluginController.prototype={
		...
		_setActiveTool:function(_newActiveObj){
			var self=e.data.obj;
			$(".ui-dialog").hide();
		...
		parseLink:function(obj1,obj2){
			...
			if((obj1!=null)&&(obj2!=null)){
				...
				if(/http\:|\.[jgpt]*/i.test(obj1.jsonName)){
					...
					if(found<0){
						json.pages[page][obj1.type].push(deepcopy(obj1));
					...
					if($.inArray(obj2.id,json.pages[page][obj1.type][found][obj2.type])){
						...
						$.merge(json.pages[page][url][obj1.type][found][obj2.type],[obj2.id]);
				...
				if(/http\:|\.[jgpt]*/i.test(obj2.jsonName)){
					...
					// storing as an object first, then storing the link
					if(found<0){
						json.pages[page][obj2.type].push(obj2.obj);
	...
	TileToolBar.prototype={
		...
		addButton:function(button){
			...
			var html='<div class="menuitem"><ul><li><a id="'+button.id+'" href="" class="button" title="'+button.helptext+'">'+button.display+'</a></li></ul></div>'+
			...
			for(var b in button){
				...
				if($.inArray(b,callbacks)>=0){
					// attach callback
					$("#"+button.id).live(b,eval(button[b]));
				}
			}
