...
(function($){
	...
	$.extend(FloatingDiv.prototype, {
		...
		setInputObject:function(o,refs){
			...
			if(refs){
				...
				for(var r in refs){
					for(var x in self._labels){
						if(__v) console.log("labels x: "+x);
						if(self._labels[x].id==refs[r].id){
							if(__v) console.log("self._labels["+x+"]: "+JSON.stringify(self._labels[x]));
					...
					if(!sh){
						...
						self._labels[(refs[r].display)?refs[r].display:refs[r].id]=refs[r];
						if(!self._labels[(refs[r].display)?refs[r].display:refs[r].id].refs) self._labels[(refs[r].display)?refs[r].display:refs[r].id].refs=[];
						self._labels[(refs[r].display)?refs[r].display:refs[r].id].refs=[self._curLink.id];
					}
				}
			}
			...
			for(var x in self._labels){
				...
				if($.inArray(self._curLink.id,self._labels[x].refs)>=0){
					html+="<div id=\""+self._labels[x].id+"\" class=\"labelItem\">"+x+"<span id=\"del_"+self._labels[x].id+"\" class=\"button shape delete formLink\">X</span></div>";
		...
		deleteLinkHandle:function(id){
			...
			if(__v) console.log("deleting lb from FloatingDiv: "+JSON.stringify(lb));
	...
	**/
	...
	// var NewImageDialog=function(args){
	// 		// Constructor: Same as Dialogx
	// 			this.$super(args);
	// 
	// 			// this.loc.append($.ajax({
	// 			// 				async:false,
	...
	TILE_ENGINE.prototype={
		...
		_parseOutJSON:function(){
			...
			for(var key in self.json.pages){
				...
				// if(__v) console.log("manifest url "+self.json.pages[key].url+"  :: ");
				// if(__v) console.log(JSON.stringify(self.manifest[self.json.pages[key].url]));
			...
			// if(__v) console.log("JSON is parsed. Sending to transcript bar and PC");
			// 		if(__v) console.log(JSON.stringify(self.manifest));
			//send to toolbar
	...
	PluginController.prototype={
		...
		_toolOutputHandle:function(e,data){
			if(__v) console.log("tooloutputreached "+e.type+"  "+JSON.stringify(data));
			var self=e.data.obj;
			...
			if(self.activeTool.inputData){
				if(__v) console.log("inputting data into "+self.activeTool.id+". data: "+JSON.stringify(data));
		...
		_attachFloatDiv:function(id,link,refs){
			...
			self.floatDiv.setInputObject(link,(refs)?refs:null);
		},
		...
		_deleteLinkHandle:function(e,ref){
			var self=e.data.obj;
			if(__v) console.log("deleteLinkHandle ref: "+JSON.stringify(ref));
