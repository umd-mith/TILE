...
(function($){
	...
	**/
	...
	TILE_ENGINE.prototype={
		...
		// add content object to a set of plugins (mode)
		insertModePlugin:function(cObj){
			var self=this;
		},
	...
	Mode.prototype={
			...
			switch(section){
				case 'contentarea':
					if(area=='toolbar'){
						...
						// to current shell
						if($("#azcontentarea > .az.inner."+self.name).length){
							// append to the shell
							$("#azcontentarea > .az.inner."+self.name).append(html);
						
						
						} else {
							// create outer shell
							var shell='<div class="az inner '+self.name+'">'+html+'</div>';
				...
				case 'log':
					...
					// if shell already attached - append
					// to current shell
					if($("#az_log > .az.inner."+self.name).length){
						// append to the shell
						$("#az_log > .az.inner."+self.name).append(html);
				...
				case 'active':
					...
					// if shell already attached - append
					// to current shell
					if($("#az_activeBox > .az.inner."+self.name).length){
						// append to the shell
						$("#azcontentarea > .az.inner."+self.name).append(html);
		...
		// returns the button 
		appendButtonHTML:function(button){
			var self=this;
			
			
		},
		appendPluginHTML:function(plugin){
			var self=this;
			if(!plugin) return;
		...
		setActive:function(){
			...
			// show ours
			$("#.az.main.twocol > div > .az.inner."+self.name).show();
