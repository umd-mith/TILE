	...
	$.extend(FloatingDiv.prototype, {
		...
		insertNewLabels:function(lbls){
			...
			for(var x in lbls){
				...
				var label=lbls[x].obj;
				
				if(!label.name){
					label.name=(label.display)?label.display:label.id;	
				} 
				autoCompleteLbls.push(label.name);
				var f=null;
				...
				if(__v) console.log('insering '+JSON.stringify(label)+' into _labels');
				self._labels[label.name]=label;
				
				
			}
			
			if(__v) console.log("_labels in insertNewLabels: "+self._labels+'   '+autoCompleteLbls);
	...
	**/
	...
	TILE_ENGINE.prototype={
		...
		setActiveObj:function(obj){
			...
			// find the activeObjs references
			// in json
			// if(obj.type!=obj.jsonName){
			// 				// at page level
			// 				var page=null;
			// 				for(var prop in json.pages){
			// 					if(json.pages[prop].url==TILEPAGE){
			// 						page=json.pages[prop];
			// 						break;
			// 					}
			// 				}
			// 				for(var prop in page[obj.type]){
			// 					if(!(prop in obj.obj)){
			// 						// unique ref array - add to refs
			// 						
			// 						// find in json
			// 						if(page[prop]){
			// 							// page level
			// 							for(var x in page[prop]){
			// 								// if in the obj's array of this type
			// 								// then add to refs array
			// 								if($.inArray(page[prop][x].id,page[obj.type][prop])>=0){
			// 									refs.push(deepcopy(page[prop][x]));
			// 								}
			// 							}
			// 						} else if(json[prop]){
			...
			// 								// if in the obj's array of this type
			// 								// then add to refs array
			// 								if($.inArray(json[prop][x].id,page[obj.type][prop])>=0){
			// 									refs.push(deepcopy(json[prop][x]));
			// 								}
			// 							}
			// 						}
			// 					}
			...
			// 								// if in the obj's array of this type
			// 								// then add to refs array
			// 								if($.inArray(page[prop][x].id,json[obj.type][prop])>=0){
			// 									refs.push(deepcopy(page[prop][x]));
			// 								}
			// 							}
			// 						} else if(json[prop]){
			...
			// 								// if in the obj's array of this type
			// 								// then add to refs array
			// 								if($.inArray(json[prop][x].id,json[obj.type][prop])>=0){
			// 									refs.push(deepcopy(json[prop][x]));
			// 								}
			// 							}
			// 						}
	...
	PluginController.prototype={
		...
		_attachFloatDiv:function(data,handle){
			...
			self.floatDiv.setInputObject(data,self.refArray[data.id]);
		},
			...
			if((obj1!=null)&&(obj2!=null)){
				...
				if(/http\:|\.[jgpt]*/i.test(obj1.jsonName)){
					...
					if($.inArray(obj2.id,json.pages[page][obj1.type][found][obj2.type])){
						...
						self.refArray[obj1.id].push(obj2);
				...
				if(/http\:|\.[jgpt]*/i.test(obj2.jsonName)){
					...
					if($.inArray(obj2.id,json.pages[page][obj2.type][found][obj1.type])){
						...
						self.refArray[obj2.id].push(obj1);
						if(__v) console.log('self.refArray: '+JSON.stringify(self.refArray[obj1.id]));
					}	
				} else {
