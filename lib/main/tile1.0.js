...
(function($){
	...
	**/
	...
	$.extend(FloatingDiv.prototype, {
		...
		setInputObject:function(o,refs){
			...
			// store refs in here
			var lbls=[];
			if(__v) console.log("refs for curlink: "+JSON.stringify(refs));
			...
			if(__v) console.log("labels creates lbls to feed into list: "+JSON.stringify(lbls));
	...
	**/
	...
	var Mode=function(name,active,unactive){
		...
		// attach a listener for when another mode is active
		$(".globalbuttons > .modeitems > .menuitem > a").not("#"+self.button.elem.attr('id')).live('mousedown',function(e){
			self.setUnActive();
		});
	};
	Mode.prototype={
