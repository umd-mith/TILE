		...
		setInputObject:function(o,refs){
			...
			if(refs){
				...
				for(var r in refs){
					...
					if(!sh){
						var key=(refs[r].display)?refs[r].display:refs[r].id;
			...
			for(var a in self._labels){
				...
				if($.inArray(self._curLink.id,self._labels[a].refs)>=0){
					html+="<div id=\""+self._labels[a].id+"\" class=\"labelItem\">"+a+"<span id=\"del_"+self._labels[a].id+"\" class=\"button shape delete formLink\">X</span></div>";
	...
	**/
	...
	TILE_ENGINE.prototype={
		...
		_parseOutJSON:function(){
			...
			for(var key in self.json.pages){
				// if(__v) console.log("json page url: "+self.json.pages[key].url);
				self.manifest[self.json.pages[key].url]=self.json.pages[key];
				
			}
			var randString="o ka y w e m ade it u p bu t th at si ng le so rt of wo r d";
			...
			for(var key in self.json){
				...
				for(var item in self.manifest[key]){
					if(!(self.manifest[key][item].id)){
		...
		_saveSettingsHandle:function(e){
			...
			for(var m in self.manifest){
				...
				if(/^http|\/|\./.test(m)){
					...
					for(var p in self.json['pages']){
						// page p
						if(self.json['pages'][p].url==m){
					...
					if(__v) {
						console.log("-----------------------");
						console.log("Found the http "+m);
						console.log("NOW THE JSON LOOKS LIKE:");
					...
					for(var o in self.manifest[m]){
						self.json[m].push(self.manifest[m][o]);
					}
					
					if(__v) console.log("So, now self.json["+m+"] looks like: "+JSON.stringify(self.json[m]));
				}
			}
			if(__v) console.log("FINAL JSON SENT TO userPrompt for Save: "+JSON.stringify(self.json));
			self.savePrompt(self.json);
		},
		...
		_exportToXMLHandle:function(e){
			...
			for(var m in self.manifest){
				...
				if(/^http|\/|\./.test(m)){
					...
					for(var p in self.json['pages']){
						// page p
						if(self.json['pages'][p].url==m){
							if(__v) console.log("inserting url "+m+" into "+JSON.stringify(self.json['pages'][p]));
							var temp=$.extend(true,{},self.manifest[m]);
							// if(!self.json[m][p]) self.json[m][p]=[];
							self.json['pages'][p]=temp;
							if(__v) console.log("self.json[pages]"+p+"  now: "+JSON.stringify(self.json['pages'][p]));
							break;
						}
					}
			...
			if(__v) console.log("FINAL JSON SENT TO userPrompt in ExportXML: "+JSON.stringify(self.json));
	...
	PluginController.prototype={
		...
		_toolOutputHandle:function(e,data){
			...
			if((!(self.activeTool))||(self.activeTool.id==data.tool)){
				...
				return;
			} else if(!(self.activeObj)&&(self.activeTool)){
				if(data.attachHandle) self._attachFloatDiv(data.attachHandle,data);
				return;
			}
			...
			if($.isArray(data.obj)){
				...
				self.parseLink(data.obj,self.activeObj);
			}
		...
		_attachFloatDiv:function(id,link,refs){
			...
			self.floatDiv.setInputObject(link,self.manifest[link.id]);
		},
			...
			if((obj1!=null)&&(obj2!=null)){
				...
				if(/http\:|\.[jgpt]*/i.test(obj1.jsonName)){
					...
					if($.inArray(obj2.id,self.linkArray[url][obj1.type][found][obj2.type])){
						self.linkArray[url][obj1.type][found][obj2.type].push(obj2.id);
					}					
					...
					if(!(self.linkArray[obj1.type][p][obj2.type])){
						self.linkArray[obj1.type][p][obj2.type]=[];
					}
					if($.inArray(obj1.id,self.linkArray[obj1.type][found][obj2.type])){
						self.linkArray[obj1.type][found][obj2.type].push(obj2.id);
					}
				}
				...
				if(/http\:|\.[jgpt]*/i.test(obj2.jsonName)){
					...
					if(!(self.linkArray[url][obj2.type][found][obj2.type])){
						self.linkArray[url][obj2.type][found][obj2.type]=[];
					}
					// creating the link between obj1 as object and obj2 as ID reference
					if($.inArray(obj2.id,self.linkArray[url][obj2.type][found][obj2.type])){
						self.linkArray[url][obj2.type][found][obj2.type].push(obj2.id);
					}					
					...
					for(var p in self.linkArray[obj2.type]){
						if(self.linkArray[obj2.type][p].id==obj2.id){
							...
							self.linkArray[obj2.type][p][obj2.type].push(obj2.id);
					...
					if(found<0){
						self.linkArray[obj2.type].push(obj1);
						found=self.linkArray[obj2.type].length-1;
						// self.linkArray[obj2.type][self.linkArray[obj2.type].length-1][obj2.type]=[obj2.id];
						
					}
					if(!(self.linkArray[obj2.type][p][obj2.type])){
						self.linkArray[obj2.type][p][obj2.type]=[];
					}
					if($.inArray(obj2.id,self.linkArray[obj2.type][found][obj2.type])){
						self.linkArray[obj2.type][found][obj2.type].push(obj2.id);
					}
				}
