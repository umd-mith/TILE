	...
	**/
	...
	$.extend(FloatingDiv.prototype, {
		...
		setInputObject:function(o,refs){
			...
			self._curLink=o;
			if(__v) console.log("setInputObject self._curLink: "+JSON.stringify(self._curLink));
			if(__v) console.log("refs passed to setInputObject: "+JSON.stringify(refs));
	...
	**/
	...
	TILE_ENGINE.prototype={
		...
		attachMetadataDialog:function(obj,handle){
			var self=this;
			if(__v) console.log('attachmetadatadialog tile1.0: '+JSON.stringify(obj));
	...
	PluginController.prototype={
		...
		initTools:function(){
			...
			if(self.toolSet){
				...
				// return the interface back to standard
		...
		setJSON:function(){
			...
			// 					self.toolSet[s].loadJSON(ENGINE,activeItems);
			// 				}
			// 			}
		},
		...
		_attachFloatDiv:function(data,handle){
			...
			if(__v) console.log('attachfloatdiv plugincontrol '+JSON.stringify(data));
		...
		addDataToJSON:function(data){
			var self=this;
			// set activeObj if none is set
			if(!self.activeObj){
				self._setActiveObj(data);
				return;
			}
			// Can either add the data to the core JSON or return
			// if already in JSON
			self.parseLink(self.activeObj,data);
			...
			if((obj1!=null)&&(obj2!=null)){
				...
				if(/http\:|\.[jgpt]*/i.test(obj1.jsonName)){
					...
					if($.inArray(obj2.id,json.pages[page][obj1.type][found][obj2.type])){
						...
						if(__v) console.log(obj2.type+" INSERTED IN PAGE: "+json.pages[page][obj1.type][found][obj2.type]);
					}					
					...
					if(__v) console.log(obj2.type+" INSERTED: "+json[obj1.type][found][obj2.type]);
					}
				...
				if(/http\:|\.[jgpt]*/i.test(obj2.jsonName)){
					...
					if($.inArray(obj2.id,json.pages[page][obj2.type][found][obj1.type])){
						...
						if(__v) console.log(obj1.type+" INSERTED IN PAGE: "+json.pages[page][obj2.type][found][obj1.type]);
					}	
				} else {
					var found=-1;
					...
					if($.inArray(obj1.id,json[obj2.type][found][obj1.type])<0){
						...
						if(__v) console.log(obj1.type+" INSERTED: "+json[obj2.type][found][obj1.type]);
					}
				}
					...
					if(json[obj.jsonName][item].id==obj.id){
						...
						if(json[obj.jsonName][item][ref.type]){
							...
							for(var a in json[obj.jsonName][item][ref.type]){
								if(ref.id!=json[obj.jsonName][item][ref.type][a].id){
					...
					if(page[obj.type][item].id==obj.id){
						if(page[obj.type][item][ref.type]){
							...
							for(var a in page[obj.type][item][ref.type]){
								if(ref.id!=page[obj.type][item][ref.type][a].id){
	...
	TileToolBar=function(args){
		...
		// }
			...
			var html='<div class="category inout"><div class="menuitem"><ul><li><a id="save_tags" href="" class="button" title="Save the current session">Save</a></li></ul></div></div>|<div class="category mode"></div>|<div class="category data"></div>';
			// +'<div id="ddown"><div class="menuitem ddown"><p>Tools:</p><select class="menu_body"></select></div>';
			$("#"+self.loc).append(html);
			
			self.SaveB=$("#"+self.loc+" > .category.inout > .menuitem > ul > li > #save_tags");
	...
	TileToolBar.prototype={
		...
		addButton:function(button){
			...
			var ids=[];
			$("#tile_toolbar > div > .menuitem > ul > li > a").each(function(i,el){
				ids.push($(el).attr('id'));
			});
			...
			switch(button.type.toLowerCase()){
				case 'inout':
					$("#tile_toolbar > div.category.inout").append(html);
					break;
				case 'data':
					$("#tile_toolbar > div.category.data").append(html);
					break;
				case 'mode':
					$("#tile_toolbar > div.category.mode").append(html);
					break;
			}
