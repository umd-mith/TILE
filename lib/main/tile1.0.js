	...
	PluginController.prototype={
		...
		initTools:function(json){
			...
			if(self.toolSet){
				...
				// 						self.linkArray[json.pages[page].url]=json.pages[page];
				// 					}
				// 				}
				for(var a in json){
		...
		parseLink:function(obj1,obj2){
			...
			if((obj1!=null)&&(obj2!=null)){
				// store in quick-reference array first
				if(!self.manifest[obj1.id]) self.manifest[obj1.id]=[];
				if(!self.manifest[obj2.id]) self.manifest[obj2.id]=[];
				// make sure they haven't already been linked
					...
					case 'global':
						...
						for(var p in self.linkArray[obj1.type]){
							if(self.linkArray[obj1.type][p].id==obj1.id){
								...
								self.linkArray[obj1.type][p][obj2.type].push(obj2.id);
						...
						if($.inArray(obj2.id,self.linkArray[obj1.type][found][obj2.type])){
					...
					case 'page':
						...
						for(var p in self.linkArray[url][obj1.type]){
							if(self.linkArray[url][obj1.type][p].id==obj1.id){
								self.linkArray[url][obj1.type][p]=obj1;
						...
						if(found<0){
							self.linkArray[url][obj1.type].push(obj1);
