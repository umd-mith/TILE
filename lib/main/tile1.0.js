			...
			if(self.refArray[self.activeObj.id]){
				for(var ref in self.refArray[data.id]){
					...
					if(item.jsonName!=item.type){
						// page level
						if(!json[item.jsonName][item.type]) continue;
						for(var prop in json[item.jsonName][item.type]){
							if(json[item.jsonName][item.type][prop].id==item.id){
								refs.push(deepcopy(json[item.jsonName][item.type][prop]));
							}
						}
