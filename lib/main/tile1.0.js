	...
	**/
	...
	PluginController.prototype={
		...
		_setActiveObj:function(_newActiveObj){
			...
			// 			}
			var refs=[];
			if(self.activeObj.id==_newActiveObj.id) return;
			self.activeObj=null;
			// find object in JSON
			...
			for(var prop in self.activeObj.obj){
				if($.isArray(self.activeObj.obj[prop])){
					...
					for(var id in self.activeObj.obj[prop]){
						var n=self.findRef(self.activeObj.obj[prop][id],prop);
		...
		findObj:function(id,type){
			var self=this;
			var obj=null;
			if(/http:|\.[pngjpggif]/.test(type)){
