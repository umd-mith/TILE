(function ($, R, _) {
	...
	var ShapeData = function(args){
		/*
		 * args:
		 *   canvasId: DOM id of a canvas Element
		 *   
		 */
		this.canvas = $("#"+args.canvasId);
		this.shapes = [];
		
	}
	ShapeData.prototype = {};
	$.extend(ShapeData.prototype, {
...
$(this).removeClass("axe_node_selected");
	 ...
	 */
	var VectorDrawer = function (args) {
		var self = this;
		...
		self._drawMode = args.initDrawMode || 's';
		self._scale = args.initScale || 1;
		...
		self._over = {
			elm: $(args.overElm),
			offset: args.overElm.offset(),
			origWidth: args.overElm.width()/args.initScale,
			origHeight: args.overElm.height()/args.initScale};
		//self._buildCanvas();
	}
				...
				$(this).removeClass("axe_start_node_selected");
					...
					if (!(e.target.id == "selBB")) {
						...
						if (targetObj.con=="path"){
							targetObj.points = _createPointsFromPath(targetObj.cur.attr("path").toString());
								...
								if (tobj.con == "rect") {
									...
									if (tobj.con == "path") {
										cur = self._getCanvasXY(e);
											...
											tobj.points = _createPointsFromPath(tobj.cur.attr("path").toString());
											...
											tobj.cur.attr({
												path:_makePathStr(tobj.points)+"z"
											});
	...
	var TILEDRAW=VectorDrawer.extend({
		init:function(args){
			...
			//global bind for adding a shape outside of the vectordrawer
			$("body").bind("VD_ADDSHAPE",{obj:this},self._addExtraShape);
		...
		//NEW: controls the zoom level of the current image object
		 ...
		 changeINIT_ATTRColor:function(e,hex){
			var self=e.data.obj;
			SHAPE_ATTRS.stroke="#"+hex;
			//if there is a current shape, then change that shape's color
			if(self._obj){
				self._obj.color="#"+hex;
				self._obj.cur.attr(SHAPE_ATTRS);
			}
		 },	
				...
				if(S){
					switch(mode){
						...
						//deactivate all shapes
							jQuery.each(self._allObjs,function(i,o){
								o.deactivate();
							});
							//activate this one shape
							S.activate();
							self._drawMode='s';
							break;
						case 'd':
		...
		sendSingleShape:function(obj){
			...
			$.each(self._allObjs,function(i,o){
				o.deactivate();
			});
			obj.activate();
			//selecting
