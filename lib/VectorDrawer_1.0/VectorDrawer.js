	...
	setHTML:function(){
		...
		_buildCanvas: function() {
			...
			$(window).resize(function(e){
				// console.log('offset:  '+self._canvas.elm.offset().left);
				if(trueSizeX>$(this).width()){
					...
					var offD=($(this).width()-trueSizeX);
					self._canvas.off=(lastSizes['off1']==0)?{'left':self._canvas.elm.offset().left-offD,'top':self._canvas.elm.offset().top}:lastSizes['off1'];
					self._over.offset=(lastSizes['off2']==0)?{'left':self._over.elm.offset().left-offD,'top':self._over.elm.offset().top}:lastSizes['off2'];
				}
			});
