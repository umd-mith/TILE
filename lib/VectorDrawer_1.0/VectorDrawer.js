...
(function ($, R, _) {
	...
	function _createPointsFromPath(path){
		if(__v) console.log('path string/object is: '+path);
	...
	setHTML:function(){
		...
		importShapes: function(shapeStr){
			...
			for (var i=0;i<shapes.length;i++){
				...
				// var p = (self._scale!=s._scale) ? parseFloat(self._scale-s._scale):1;
				// if(self._scale!=s._scale){
				// 					s=self.equalizeScales(s);
				// 				}
				...
				// cur.y = cur.y+parseFloat(self._cont.scrollTop());
                 if (self._drawMode == 'r') {
					...
					if (!(e.target.id == "selBB")) {
						...
						self._obj = targetObj;
						self._start = cur;
						if(__v) console.log('click entered  '+targetObj);
						// Create bounding box
						$(".axe_node").remove();
						if (targetObj.type=="path"){
							targetObj.points = _createPointsFromPath(targetObj.attr("path").toString());
