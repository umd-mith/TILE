...
$(this).removeClass("axe_node_selected");
		...
		importShapes: function(shapeStr){
			...
			for (var i=0;i<shapes.length;i++){
				...
				if (type == 'rect') {
					...
					newEllipse._scale=s._scale;
					this._activeShapes[s.id]=newEllipse;
                }	
				else if (type == 'path') {
					var newPath = this._paper.path();
					newPath.node.id = s.id;
					newPath._scale=s._scale;
		...
		exportShapes: function(){
			...
			_.each(this._activeShapes, function (o) {
				...
				thisShape = {"id": id, "type": type, "_scale": this._scale,"color":o.attr('stroke'),"posInfo":posInfo};		
		...
		scale: function(x) {
			...
			//self._paper.node.attr("height",(self._cont.height()*self._scale)+'px');
			// var w=parseFloat(self._canvas.elm.attr('width'));
			// 			self._canvas.elm.css('width',(self._scale*parseFloat(self._canvas.elm.attr('width')))+'px');
			// 			self._canvas.elm.css('height',(self._scale*parseFloat(self._canvas.elm.attr('height')))+'px');
			 $(".axe_node").remove();
		...
		_buildCanvas: function() {
			...
			//self._over.offset.left-10, top: self._over.offset.top-10, position: "absolute"});
			
			self._paper = R(self._cont[0],
			self._over.elm.width()-10, self._over.elm.height()-10);
		...
		// 					throw "Unrecognized shape";
		// 				}	
		// 				thisShape = {"id": id, "type": type, "scale": o.scale, "color":o.attr('stroke'),"posInfo":posInfo};		
		// 				JSONoutput.push(thisShape);
		// 				});
		// 						 
		// 				return JSONoutput;		
		...
		_getCanvasXY: function (e) {
			...
			return {
				x: (e.clientX-self._canvas.off.left)+parseFloat(self._cont.parent().scrollLeft()),
				y: ((e.clientY-self._canvas.off.top)+parseFloat(self._cont.parent().scrollTop()))+$("#raphshapebar_").height()
			};
		},
				...
				// cur.y = cur.y+parseFloat(self._cont.scrollTop());
                 if (self._drawMode == 'r') {
					...
					self._obj.attr({"stroke":self._penColor});
						var randD=new Date(); 
						var id=randD.getTime('seconds')+"_"+_.size(self._allObjs);
						...
						// 						scale:self._scale
						// 					};
						// 					$("body:first").trigger("shapeDrawn",[newR]);
					...
					self._obj.attr({"stroke":self._penColor});
						var randD=new Date(); 
						var id=randD.getTime('milliseconds')+"_"+_.size(self._allObjs);
						$.data(self._obj.node,'uid',id);
						self._obj.node.id=id;
						// var newE={
						// 						"id":id,
						...
						// 					
						// 					
						// 					$("body:first").trigger("shapeDrawn",[newE]);
                    // self._obj.attr(SHAPE_ATTRS);
                     ...
                     if (self._points) {
                         ...
                         if (maybeClosing) {
                             ...
                             if (bbox.width > TOO_SMALL && bbox.height > TOO_SMALL) {
								...
								//TILE tag
                         		var randD=new Date(); 
								var id=randD.getTime('minutes')+"_"+_.size(self._allObjs);
								self._obj.node.id=id;
                 ...
                 if (self._drawMode == 'r' || self._drawMode == 'e') {
					...
					self._obj.attr({"stroke":self._penColor});
                     ...
                     if (self._drawMode == 'r') {
						 ...
						 posInfo = {"x":as.x,"y":as.y,"width":as.width,"height":as.height};
						 shapeObj = {"id": id,"type":"rect","_scale": self._scale,"color":"#000000","posInfo": posInfo};
						
                     } else if (self._drawMode == 'e') {
                         var as = self._obj.attr(["cx", "cy", "rx", "ry"]);
						 var id=""+$.data(self._obj.node,"uid");
						 posInfo = {"cx":as.cx,"cy":as.cy,"rx":as.rx,"ry":as.ry};
                         shapeObj = {"id": id,"type":"ellipse","_scale": self._scale,"color":"#000000","posInfo": posInfo};	
