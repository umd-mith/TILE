					...
					$("#selBB").resizable({
						...
						resize: function(e, ui){
							...
							if (tobj.type == "rect") {
								...
								var xDiff=ui.originalPosition.left-$(".vd-container").position().left+$(".workspace").scrollLeft();
								var yDiff=ui.originalPosition.top-$(".vd-container").position().top+$(".workspace").scrollTop();
				...
				$(this).removeClass("axe_start_node_selected");
					...
					$("#selBB").draggable({
						...
						drag: function(e, ui){
							tobj = self._tarObj;
							var rs = _relScale(self, tobj);
							xDiff = parseFloat(ui.position.left)-parseFloat(self._over.offset.left)+$(".workspace").scrollLeft();
							yDiff = parseFloat(ui.position.top)-parseFloat(self._over.offset.top)+$(".workspace").scrollTop();
							if(__v) console.log("dragging shape toward: "+xDiff+", "+yDiff);
							if (tobj.type == "rect") {
								//self._activeShapes[tobj.id].attr({
								tobj.attr({
					...
					if($("#selBB > .shpButtonHolder").length==0) $("body:first").trigger("shapeActive",[$(targetObj.node).attr("id")]);
