(function ($, R, _) {
	...
	var TILEDRAW=VectorDrawer.extend({
		...
		_addExtraShape:function(e,shape){
			...
			$.extend(shape,{
				cur:null,
				scale:shape.points.scale,
				id:shape.uid,
				shapeChangeCall:"shapeCoordUpdate"+shape.uid,
				anchor:false,
				_display:false,
				deactivate:function(){
					this.cur.hide();
			...
			self._manifest[shape.uri.replace(/^[ \t\r\n]+|[ \t\r\n]+$/,"")].push(shape);
			alert('sending shape '+shape.uid);
			self.DOM.trigger("shapeCommit",[shape]);
		},
