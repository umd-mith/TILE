...
(function ($, R, _) {
	...
	setHTML:function(){
		...
		importShapes: function(shapeStr){
			var self=this;
			var shapes=shapeStr;
			if(__v) console.log('LOADING SHAPES: VECTORDRAWER');
			if(__v) console.log(shapes);
			...
			for (var i=0;i<shapes.length;i++){
				...
				if (type == 'rect') {
					...
					if(__v) console.log("finished drawing "+s);
			...
			if(__v) console.log('SHAPES IMPORTED: VECTORDRAWER');
		...
		clearShapes: function(){
			...
			$(".axe_node").remove();
			if(__v) console.log("vectordrawer cleared shapes");
		},
			...
			if(self._activeShapes[id]){
				...
				$("#selBB").draggable({
					...
					drag: function(e, ui){
						...
						if(__v) console.log("dragging shape toward: "+xDiff+", "+yDiff+"  ui.position: "+ui.position.left+"  "+ui.position.top);
						if (tobj.type == "rect") {
		...
		setPenColor:function(_newColor){
			...
			if(__v) console.log("self._tarObj has color: "+self._tarObj.color);
		...
		_buildCanvas: function() {
			...
			$(window).resize(function(e){
				...
				if(lastSizes['off1'].left<self._canvas.elm.offset().left){
					//var xd=self._canvas.elm.offset().left-lastSizes['off1'].left;
					//var yd=self._canvas.elm.offset().top-lastSizes['off1'].top;
				...
				/*
				if(trueSizeX>$(this).width()){
					if((nextSizes['off1']==0)&&(nextSizes['off2']==0)){
						lastSizes['off1']=self._canvas.off;
						lastSizes['off2']=self._over.offset;
						self._canvas.off=self._canvas.elm.offset();
						self._over.offset=self._over.elm.offset();
					} else {
						self._canvas.off={'left':(self._canvas.elm.offset().left-self._cont.offset().left),'top':(self._canvas.elm.offset().top-self._cont.offset().top)};
						self._over.offset=self._over.elm.offset();
					}
					
				} else if(trueSizeX<$(this).width()) {
					if((lastSizes['off1']==0)&&(lastSizes['off2']==0)){
						nextSizes['off1']=self._canvas.off;
						nextSizes['off2']=self._over.offset;
						self._canvas.off=self._canvas.elm.offset();
						self._over.offset=self._over.elm.offset();
					} else {
						self._canvas.off={'left':(self._canvas.elm.offset().left-self._cont.offset().left),'top':(self._canvas.elm.offset().top-self._cont.offset().top)};
						self._over.offset=lastSizes['off2'];
					}
                 ...
                 if (self._drawMode == 'r') {
					...
					if (!(e.target.id == "selBB")) {
						...
						targetObj = _.first(_.select(self._activeShapes, function(o){
							if(__v) console.log("self._activeShapes o: "+o.attr('id'));
						    if (!(o.type=="skip")) {
						...
						if(__v) console.log('targetObj found: '+targetObj+" activeShapes: "+self._activeShapes);
						if (!targetObj) {
							$("#selBB").remove();
							return;
						}
					...
					$("#selBB").draggable({
						...
						drag: function(e, ui){
							tobj = self._tarObj;
							var rs = _relScale(self, tobj);
							if(__v) console.log("ui.position.left: "+ui.position.left+" top: "+ui.position.top);
							xDiff = parseFloat(ui.position.left)+$(".workspace").scrollLeft();
							yDiff = parseFloat(ui.position.top)+$(".workspace").scrollTop();
							if(__v) console.log("dragging shape toward: "+xDiff+", "+yDiff);
							if (tobj.type == "rect") {
								//self._activeShapes[tobj.id].attr({
								tobj.attr({
									x: xDiff,
						...
						stop:function(e,ui){
							...
							$(this).trigger("shapeChanged",[tobj]);
