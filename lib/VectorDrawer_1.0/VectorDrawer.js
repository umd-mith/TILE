	...
	setHTML:function(){
		...
		importShapes: function(shapeStr){
			...
			for (var i=0;i<shapes.length;i++){
				...
				if (type == 'rect') {
					...
					var newPath = this._paper.path(s.posInfo);
		...
		exportShapes: function(){
			...
			_.each(this._activeShapes, function (o) {
				...
				var p =  (this._scale)/(o._scale);
				if (type == 'rect') {
                    var as = o.attr(["x", "y", "width", "height"]);
					 posInfo = {"x":p*as.x,"y":p*as.y,"width":p*as.width,"height":p*as.height};
                } else if (type == 'ellipse') {
                    var as = o.attr(["cx", "cy", "rx", "ry"]);
					posInfo = {"cx":p*as.cx,"cy":p*as.cy,"rx":p*as.rx,"ry":p*as.ry};
                }	
				else if (type == 'path') {
                    var path = ""+o.attr("path");
