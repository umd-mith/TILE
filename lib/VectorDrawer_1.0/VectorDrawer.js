(function ($, R, _) {
	...
	setHTML:function(){
		...
		var newNode = $(self._html).appendTo((self._loc)).mouseenter(function(){	
			$(this).css({'width':'10px', 'height':'10px','background-image':'url("../images/axe_blue_large.png")'}).addClass("node_selected");
			...
			$(this).css({'width':'7px', 'height':'7px','background-image':'url("../images/axe_blue_small.png")'}).removeClass("node_selected");
		}).draggable({
	...
	var TILEDRAW=VectorDrawer.extend({
		...
		_addExtraShape:function(e,shape,args){
			...
			for(x in shape){
				...
				$.extend(o,{
					cur:null,
					id:(o.uri)?o.uri:$("#srcImageForCanvas").attr("src"),
					...
					if (!(e.target.id == "selBB")) {
						...
						if (targetObj.con=="path"){
							...
							_.each(targetObj.points, function(p){
							 ...
							 var node = new TILENode({pos:{
									x: p.x,
									y: p.y
								},shape:targetObj,vd:self,offsets:{offsetTop: self._offTop, offsetLeft: self._offLeft}});
