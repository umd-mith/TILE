...
(function ($, R, _) {
	...
	var INIT_ATTRS = {"stroke-width": "2px", "stroke": "#a12fae"};
	var SELECTED_ATTRS = {"stroke-width": "2px", "stroke": "#ff6666"};
	...
	function _relScale(vd, o) {
		if(__v) console.log('what is the scale? '+o._scale);
	...
	function _isNear(p, o,rs) {
		...
		if (o.type == "rect") {
			...
			if(__v) console.log(JSON.stringify(ul)+' '+JSON.stringify(ur)+' '+JSON.stringify(ll)+'  '+JSON.stringify(lr));
	...
	setHTML:function(){
		...
		importShapes: function(shapeStr){
			...
			var shapes=shapeStr;
			if(__v){
				console.log('importing into VD: '+typeof(shapeStr));
                 ...
                 if (self._drawMode == 'r') {
					...
					if (!(e.target.id == "selBB")) {
						...
						$.each(self._activeShapes, function(i,sh){
							...
							if(sh){
								if(__v) console.log('isNear is returning '+(_isNear(cur, sh, self._scale)));
						...
						if(__v) console.log('clicked on shape for '+targetObj);
						if (!targetObj) {
					...
					if(__v) console.log('finding bounding box for '+targetObj);
					var bb = targetObj.getBBox();
					var bbH = parseInt(bb.height,10);
					var bbW = parseInt(bb.width,10);
