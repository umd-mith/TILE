 ...
 var CanvasImage=Image.extend({
	...
	//	e.stopPropagation();
		...
		if(obj.Region){
			...
			for (var y=0; y<obj.Region.h; y++)
			{
	
			  for (var x=0; x<obj.Region.w; x++)
			  {
			    // var index=(i*4)*imageData.width+(j*4);
			     var index=(x +y*obj.Region.w)*4;
				...
				if(this.dots[y][x]>=0){
					...
					if(dot==0){
						...
						if (sides == 0) {
							//new shape 
							blobcount++;
							this.dots[y][x] = blobcount;
							this.shapes[this.dots[y][x]]=new Shape();
	...
	/*	var gds=jQuery.grep(this.shapes,function(n,i){
			return (n&&(n.coords.length>=15));
		});
		this.shapes=gds;
	*/
	    setTimeout(function(obj){
			...
			if (obj.shapes.length > 3) {
				for (var n = 3; n < obj.shapes.length; n++) {
					//showMe=showMe+","+obj.shapes[n].Right;
					if ((obj.shapes[n-1].Right > obj.shapes[n].Right)) 
					//&&
					//((obj.shapes[(n-3)].Right > obj.shapes[n].Right)))
					 {
						obj.lineBreaks[n] = true;
					}
					prev = n;
				}
				obj.colorEndings();
	...
	createWordBlobs:function(){
		...
		for(y in this.dots){
			var row=parseInt(y.substring(1),10);
			for(x in this.dots[y]){
				var col=parseInt(y.substring(1),10);
				var shape=this.dots[y][x];
	...
	fixWordBlobs:function(){
		var gds=jQuery.grep(this.shapes,function(n,i){
			return (n.length>=DOTTHRESH);
		});
	},
