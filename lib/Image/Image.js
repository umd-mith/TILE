...
var RaphaelImage=Image.extend({
	init:function(args){
		...
		$("body").bind("setCoordRegion",{obj:this},this.editCoordRegion);
	...
	zoom:function(e,val){
		...
		if(obj.srcImage&&obj.image){
			...
			} else if(obj.zoomLevel>obj.zoomMin){
				obj.zoomLevel=0;
	...
	figureSizeInfo:function(min,max){
		...
		this.zoomIncrement=Math.ceil(this.srcImage[0].height/this.zoomMax);alert(this.zoomIncrement);
		for(i=this.zoomMin;i<this.zoomMax;i++){
			this.zoomLevels[i]=(this.zoomIncrement*i);
		}
	...
	drawRectangle:function(e){
		...
		if(obj.canvasObj&&(obj.shapeData[obj.currShape])){
			if(obj.shapeData[obj.currShape].shape) obj.shapeData[obj.currShape].shape.destroy();
			var l=10;
			var t=10;
			var r=100;
			var b=100;
			obj.shapeData[obj.currShape].shape=new SVGRectangle({
											left:l,
											top:t,
											right:r,
											bottom:b,
	...
	drawEllipse:function(e){
		...
		if(obj.canvasObj&&(obj.shapeData[obj.currShape])){
			if(obj.shapeData[obj.currShape].shape) obj.shapeData[obj.currShape].shape.destroy();
				...
				obj.shapeData[obj.currShape].shape=new SVGEllipse({
					cx:100,
					cy:45,
					rx:100,
					ry:20,
	...
	drawPolygon:function(e){
		...
		if(obj.canvasObj&&(obj.shapeData[obj.currShape])){
			...
			obj.shapeData[obj.currShape].shape=new SVGPolygon({
				svg:obj.canvasObj,
				loc:obj.DOM,
				color:"#FF0000"
			});
	...
	sendCoordData:function(e){
		...
		if(obj.shapeData[obj.currShape]&&obj.shapeData[obj.currShape].shape){
			...
			obj.DOM.trigger("coordDataSent",[data]);
		}
	},
	...
	deleteShapeData:function(e,id){
		var obj=e.data.obj;
		if(obj.shapeData[id]){
			if(obj.shapeData[id].shape) obj.shapeData[id].shape.destroy();
			obj.shapeData[id]=null;
