...
var RaphaelImage=TileImage.extend({
	init:function(args){
		...
		$("body").bind("deleteCurrentShape",{obj:this},this.resetImageClick);
	...
	//	$("body").bind("requestCoordData",{obj:this},this.sendCoordData);
		...
		//$("body").bind("zoom",{obj:this},this.zoom);
		$("body").bind("clearSession",{obj:this},this.terminateSession);
	...
	addUrl:function(url){
		if((typeof url=="object")&&(url!=null)){
			...
			for(u in url){
				...
				if((typeof url[u].uri != "undefined")&&(url[u].uri!="")&&(jQuery.inArray(url[u].uri,this.url)<0)){
	...
	setPageInfo:function(url){
		if((this.curUrl==url)&&(this.manifest[url])){
			...
			//	alert('zoommax: '+this.zoomMax+' zoommin: '+this.zoomMin+' '+this.zoomLevel+' beginning scale: '+(this.zoomLevel/this.zoomMax));
			...
			if(!this.drawTool){
				...
				this.drawTool.switchImage(this.curUrl,this.manifest[this.curUrl].shapes,(this.zoomLevel/this.zoomMax));
			}
	...
	goToPage:function(uri){
		var uri=uri.replace(/[\t\n\r]+/i,"");
		//find the correct page number from array
		var patt=new RegExp(uri);
		for(i=0;i<this.url.length;i++){
			//have to use regex because simple pattern matching doesn't seem to work
			if(patt.test(this.url[i])){
				this.pageNum=i;
				break;
			}
		}
		if(this.pageNum>-1){
			//var file=this.url[this.pageNum];alert("gotopage this.url[this.pageNum]  "+file);
			
			if(/[^.jpg|.tif|.png]/.test(uri)){
				this.setUpCanvas(uri);
				
			} else {
				alert("The file: "+uri+" returned an error.");
			}
		}
	},
	showCurrentImage:function(e){
	...
	**/
		...
		if(obj.manifest[obj.curUrl].tags){
			...
			if(test.length==0){
				...
				obj.manifest[obj.curUrl].tags.push({
					tagEl:tag,
					shape:tag.shape,
					n:(obj.manifest[obj.curUrl].tags.length-1)
				});
	...
	bundleData:function(){
		...
		var data=[];
		var date=new Date();
		var json="{uid:"+date.getDate();
		var images="Images:[";
		var tags="Tags:[";
		var shapes="Shapes:[";
		for(p in this.manifest){
			var record=this.manifest[p];
			images+="{uid:"+date.getTime()+",uri:"+p+",tags:[";
			for(t=0;t<this.manifest[p].tags;t++){
				...
				tags+=tgdata[0];
				if(tgdata[1]) shapes+=tgdata[1];
				
			}
			// json+="url:"+p+",tags:[";
			// 			//go through the tags
			// 			for(t=0;t<record.tags.length;t++){
			// 				json+=((t+1)==record.tags.length)?record.tags[t].bundleData():record.tags[t].bundleData()+",";
			// 			}
			// 			
			// 			json+="],shapes:[";
			// 			for(s=0;s<record.shapes.length;s++){
			// 				
			// 			},
			// 			json+="]}";
			// 			data[p]=json;
	...
	//destroy all tags that were made in this session
	//called by clearSession
	terminateSession:function(e){
		var obj=e.data.obj;
		for(p in obj.manifest){
			for(t=0;t<obj.manifest[p].tags;t++){
				obj.manifest[p].tags[t].destroy();
			}
		}
	}
