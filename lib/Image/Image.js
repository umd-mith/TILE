	...
	createWordBlobs:function(){
		...
		var cleanerBlobs=jQuery.grep(this.wordBlobs,function(value,index){
			if(value.coords.length>DOTTHRESH){
				...
				var misfitcoords=[];
				if(value.coords) jQuery.merge(misfitcoords,value.coords);
				//check previous
				if(FULL[(index-1)]){//is there a previous?
					var prev=FULL[(index-1)];
					
					//test to see if prev has less than needed dots
					/*if(prev.coords.length<=DOTTHRESH) {//still below thresh?
					...
					}*///end still below thresh
				/*	if(prev.coords.length<=DOTTHRESH&&(FULL[index+1])){//even now below thresh?
						//go forward now
						prev=FULL[index+1];
						jQuery.merge(misfitcoords,prev.coords);
						k=2;
			...
			*/
				...
				//end: is there previous?
				} if(FULL[(index+1)]){//now look at the one right ahead
					var next=FULL[(index+1)];
					if(next.coords.length>DOTTHRESH){
						//passes test
						for(i=0;i<misfitcoords.length;i++){
							var val=misfitcoords[i];
				...
				*/
				...
				return false; //get rid of this one
			}//end Else
		});//END grep
