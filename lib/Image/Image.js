 ...
 var CanvasImage=Image.extend({
				...
				if(this.dots[y][x]>=0){
					...
					if(dot==0){
						...
						if (sides == 0) {
							...
							this.shapes[this.dots[y][x]]=new Shape({index:blobcount});
	...
	createLineBreaks:function(){
		...
		if (this.shapes.length > 3) {
			for (var n = 1; n < this.shapes.length; n++) {
				if ((this.shapes[n-1].Right > this.shapes[n].Right)) 
				 {
	...
	cleanLineBreaks:function(){
		for(c=0;c<this.lineBreaks.length;c++){
			if(this.lineBreaks[(c-1)]){
				shape=this.lineBreaks[c];
				pshape=this.lineBreaks[(c-1)];
				if((shape.Top-pshape.Top)<=10){
					a=this.lineBreaks.slice(0,c);
					b=this.lineBreaks.slice((c+1));
					this.lineBreaks.concat(a,b);
				}
			}
		}
	},
	...
	**/
	...
	createCleanShapes:function(){
		//RETURNS ARRAY
		//filter out noise from shapes array
		//Noise is any shape whose coords array 
		//does not meet threshold
		var DOTTHRESH=this.dotThresh;
		var clean=jQuery.grep(this.shapes,function(n,i){
			
			return ((n)&&(n.coords.length>DOTTHRESH));
		});
		return clean;
	},
	convertShapesToLines:function(){
		...
		for(s=0;s<this.shapes.length;s++){
			...
			if(jQuery.inArray(shape.index,this.lineBreaks)>0){
				...
				var left=(this.Region.ox+shape.Left+this.canvas.position().left);
