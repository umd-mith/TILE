 ...
 */
 ...
 var CanvasImage=Image.extend({
 	init:function(args){
		...
		this.dotThresh=(args.dotMin)?args.dotMin:10;
		this.Region=null;
	...
	filterDots:function(){
					...
					if(dot==0){
						...
						if(this.dots[i][kl]){
							if((this.dots[i][kl]>0)){
								...
								// 								var section=this.wordBlobs[k];
								// 								if(section.shape==this.dots[kl][j]){
								// 									section.coords.push([col,row]);
								// 									
								// 								}
								// 							}
								sides++;
		...
		//FOR THE FUTURE:
		//now go through and filter out noise
		//Noise: blobs with less than threshold of 
		//dots in them
	//	var filternoise=[];
	...
	createRectangles:function(){
		...
		for(i in this.dots){
			...
			for(j in this.dots[i]){
				var col=parseInt(j.substring(1));
