 ...
 var CanvasImage=TileImage.extend({
	...
	showCurrentImage:function(e){
		...
		if(obj.url[obj.pageNum]){
			var file=obj.url[obj.pageNum];alert('showcurrentimagea file: '+file);
...
var RaphaelImage=TileImage.extend({
	...
	addUrl:function(url){
		if((typeof url=="object")&&(url!=null)){
			...
			for(u in url){
				...
				if((typeof url[u].uri != "undefined")&&(jQuery.inArray(url[u].uri,this.url)<0)){
	...
	goToPage:function(uri){
		...
		if(this.url[this.pageNum]){
			var file=this.url[this.pageNum];
			if(/[^.jpg|.tif|.png]/.test(file)){
				this.setUpCanvas(file);
	...
	changeShapeData:function(e,id,tag){
		...
		if(obj.manifest[obj.curUrl].tags){
			...
			if(test.length==0){
				...
				//set listener for shape 
				if(!tag.shape) $(obj.image.node).bind("mouseup",{obj:obj},obj.handleMouseCanvasClick);
			} else {
