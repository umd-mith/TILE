	...
	testDot:function(dot){
		...
		while(sides>0){
			...
			if(this.regionData.data[dotdown]){
				...
				if((((red+green+blue)/3)==0)&&(jQuery.inArray(trackarray,[x,nextdoty])<0)){
			...
			if(this.regionData.data[dotright]){
				...
				if((((red+green+blue)/3)==0)&&(jQuery.inArray(trackarray,[nextdotx,y])<0)){
					trackarray.push([x,nextdoty]);
					sides++;
			...
			if(x>0){
				...
				if(this.regionData.data[dotleft]){
					var red=this.regionData.data[dotleft];	  
				    var green=this.regionData.data[dotleft+1];
				    var blue=this.regionData.data[dotleft+2];
					if((((red+green+blue)/3)==0)&&(jQuery.inArray(trackarray,[(x-1),y])<0)){
			...
			if(sides==0){
				...
				if(jQuery.inArray(trackarray,[nextdotx,y])<0){
					x=nextdotx;
					nextdotx+=1;
				} else if(jQuery.inArray(trackarray,[x,nextdoty])<0){
					y=nextdoty;
					nextdoty+=1;
				} else {
					x=(x-1);
