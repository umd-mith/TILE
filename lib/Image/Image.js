...
var RaphaelImage=TileImage.extend({
	...
	//JSON-like associative array {uri:"",tags:[]}
	//{String} representing a factual URI
	//{Object} json - optional JSON object for adding shapes
	addUrl:function(url){
	...
	setUpCanvas:function(url,max,min){
		...
		//try to slow down the process in order 
		//to make sure image is appended and loaded
	...
	testURL:function(url){
		return $.ajax({url:url,async:false,dataType:'text'}).responseText;
	},
	...
	**/
	...
	bundleData:function(){
		...
		for(p in this.manifest){
			...
			// 				
			// 				var tg=record.tags[t].tagEl;
			// 				//alert(t+":  "+tg.htmlindex);
			// 				i.tags.push(tg.htmlindex);
			// 				
			// 				var tgdata=tg.bundleData();alert(tgdata); //creates an array
			// 				tags.push(tgdata);
