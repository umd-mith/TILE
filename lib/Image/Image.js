...
var RaphaelImage=TileImage.extend({
	init:function(args){
		...
		$("body").bind("deleteCurrentShape",{obj:this},this.deleteShape);
	...
	drawRectangle:function(x,y){	
		if(this.image&&this.canvasObj&&this.tagData[this.currShape]){
			...
			var r=(x+100);
			var b=(y+100);
			this.tagData[this.currShape].shape=new SVGRectangle({
											left:x,
											top:y,
											right:r,
											bottom:b,
											...
											htmlID:this.tagData[this.currShape].index
										});							
	
			//done, send to the tag
			this.DOM.trigger("coordDataSent",[this.tagData[this.currShape].shape,this.tagData[this.currShape].shape.getData()]);
	...
	drawEllipse:function(x,y){
		...
		if(this.image&&this.canvasObj&&(this.tagData[this.currShape])){
			if(this.tagData[this.currShape].shape) this.tagData[this.currShape].shape.destroy();
				...
				this.tagData[this.currShape].shape=new SVGEllipse({
					cx:x,
					cy:y,
					rx:100,
					ry:20,
	...
	drawPolygon:function(x,y){
		...
		if(this.canvasObj&&this.image&&(this.tagData[this.currShape])){
			if(this.tagData[this.currShape].shape) this.tagData[this.currShape].shape.destroy();
		
			this.tagData[this.currShape].shape=new SVGPolygon({
				svg:this.canvasObj,
				svgImage:this.image,
				loc:this.DOM,
				color:"#FF0000",
	...
	editCoordRegion:function(e,id){
		...
		if(!obj.tagData[id]) {
			...
			if(obj.tagData[id].shape){
				obj.tagData[id].shape.releaseAnchor();
			} else {
				obj.drawRectangle(e);
			}
		}
	...
	**/
	changeShapeData:function(e,id,tag){
		...
		obj.currShape=tag.htmlindex;
		if(obj.tagData[tag.htmlindex]){
			
			if(!obj.tagData[obj.currShape].shape){
				$(obj.image.node).bind("mouseup",{obj:obj},obj.handleMouseCanvasClick);
			}
		} else {
	
			obj.tagData[obj.currShape]={
				tag:tag,
				n:id,
				shape:null
			};
	...
	deleteShape:function(e){
		...
		if(obj.tagData[obj.currShape]){
			if(obj.tagData[obj.currShape].shape) {
				obj.tagData[obj.currShape].tag.shapeRemoved();
				obj.tagData[obj.currShape].shape=null;
			}
			$(obj.image.node).bind("mouseup",{obj:obj},obj.handleMouseCanvasClick);
			obj.drawMode=true;
		}
	...
	bundleData:function(){
		var data=[];
		for(d in this.url){
			if(this.url[d]) data.push(this.url[d].uri);
		}
		return data;
	}
