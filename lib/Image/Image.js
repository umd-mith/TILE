...
var RaphaelImage=Image.extend({
	init:function(args){
		...
		this.canvasObj=Raphael(this.DOM[0],this.srcImage.width(),this.srcImage.height());
	...
	zoom:function(e,val){
		...
		if(obj.srcImage&&obj.image){
			if((val>0)&&(obj.zoomLevel<=0)){
				...
				var w=obj.srcImage[0].width;
				var h=obj.srcImage[0].height;
				obj.image.attr({width:w,height:h});
	...
	setUpCanvas:function(url){
		...
		this.zoomLevel=2;
		this.size={w:this.srcImage[0].width,h:this.srcImage[0].height,zoomLevel:this.zoomLevel};
		...
		//this.canvasObj.nodeattr({width:this.size.w,height:this.size.h});
