...
var TileScrollingImages=ScrollingImages.extend({
	...
	loadImages:function(data){
		...
		for(d=0;d<data.length;d++){
			...
			li.attr("id",function(e){
				return "scrollDiv_"+$("#scrollerlist > div").length;
			});
			if(/http:\/\//.test(data[d])){
			...
			this.items[li.attr("id")]={html:li.attr("id"),values:data[d],n:($("#scrollerlist > div").length)};
			li.click(function(e){
	...
	scrollThumbClick:function(e,id){
		...
		obj.DOM.trigger("switchImage",[item.n]);
	},
	minimize:function(e){
		var obj=e.data.obj;
